// Skeleton Binance adapter (implements ExchangeAdapter interface)
// NOTE: do NOT store keys here. Use process.env or secret manager.
import ccxt from 'ccxt';
import type { ExchangeAdapter, OrderBook, FundingInfo } from './exchange_adapter';

export const BinanceAdapter = (opts: { apiKey?: string; secret?: string; test?: boolean }): ExchangeAdapter => {
  const id = 'binance';
  ...
  return {
    ...
    async fetchServerTime() {
      try { return Date.now(); } catch { return Date.now(); }
    },
    ...
    async fetch24hVolume(symbol: string) {
      const t = await exchange.fetchTicker(symbol);
      const vol = Number(t.quoteVolume || 0) * Number(t.last || 0);
      return vol;
    },
