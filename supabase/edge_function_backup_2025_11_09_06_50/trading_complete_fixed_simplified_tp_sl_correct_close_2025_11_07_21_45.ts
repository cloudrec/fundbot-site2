...
serve(async (req) => {
  ...
  try {
    console.log('🔥 COMPLETE FIXED Gate.io Function started');
...
// 🆕 ПОЛНАЯ ФУНКЦИЯ СОЗДАНИЯ ПОДПИСИ ДЛЯ GATE.IO API V4
...
async function getBalance(apiKey: any, settings: any) {
  try {
    console.log('🔥 COMPLETE GATE.IO getBalance started for exchange:', apiKey.exchange);
    ...
    // 🆕 ПОЛНАЯ GATE.IO API V4 РЕАЛИЗАЦИЯ
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing COMPLETE GATE.IO API v4 balance request');
      ...
      console.log('🔥 COMPLETE GATE.IO v4: Base URL:', baseUrl);
      console.log('🔥 COMPLETE GATE.IO v4: Prefix:', prefix);
      console.log('🔥 COMPLETE GATE.IO v4: URL path:', url);
      console.log('🔥 COMPLETE GATE.IO v4: Full URL:', baseUrl + prefix + url);
      ...
      console.log('🔥 COMPLETE GATE.IO v4 balance request URL:', fullUrl);
      ...
      const data = await response.json();
      console.log('🔥 COMPLETE GATE.IO v4 balance response status:', response.status);
      console.log('🔥 COMPLETE GATE.IO v4 balance response:', data);

      if (response.status !== 200) {
        throw new Error(`COMPLETE GATE.IO v4 API Error: ${data.message || 'Unknown error'} (Code: ${response.status})`);
      }
      ...
      const result = {
        ...
        result: data,
        debug_url: fullUrl,
        api_version: 'COMPLETE_GATE_V4',
        base_url: baseUrl,
        signature_format: 'COMPLETE_DOCUMENTATION_BASED'
      };
      
      console.log('🔥 COMPLETE GATE.IO v4 balance result:', result);
      return result;
    }
    ...
    console.error('🔥 COMPLETE GATE.IO Balance error:', error);
...
async function placeOrderWithTPSL(apiKey: any, settings: any) {
  try {
    console.log('🔥 COMPLETE GATE.IO placeOrderWithTPSL started for exchange:', apiKey.exchange);
    ...
    console.error('🔥 COMPLETE GATE.IO Set TP/SL error:', error);
    throw error;
  }
}
...
async function getPositions(apiKey: any, settings: any) {
  try {
    console.log('🔥 COMPLETE GATE.IO getPositions started for exchange:', apiKey.exchange);
    
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing COMPLETE GATE.IO API v4 positions request');
      ...
      console.log('🔥 COMPLETE GATE.IO v4 positions URL:', baseUrl + prefix + url);
      ...
      const data = await response.json();
      console.log('🔥 COMPLETE GATE.IO v4 positions response status:', response.status);
      console.log('🔥 COMPLETE GATE.IO v4 positions response:', data);

      if (response.status !== 200) {
        throw new Error(`COMPLETE GATE.IO v4 Positions API Error: ${data.message || 'Unknown error'} (Code: ${response.status})`);
      }

      // Фильтруем только открытые позиции
      const openPositions = data.filter((position: any) => parseFloat(position.size) !== 0);
      
      console.log('🔥 COMPLETE GATE.IO v4 open positions:', openPositions);
    ...
    console.error('🔥 COMPLETE GATE.IO Positions error:', error);
...
async function cancelAllOrders(apiKey: any, settings: any) {
  try {
    console.log('🔥 COMPLETE GATE.IO cancelAllOrders started for exchange:', apiKey.exchange);
    
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing COMPLETE GATE.IO API v4 cancel orders request');
      ...
      try {
        ...
        console.log('🔥 COMPLETE GATE.IO v4 cancel orders URL:', baseUrl + prefix + ordersUrl);
        ...
        console.log('🔥 COMPLETE GATE.IO v4 cancel orders response status:', ordersResponse.status);
        console.log('🔥 COMPLETE GATE.IO v4 cancel orders response:', ordersData);
        ...
        console.error('🔥 COMPLETE GATE.IO v4 orders cancel error:', regularError);
        cancelResults.push({
          type: 'REGULAR_ORDERS',
          cancelled: 0,
          status: 'ERROR',
      ...
      return {
        ...
        exchange: 'GATE',
        status: 'LIVE',
        message: `Отмена ордеров COMPLETE GATE.IO v4: ${totalCancelled} ордеров отменено`,
        api_version: 'COMPLETE_GATE_V4',
        symbol: symbol,
    ...
    console.error('🔥 COMPLETE GATE.IO Cancel orders error:', error);
...
async function closeAllPositions(apiKey: any, settings: any) {
  try {
    console.log('🔥 COMPLETE GATE.IO closeAllPositions started for exchange:', apiKey.exchange);
    
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing COMPLETE GATE.IO API v4 close positions request');
      ...
      console.log('🔥 COMPLETE GATE.IO v4 get positions URL:', baseUrl + prefix + positionsUrl);
      ...
      console.log('🔥 COMPLETE GATE.IO v4 positions response status:', positionsResponse.status);
      console.log('🔥 COMPLETE GATE.IO v4 positions response:', positionsData);

      if (positionsResponse.status !== 200) {
        throw new Error(`COMPLETE GATE.IO v4 Positions API Error: ${positionsData.message || 'Unknown error'} (Code: ${positionsResponse.status})`);
      }
      ...
      console.log('🔥 COMPLETE GATE.IO v4 open positions for', symbol, ':', openPositions);
      ...
      // 2. 🔧 ИСПРАВЛЕННОЕ ЗАКРЫТИЕ ПОЗИЦИЙ ЧЕРЕЗ СПЕЦИАЛЬНЫЙ API
      ...
      for (const position of openPositions) {
        try {
          ...
          console.log('🔥 COMPLETE GATE.IO v4 closing position:', { 
            contract: position.contract, 
            originalSize: positionSize,
            positionType: positionSize > 0 ? 'LONG' : 'SHORT'
          });

          // 🔧 ИСПОЛЬЗУЕМ СПЕЦИАЛЬНЫЙ API ДЛЯ ЗАКРЫТИЯ ПОЗИЦИЙ
          const closePositionData = {
            contract: position.contract,
            size: 0 // 🔧 ИСПРАВЛЕНО: size=0 означает закрыть всю позицию
          };

          console.log('🔥 COMPLETE GATE.IO v4 close position data (SPECIAL API):', closePositionData);

          const closePayload = JSON.stringify(closePositionData);
          const { signature: closeSig, timestamp: closeTs } = await createCompleteGateSignature(apiKey.api_secret, 'POST', prefix + '/futures/usdt/positions/' + position.contract, '', closePayload);

          // 🔧 ИСПОЛЬЗУЕМ СПЕЦИАЛЬНЫЙ ENDPOINT ДЛЯ ЗАКРЫТИЯ ПОЗИЦИЙ
          const closeResponse = await fetch(baseUrl + prefix + '/futures/usdt/positions/' + position.contract, {
            method: 'POST',
            headers: {
              'KEY': apiKey.api_key,
              'SIGN': closeSig,
          ...
          console.log('🔥 COMPLETE GATE.IO v4 close position response status:', closeResponse.status);
          console.log('🔥 COMPLETE GATE.IO v4 close position response:', closeData);
          ...
          if (closeResponse.status === 200) {
            ...
            closeResults.push({
              ...
              close_method: 'SPECIAL_POSITION_API'
            });
            console.log('🔥 COMPLETE GATE.IO v4 position closed successfully via SPECIAL API');
          } else {
            closeResults.push({
              contract: position.contract,
              status: 'ERROR',
            ...
            console.error('🔥 COMPLETE GATE.IO v4 close position error:', closeData.message);
          }
          ...
          console.error('🔥 COMPLETE GATE.IO v4 close position error:', closeError);
          closeResults.push({
            contract: position.contract,
            status: 'ERROR',
      ...
      return {
        ...
        exchange: 'GATE',
        status: 'LIVE',
        message: `Закрытие позиций COMPLETE GATE.IO v4: ${closedPositions}/${openPositions.length} успешно`,
        api_version: 'COMPLETE_GATE_V4',
        symbol: symbol,
        close_method: 'SPECIAL_POSITION_API_SIZE_ZERO',
        api_endpoint: '/futures/usdt/positions/{contract}'
      };
    }
    ...
    console.error('🔥 COMPLETE GATE.IO Close positions error:', error);
