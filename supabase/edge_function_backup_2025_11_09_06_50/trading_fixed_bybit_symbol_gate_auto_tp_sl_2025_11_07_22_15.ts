...
serve(async (req) => {
  ...
  try {
    console.log('🔥 FIXED BYBIT + GATE TP/SL Function started');
...
async function placeOrderWithTPSL(apiKey: any, settings: any) {
  try {
    ...
    // 🆕 GATE.IO API V4 С АВТОМАТИЧЕСКИМИ TP/SL
    if (apiKey.exchange === 'gate') {
      ...
      console.log('🔥 GATE.IO v4 ORDER WITH TP/SL: Base URL:', baseUrl);
      console.log('🔥 GATE.IO v4 ORDER WITH TP/SL: Symbol:', symbol);
      
      // Получаем текущую цену
      const priceUrl = `/futures/usdt/tickers?contract=${symbol}`;
      const fullPriceUrl = baseUrl + prefix + priceUrl;
      console.log('🔥 GATE.IO v4 ORDER WITH TP/SL: Price URL:', fullPriceUrl);
      
      const priceResponse = await fetch(fullPriceUrl);
      const priceData = await priceResponse.json();
      
      console.log('🔥 GATE.IO v4 ORDER WITH TP/SL: Price response status:', priceResponse.status);
      console.log('🔥 GATE.IO v4 ORDER WITH TP/SL: Price data:', priceData);
      ...
      console.log('🔥 GATE.IO v4 order params WITH TP/SL:', { 
        symbol, 
        quantity, 
        currentPrice,
        tpPrice,
      ...
      // 2. АВТОМАТИЧЕСКИЕ TP/SL через Price Orders API
      let tpStatus = 'NOT_SET';
      let slStatus = 'NOT_SET';
      let tpOrderId = null;
      let slOrderId = null;
      ...
      return {
        ...
        api_version: 'FIXED_GATE_V4_WITH_AUTOMATIC_TP_SL',
        base_url: baseUrl,
        signature_format: 'COMPLETE_DOCUMENTATION_BASED',
        text_field_fix: 'CORRECT_T_PREFIX_APPLIED',
        tp_sl_implementation: 'AUTOMATIC_PRICE_ORDERS_API_WITH_PROPER_ROUNDING',
