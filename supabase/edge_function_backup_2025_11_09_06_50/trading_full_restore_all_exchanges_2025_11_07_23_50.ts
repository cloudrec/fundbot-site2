...
serve(async (req) => {
  ...
  try {
    console.log('🔥 FULL RESTORE Function started');
...
// 🟡 BYBIT API FUNCTIONS
async function createBybitSignature(secret: string, timestamp: string, apiKey: string, recv_window: string, queryString: string): Promise<string> {
...
async function getBalance(apiKey: any, settings: any) {
  try {
    ...
    if (apiKey.exchange === 'bybit') {
      ...
      const signature = await createBybitSignature(apiKey.api_secret, timestamp, apiKey.api_key, recv_window, queryString);
...
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
    ...
    if (apiKey.exchange === 'bybit') {
      ...
      const queryString = Object.entries(orderPayload)
...
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
    ...
    // 🔵 GATE.IO IMPLEMENTATION (MANUAL TP/SL)
    if (apiKey.exchange === 'gate') {
...
async function getPositions(apiKey: any, settings: any) {
  try {
    ...
    if (apiKey.exchange === 'bybit') {
      ...
      const timestamp = Date.now().toString();
      const recv_window = '5000';
      const queryString = `category=linear&symbol=${settings.base_asset}USDT`;
...
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
    ...
    if (apiKey.exchange === 'bybit') {
      ...
      const queryString = Object.entries(cancelPayload)
        .map(([key, value]) => `${key}=${encodeURIComponent(value)}`)
        .join('&');
...
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
    ...
    if (apiKey.exchange === 'bybit') {
      ...
      const recv_window = '5000';
      const posQueryString = `category=linear&symbol=${symbol}`;
      
      const posSignature = await createBybitSignature(apiKey.api_secret, timestamp1, apiKey.api_key, recv_window, posQueryString);
      ...
      for (const position of openPositions) {
        try {
          ...
          const closeQueryString = Object.entries(closePayload)
            .map(([key, value]) => `${key}=${encodeURIComponent(value)}`)
            .join('&');
          
          const closeSignature = await createBybitSignature(apiKey.api_secret, timestamp2, apiKey.api_key, recv_window, closeQueryString);
