...
async function placeOrderWithTPSL(apiKey: any, settings: any) {
  try {
    ...
    if (apiKey.exchange === 'binance') {
      console.log('🔥 Processing Binance order with PURE POSITIONAL TP/SL (NO ADDITIONAL ORDERS)');
      ...
      console.log('🔥 Waiting 3 seconds before setting PURE POSITIONAL TP/SL...');
      await delay(3000);

      // 4. ЧИСТЫЕ ПОЗИЦИОННЫЕ TP/SL (НЕ ОРДЕРА!) через /fapi/v1/positionSide/dual
      ...
      try {
        // ПОЗИЦИОННЫЙ Take Profit через специальный API
        const tpTimestamp = Date.now();
        const tpQuery = `symbol=${symbol}&positionSide=BOTH&type=TAKE_PROFIT&stopPrice=${tpPrice}&timestamp=${tpTimestamp}`;
        const tpSignature = await createBinanceSignature(apiKey.api_secret, tpQuery);
        
        console.log('🔥 PURE POSITIONAL TP API call:', tpQuery);
        
        const tpResponse = await fetch(`${baseUrl}/fapi/v1/positionSide/dual`, {
          method: 'POST',
          headers: {
            'X-MBX-APIKEY': apiKey.api_key,
            'Content-Type': 'application/x-www-form-urlencoded'
        ...
        console.log('🔥 Binance PURE POSITIONAL TP response status:', tpResponse.status);
        console.log('🔥 Binance PURE POSITIONAL TP response:', tpData);
        ...
        if (tpResponse.status === 200) {
          ...
          tpStatus = 'ERROR';
          tpError = tpData.msg;
          console.error('🔥 PURE POSITIONAL TP error:', tpData.msg);
        }

        // Задержка между TP и SL
        await delay(2000);

        // ПОЗИЦИОННЫЙ Stop Loss через специальный API
        const slTimestamp = Date.now();
        const slQuery = `symbol=${symbol}&positionSide=BOTH&type=STOP_LOSS&stopPrice=${slPrice}&timestamp=${slTimestamp}`;
        const slSignature = await createBinanceSignature(apiKey.api_secret, slQuery);
        
        console.log('🔥 PURE POSITIONAL SL API call:', slQuery);
        
        const slResponse = await fetch(`${baseUrl}/fapi/v1/positionSide/dual`, {
          method: 'POST',
          headers: {
            'X-MBX-APIKEY': apiKey.api_key,
            'Content-Type': 'application/x-www-form-urlencoded'
        ...
        console.log('🔥 Binance PURE POSITIONAL SL response status:', slResponse.status);
        console.log('🔥 Binance PURE POSITIONAL SL response:', slData);
        ...
        if (slResponse.status === 200) {
          ...
          slStatus = 'ERROR';
          slError = slData.msg;
          console.error('🔥 PURE POSITIONAL SL error:', slData.msg);
        }

      } catch (tpslError) {
        console.error('🔥 PURE POSITIONAL TP/SL API error:', tpslError);
        tpStatus = 'ERROR';
        slStatus = 'ERROR';
      ...
      return {
        ...
        side: orderData.side,
        status: 'LIVE',
        message: `Боевой ордер Binance с ЧИСТЫМИ позиционными TP/SL: ${orderData.orderId}`,
        quantity: quantity,
        price: currentPrice,
        tp_price: tpPrice,
        sl_price: slPrice,
        ...
        price_source: 'CACHED',
        tp_api_type: 'PURE_POSITIONAL_TAKE_PROFIT',
        sl_api_type: 'PURE_POSITIONAL_STOP_LOSS',
        duplicate_protection: 'ENABLED',
        binance_version: 'PURE_POSITIONAL_TP_SL_NO_ORDERS'
      };
    }
