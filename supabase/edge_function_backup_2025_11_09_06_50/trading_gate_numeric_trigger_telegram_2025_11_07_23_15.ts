...
serve(async (req) => {
  ...
  try {
    console.log('🔥 GATE NUMERIC TRIGGER PRICE + TELEGRAM Function started');
...
async function sendTelegramNotification(supabase: any, user_id: string, message: string, type: 'order' | 'close' | 'cancel' = 'order') {
  try {
  ...
  try {
    ...
    if (apiKey.exchange === 'gate') {
      ...
      const result = {
        ...
        result: data,
        debug_url: fullUrl,
        api_version: 'GATE_V4_WITH_NUMERIC_TRIGGER_PRICE',
        base_url: baseUrl,
...
async function placeOrderWithTPSL(apiKey: any, settings: any, supabase: any, user_id: string) {
  try {
    ...
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing GATE.IO API v4 order WITH NUMERIC TRIGGER PRICE');
      ...
      // 🔧 ИСПРАВЛЕНО: TRIGGER_PRICE КАК ЧИСЛО, А НЕ СТРОКА
      const tpPriceNumeric = parseFloat(tpPriceNumber.toFixed(4));
      const slPriceNumeric = parseFloat(slPriceNumber.toFixed(4));
      const tpPriceString = tpPriceNumber.toFixed(4);
      const slPriceString = slPriceNumber.toFixed(4);
      ...
      // 🔧 ПРАВИЛЬНЫЕ TP/SL ЧЕРЕЗ PRICE ORDERS API С NUMERIC TRIGGER_PRICE
      let tpOrderId = null;
      let slOrderId = null;
      ...
      try {
        ...
        console.log('🔥 GATE.IO v4: Creating TP Price Order with NUMERIC trigger_price');
        ...
        const tpPriceOrderData = {
          ...
          trigger: {
            ...
            trigger_price: tpPriceNumeric, // 🔧 ЧИСЛО, А НЕ СТРОКА!
        ...
        console.log('🔥 GATE.IO v4: Creating SL Price Order with NUMERIC trigger_price');
        ...
        const slPriceOrderData = {
          ...
          trigger: {
            ...
            trigger_price: slPriceNumeric, // 🔧 ЧИСЛО, А НЕ СТРОКА!
            price_type: 0 // 0 - latest deal price
          }
        };
...
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
      ...
      return {
        ...
        side: "long",
        status: 'LIVE',
        message: `Боевой ордер GATE.IO v4 с NUMERIC trigger_price: ${orderId}`,
        quantity: quantity,
        price: currentPrice,
        ...
        price_type: 0, // 0 - latest deal price
        trigger_price_format: 'NUMERIC_NOT_STRING',
        note: 'TP/SL установлены через Price Orders API с NUMERIC trigger_price полями'
      };
    }
