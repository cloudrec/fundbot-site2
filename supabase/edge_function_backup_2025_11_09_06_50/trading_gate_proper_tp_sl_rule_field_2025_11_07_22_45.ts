...
serve(async (req) => {
  ...
  try {
    console.log('🔥 CORRECTED GATE TP/SL WITH RULE Function started');
...
// 🔒🔒🔒 РАБОЧИЕ ФУНКЦИИ BYBIT - ИСПРАВЛЕНЫ! 🔒🔒🔒
...
// 🔒🔒🔒 РАБОЧАЯ ФУНКЦИЯ BYBIT - ИСПРАВЛЕНА! 🔒🔒🔒
  ...
  try {
    ...
    if (apiKey.exchange === 'binance') {
      ...
      return {
        ...
        side: orderData.side,
        status: 'LIVE',
        message: `Боевой ордер Binance с позиционными TP/SL: ${orderData.orderId}`,
        quantity: quantity,
        price: currentPrice,
        tp_price: tpPrice,
        sl_price: slPrice,
        ...
        binance_version: 'POSITIONAL_TP_SL_WITH_RATE_LIMIT_PROTECTION'
      };
    }
    ...
    if (apiKey.exchange === 'bybit') {
      ...
      // РАБОЧИЙ РАСЧЕТ КОЛИЧЕСТВА - ИСПРАВЛЕН!
      ...
      console.log('🔥 FIXED Quantity calculation:', {
        notionalValue,
        currentPrice,
        rawQuantity,
      ...
      return {
        ...
        sl_status: 'SUCCESS',
        exchange: 'BYBIT',
        quantity_calculation: 'FIXED'
      };
    }

    // 🆕 ИСПРАВЛЕННАЯ GATE.IO API V4 С ПРАВИЛЬНЫМИ TP/SL ЧЕРЕЗ PRICE ORDERS
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing CORRECTED GATE.IO API v4 order WITH PROPER TP/SL PRICE ORDERS');
      ...
      console.log('🔥 CORRECTED GATE.IO v4 ORDER: Base URL:', baseUrl);
      console.log('🔥 CORRECTED GATE.IO v4 ORDER: Symbol:', symbol);
      
      // Получаем текущую цену
      const priceUrl = `/futures/usdt/tickers?contract=${symbol}`;
      const fullPriceUrl = baseUrl + prefix + priceUrl;
      console.log('🔥 CORRECTED GATE.IO v4 ORDER: Price URL:', fullPriceUrl);
      
      const priceResponse = await fetch(fullPriceUrl);
      const priceData = await priceResponse.json();
      
      console.log('🔥 CORRECTED GATE.IO v4 ORDER: Price response status:', priceResponse.status);
      console.log('🔥 CORRECTED GATE.IO v4 ORDER: Price data:', priceData);
      
      if (!priceData || priceData.length === 0) {
        throw new Error(`Не удалось получить цену для ${symbol} на CORRECTED GATE.IO v4`);
      }
      
      const currentPrice = parseFloat(priceData[0].last);
      console.log('🔥 CORRECTED GATE.IO v4 current price:', currentPrice);
      ...
      console.log('🔥 CORRECTED GATE.IO v4 order params:', { 
        symbol, 
        quantity, 
        currentPrice,
        tpPrice,
      ...
      console.log('🔥 CORRECTED GATE.IO v4 MAIN order data:', orderData);
      ...
      console.log('🔥 CORRECTED GATE.IO v4 order request URL:', fullOrderUrl);
      ...
      const data = await response.json();
      console.log('🔥 CORRECTED GATE.IO v4 order response status:', response.status);
      console.log('🔥 CORRECTED GATE.IO v4 order response:', data);

      if (response.status !== 201) {
        throw new Error(`CORRECTED GATE.IO v4 API Error: ${data.message || 'Unknown error'} (Code: ${response.status})`);
      }

      const orderId = data.id;
      console.log('🔥 CORRECTED GATE.IO v4 MAIN order created:', orderId);

      // 🔧 ПРАВИЛЬНЫЕ TP/SL ЧЕРЕЗ PRICE ORDERS API С RULE
      let tpOrderId = null;
      let slOrderId = null;
      let tpStatus = 'NOT_SET';
      let slStatus = 'NOT_SET';
      ...
      try {
        // 2. TAKE PROFIT PRICE ORDER (rule: ">=")
        console.log('🔥 CORRECTED GATE.IO v4: Creating TP Price Order with rule >=');
        ...
        const tpPriceOrderData = {
          ...
          trigger: {
            rule: ">=", // 🔧 ПРАВИЛЬНОЕ ПРАВИЛО ДЛЯ TP
            trigger_price: tpPrice,
            price_type: "last_price"
          }
        };
        
        console.log('🔥 CORRECTED GATE.IO v4 TP Price Order data:', tpPriceOrderData);
        ...
        console.log('🔥 CORRECTED GATE.IO v4 TP Price Order response status:', tpPriceResponse.status);
        console.log('🔥 CORRECTED GATE.IO v4 TP Price Order response:', tpPriceData);

        if (tpPriceResponse.status === 201) {
          tpOrderId = tpPriceData.id;
          tpStatus = 'SUCCESS';
          console.log('🔥 CORRECTED GATE.IO v4 TP Price Order created:', tpOrderId);
        } else {
          tpStatus = 'ERROR';
          tpError = tpPriceData.message || 'Unknown TP error';
          console.error('🔥 CORRECTED GATE.IO v4 TP Price Order error:', tpError);
        }

        // Задержка между TP и SL
        await delay(1000);

        // 3. STOP LOSS PRICE ORDER (rule: "<=")
        console.log('🔥 CORRECTED GATE.IO v4: Creating SL Price Order with rule <=');
        ...
        const slPriceOrderData = {
          ...
          trigger: {
            rule: "<=", // 🔧 ПРАВИЛЬНОЕ ПРАВИЛО ДЛЯ SL
            trigger_price: slPrice,
            price_type: "last_price"
          }
        };
        
        console.log('🔥 CORRECTED GATE.IO v4 SL Price Order data:', slPriceOrderData);
        ...
        console.log('🔥 CORRECTED GATE.IO v4 SL Price Order response status:', slPriceResponse.status);
        console.log('🔥 CORRECTED GATE.IO v4 SL Price Order response:', slPriceData);

        if (slPriceResponse.status === 201) {
          slOrderId = slPriceData.id;
          slStatus = 'SUCCESS';
          console.log('🔥 CORRECTED GATE.IO v4 SL Price Order created:', slOrderId);
        } else {
          slStatus = 'ERROR';
          slError = slPriceData.message || 'Unknown SL error';
          console.error('🔥 CORRECTED GATE.IO v4 SL Price Order error:', slError);
        }

      } catch (priceOrderError) {
        console.error('🔥 CORRECTED GATE.IO v4 Price Orders error:', priceOrderError);
        tpStatus = 'ERROR';
        slStatus = 'ERROR';
      ...
      return {
        ...
        side: "long",
        status: 'LIVE',
        message: `Боевой ордер CORRECTED GATE.IO v4 с правильными TP/SL: ${orderId}`,
        quantity: quantity,
        price: currentPrice,
        tp_price: tpPrice,
        sl_price: slPrice,
        ...
        sl_error: slError,
        exchange: 'GATE',
        api_url: 'CORRECTED_gateio.ws',
        debug_url: fullOrderUrl,
        api_version: 'CORRECTED_GATE_V4_WITH_PROPER_PRICE_ORDERS_RULE',
        base_url: baseUrl,
        signature_format: 'COMPLETE_DOCUMENTATION_BASED',
        text_field_fix: 'CORRECT_T_PREFIX_APPLIED',
        tp_sl_implementation: 'PRICE_ORDERS_API_WITH_CORRECT_RULE_FIELD',
        tp_rule: '>=',
        sl_rule: '<=',
        note: 'TP/SL установлены через Price Orders API с правильными rule полями'
      };
    }
...
async function getPositions(apiKey: any, settings: any) {
  try {
    ...
    if (apiKey.exchange === 'bybit') {
      ...
      // 🔧 ИСПРАВЛЕНО: Добавляем symbol в параметры для Bybit
      ...
      console.log('🔥 FIXED Bybit positions request URL:', url);
      ...
      const data = await response.json();
      console.log('🔥 FIXED Bybit positions response status:', response.status);
      console.log('🔥 FIXED Bybit positions response:', data);
      ...
      console.log('🔥 FIXED Bybit open positions:', openPositions);
...
// 🔒🔒🔒 ИСПРАВЛЕННАЯ ФУНКЦИЯ ОТМЕНЫ ОРДЕРОВ ДЛЯ ВСЕХ БИРЖ 🔒🔒🔒
async function cancelAllOrders(apiKey: any, settings: any) {
  try {
    ...
    // 🔒 ИСПРАВЛЕННАЯ ФУНКЦИЯ BYBIT
    if (apiKey.exchange === 'bybit') {
      console.log('🔥 Processing FIXED Bybit cancel orders request');
      
      const timestamp = Date.now();
      const symbol = `${settings.base_asset}${settings.quote_asset}`;
      
      // 🔧 ИСПРАВЛЕНО: Добавляем settleCoin для Bybit
      const orderData = {
        category: "linear",
        symbol: symbol,
        settleCoin: "USDT" // 🔧 ИСПРАВЛЕНО: Добавляем settleCoin
      };
      ...
      console.log('🔥 FIXED Bybit cancel orders request URL:', url);
      console.log('🔥 FIXED Bybit cancel orders data:', orderData);
      ...
      const data = await response.json();
      console.log('🔥 FIXED Bybit cancel orders response status:', response.status);
      console.log('🔥 FIXED Bybit cancel orders response:', data);
      ...
      return {
        ...
        result: data.result,
        api_version: 'FIXED_BYBIT_WITH_SETTLECOIN'
      };
    }
    ...
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing CORRECTED GATE.IO API v4 cancel orders request');
      ...
      try {
        ...
        console.log('🔥 CORRECTED GATE.IO v4 cancel regular orders URL:', baseUrl + prefix + ordersUrl);
        ...
        console.log('🔥 CORRECTED GATE.IO v4 cancel regular orders response status:', ordersResponse.status);
        console.log('🔥 CORRECTED GATE.IO v4 cancel regular orders response:', ordersData);
        ...
        console.error('🔥 CORRECTED GATE.IO v4 regular orders cancel error:', regularError);
        cancelResults.push({
          type: 'REGULAR_ORDERS',
          cancelled: 0,
          status: 'ERROR',
      ...
      try {
        ...
        console.log('🔥 CORRECTED GATE.IO v4 cancel price orders URL:', baseUrl + prefix + priceOrdersUrl);
        ...
        const priceData = await priceResponse.json();
        console.log('🔥 CORRECTED GATE.IO v4 cancel price orders response status:', priceResponse.status);
        console.log('🔥 CORRECTED GATE.IO v4 cancel price orders response:', priceData);
        ...
        console.error('🔥 CORRECTED GATE.IO v4 price orders cancel error:', priceError);
        cancelResults.push({
          type: 'PRICE_ORDERS_TP_SL',
          cancelled: 0,
          status: 'ERROR',
      ...
      return {
        ...
        exchange: 'GATE',
        status: 'LIVE',
        message: `Отмена ордеров CORRECTED GATE.IO v4: ${totalCancelled} ордеров отменено`,
        api_version: 'CORRECTED_GATE_V4_ALL_ORDER_TYPES',
        symbol: symbol,
...
// 🔒🔒🔒 ИСПРАВЛЕННАЯ ФУНКЦИЯ ЗАКРЫТИЯ ПОЗИЦИЙ ДЛЯ ВСЕХ БИРЖ 🔒🔒🔒
async function closeAllPositions(apiKey: any, settings: any) {
  try {
    ...
    // 🔒 ИСПРАВЛЕННАЯ ФУНКЦИЯ BYBIT
    if (apiKey.exchange === 'bybit') {
      console.log('🔥 Processing FIXED Bybit close positions request');
      
      const timestamp = Date.now();
      const symbol = `${settings.base_asset}${settings.quote_asset}`;
      
      // 1. Получаем открытые позиции с исправленным API
      ...
      for (const position of openPositions) {
        try {
          ...
          console.log('🔥 FIXED Bybit close order data:', orderData);
          ...
          const data = await response.json();
          console.log('🔥 FIXED Bybit close response status:', response.status);
          console.log('🔥 FIXED Bybit close response:', data);
          ...
          console.error('🔥 FIXED Bybit close position error:', closeError);
          closeResults.push({
            symbol: symbol,
            status: 'ERROR',
    ...
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing CORRECTED GATE.IO API v4 close positions request');
      ...
      console.log('🔥 CORRECTED GATE.IO v4 get positions URL:', baseUrl + prefix + positionsUrl);
      ...
      console.log('🔥 CORRECTED GATE.IO v4 positions response status:', positionsResponse.status);
      console.log('🔥 CORRECTED GATE.IO v4 positions response:', positionsData);

      if (positionsResponse.status !== 200) {
        throw new Error(`CORRECTED GATE.IO v4 Positions API Error: ${positionsData.message || 'Unknown error'} (Code: ${positionsResponse.status})`);
      }
      ...
      console.log('🔥 CORRECTED GATE.IO v4 open positions for', symbol, ':', openPositions);
      ...
      for (const position of openPositions) {
        try {
          ...
          console.log('🔥 CORRECTED GATE.IO v4 closing position:', { 
          ...
          console.log('🔥 CORRECTED GATE.IO v4 close order data:', closeOrderData);
          ...
          console.log('🔥 CORRECTED GATE.IO v4 close order response status:', closeResponse.status);
          console.log('🔥 CORRECTED GATE.IO v4 close order response:', closeData);
          ...
          if (closeResponse.status === 201) {
            ...
            console.log('🔥 CORRECTED GATE.IO v4 position closed successfully');
          } else {
            closeResults.push({
              contract: position.contract,
              status: 'ERROR',
            ...
            console.error('🔥 CORRECTED GATE.IO v4 close order error:', closeData.message);
          }
          ...
          console.error('🔥 CORRECTED GATE.IO v4 close position error:', closeError);
          closeResults.push({
            contract: position.contract,
            status: 'ERROR',
            error: closeError.message
      ...
      return {
        ...
        exchange: 'GATE',
        status: 'LIVE',
        message: `Закрытие позиций CORRECTED GATE.IO v4: ${closedPositions}/${openPositions.length} успешно`,
        api_version: 'CORRECTED_GATE_V4',
        symbol: symbol,
        close_method: 'MARKET_ORDER_WITH_REDUCE_ONLY',
...
// 🔒🔒🔒 РАБОЧИЕ ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ ДЛЯ BYBIT - ИСПРАВЛЕНЫ! 🔒🔒🔒
