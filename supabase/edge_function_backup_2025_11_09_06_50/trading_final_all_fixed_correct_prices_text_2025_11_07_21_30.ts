...
serve(async (req) => {
  ...
  try {
    console.log('🔥 FINAL FIXED Gate.io Function started');
...
// 🆕 ФИНАЛЬНАЯ ФУНКЦИЯ СОЗДАНИЯ ПОДПИСИ ДЛЯ GATE.IO API V4
async function createFinalGateSignature(secret: string, method: string, url: string, queryString: string = '', payloadString: string = ''): Promise<{ signature: string, timestamp: string }> {
  try {
    // Используем текущее время в секундах (как в официальном примере)
    const timestamp = Math.floor(Date.now() / 1000).toString();
    
    console.log('🔥 FINAL GATE.IO v4 signature inputs:');
    ...
    console.log('🔥 FINAL GATE.IO v4 hashed payload:', hashedPayload);
    
    // ОФИЦИАЛЬНЫЙ ФОРМАТ СТРОКИ ПОДПИСИ ПО ДОКУМЕНТАЦИИ:
    // method + '\n' + url + '\n' + query_string + '\n' + hashed_payload + '\n' + timestamp
    const signatureString = `${method}\n${url}\n${queryString}\n${hashedPayload}\n${timestamp}`;
    
    console.log('🔥 FINAL GATE.IO v4 signature string:');
    ...
    console.log('🔥 FINAL GATE.IO v4 signature result:', result);
    return { signature: result, timestamp };
  } catch (error) {
    console.error('❌ Error creating FINAL GATE.IO v4 signature:', error);
    throw error;
  }
}
...
// 🆕 ФУНКЦИЯ ОКРУГЛЕНИЯ ЦЕН ДЛЯ GATE.IO
function roundGatePrice(price: number, tickSize: number = 0.0001): string {
  // Gate.io требует цены кратные tick size (обычно 0.0001 для USDT пар)
...
async function getBalance(apiKey: any, settings: any) {
  try {
    console.log('🔥 FINAL GATE.IO getBalance started for exchange:', apiKey.exchange);
    ...
    // 🆕 ФИНАЛЬНАЯ GATE.IO API V4 РЕАЛИЗАЦИЯ
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing FINAL GATE.IO API v4 balance request');
      ...
      console.log('🔥 FINAL GATE.IO v4: Base URL:', baseUrl);
      console.log('🔥 FINAL GATE.IO v4: Prefix:', prefix);
      console.log('🔥 FINAL GATE.IO v4: URL path:', url);
      console.log('🔥 FINAL GATE.IO v4: Full URL:', baseUrl + prefix + url);
      
      // Создаем подпись по официальному примеру
      const { signature, timestamp } = await createFinalGateSignature(apiKey.api_secret, 'GET', prefix + url, queryString, payloadString);
      
      const fullUrl = baseUrl + prefix + url;
      console.log('🔥 FINAL GATE.IO v4 balance request URL:', fullUrl);
      ...
      const data = await response.json();
      console.log('🔥 FINAL GATE.IO v4 balance response status:', response.status);
      console.log('🔥 FINAL GATE.IO v4 balance response:', data);

      if (response.status !== 200) {
        throw new Error(`FINAL GATE.IO v4 API Error: ${data.message || 'Unknown error'} (Code: ${response.status})`);
      }
      ...
      const result = {
        ...
        result: data,
        debug_url: fullUrl,
        api_version: 'FINAL_GATE_V4',
        base_url: baseUrl,
        signature_format: 'FINAL_DOCUMENTATION_BASED'
      };
      
      console.log('🔥 FINAL GATE.IO v4 balance result:', result);
      return result;
    }
    ...
    console.error('🔥 FINAL GATE.IO Balance error:', error);
...
async function placeOrderWithTPSL(apiKey: any, settings: any) {
  try {
    console.log('🔥 FINAL GATE.IO placeOrderWithTPSL started for exchange:', apiKey.exchange);
    ...
    // 🆕 ФИНАЛЬНАЯ GATE.IO API V4 РЕАЛИЗАЦИЯ С ИСПРАВЛЕННЫМИ TP/SL
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing FINAL GATE.IO API v4 order WITH CORRECTED TP/SL');
      ...
      console.log('🔥 FINAL GATE.IO v4 ORDER: Base URL:', baseUrl);
      console.log('🔥 FINAL GATE.IO v4 ORDER: Symbol:', symbol);
      
      // Получаем текущую цену
      const priceUrl = `/futures/usdt/tickers?contract=${symbol}`;
      const fullPriceUrl = baseUrl + prefix + priceUrl;
      console.log('🔥 FINAL GATE.IO v4 ORDER: Price URL:', fullPriceUrl);
      
      const priceResponse = await fetch(fullPriceUrl);
      const priceData = await priceResponse.json();
      
      console.log('🔥 FINAL GATE.IO v4 ORDER: Price response status:', priceResponse.status);
      console.log('🔥 FINAL GATE.IO v4 ORDER: Price data:', priceData);
      
      if (!priceData || priceData.length === 0) {
        throw new Error(`Не удалось получить цену для ${symbol} на FINAL GATE.IO v4`);
      }
      
      const currentPrice = parseFloat(priceData[0].last);
      console.log('🔥 FINAL GATE.IO v4 current price:', currentPrice);
      
      // Рассчитываем количество (Gate.io использует контракты)
      const calculatedQuantity = Math.floor(settings.order_amount_usd / currentPrice);
      const quantity = calculatedQuantity;
      
      // 🔧 ИСПРАВЛЕННЫЕ ЦЕНЫ TP/SL С ПРАВИЛЬНЫМ ОКРУГЛЕНИЕМ
      const tpPriceRaw = currentPrice * (1 + settings.long_tp_offset_percent / 100);
      const slPriceRaw = currentPrice * (1 - settings.long_stop_loss_percent / 100);
      
      // Округляем цены до правильного tick size для Gate.io
      const tpPrice = roundGatePrice(tpPriceRaw, 0.0001);
      const slPrice = roundGatePrice(slPriceRaw, 0.0001);
      
      console.log('🔥 FINAL GATE.IO v4 CORRECTED TP/SL prices:', { 
        symbol, 
        quantity, 
        currentPrice,
        tpPriceRaw,
      ...
      // 1. ОСНОВНОЙ ОРДЕР С ПРАВИЛЬНЫМ TEXT (t- префикс)
      const orderData = {
        contract: symbol,
        size: quantity,
      ...
      console.log('🔥 FINAL GATE.IO v4 MAIN order data (CORRECT t- TEXT):', orderData);
      
      const orderUrl = '/futures/usdt/orders';
      const payloadString = JSON.stringify(orderData);
      const { signature, timestamp } = await createFinalGateSignature(apiKey.api_secret, 'POST', prefix + orderUrl, '', payloadString);
      
      const fullOrderUrl = baseUrl + prefix + orderUrl;
      console.log('🔥 FINAL GATE.IO v4 order request URL:', fullOrderUrl);
      ...
      const data = await response.json();
      console.log('🔥 FINAL GATE.IO v4 order response status:', response.status);
      console.log('🔥 FINAL GATE.IO v4 order response:', data);
      ...
      console.log('🔥 FINAL GATE.IO v4 MAIN order created:', orderId);

      // Задержка перед установкой TP/SL
      console.log('🔥 Waiting 3 seconds before setting CORRECTED TP/SL...');
      await delay(3000);

      // 2. ИСПРАВЛЕННАЯ РЕАЛИЗАЦИЯ TP/SL ЧЕРЕЗ PRICE ORDERS API
      let tpStatus = 'NOT_SET';
      let slStatus = 'NOT_SET';
      let tpOrderId = null;
      let slOrderId = null;
      ...
      try {
        // ИСПРАВЛЕННЫЙ TAKE PROFIT ORDER через Price Orders API
        const tpOrderData = {
          initial: {
            contract: symbol,
            size: -quantity, // Противоположное направление для закрытия
            price: "0", // Market order при срабатывании
            tif: "ioc"
          },
          trigger: {
            strategy_type: 0, // 0 = by_price
            price_type: 0, // 0 = latest_price
            price: tpPrice, // 🔧 ИСПРАВЛЕНО: ПРАВИЛЬНО ОКРУГЛЕННАЯ ЦЕНА
            rule: 1 // 1 = >= (больше или равно)
          }
        };

        console.log('🔥 FINAL GATE.IO v4 CORRECTED TP order data:', tpOrderData);

        const tpPayload = JSON.stringify(tpOrderData);
        const { signature: tpSig, timestamp: tpTs } = await createFinalGateSignature(apiKey.api_secret, 'POST', prefix + '/futures/usdt/price_orders', '', tpPayload);
        ...
        const tpData = await tpResponse.json();
        console.log('🔥 FINAL GATE.IO v4 CORRECTED TP response status:', tpResponse.status);
        console.log('🔥 FINAL GATE.IO v4 CORRECTED TP response:', tpData);

        if (tpResponse.status === 201) {
          tpStatus = 'SUCCESS';
          tpOrderId = tpData.id;
          console.log('🔥 FINAL GATE.IO v4 CORRECTED TP order created:', tpOrderId);
        } else {
          tpStatus = 'ERROR';
          tpError = tpData.message || 'Unknown TP error';
          console.error('🔥 FINAL GATE.IO v4 CORRECTED TP error:', tpError);
        }

        // Задержка между TP и SL
        await delay(2000);

        // ИСПРАВЛЕННЫЙ STOP LOSS ORDER через Price Orders API
        const slOrderData = {
          initial: {
            contract: symbol,
            size: -quantity, // Противоположное направление для закрытия
          ...
          trigger: {
            strategy_type: 0, // 0 = by_price
            price_type: 0, // 0 = latest_price
            price: slPrice, // 🔧 ИСПРАВЛЕНО: ПРАВИЛЬНО ОКРУГЛЕННАЯ ЦЕНА
            rule: 2 // 2 = <= (меньше или равно)
          }
        };

        console.log('🔥 FINAL GATE.IO v4 CORRECTED SL order data:', slOrderData);

        const slPayload = JSON.stringify(slOrderData);
        const { signature: slSig, timestamp: slTs } = await createFinalGateSignature(apiKey.api_secret, 'POST', prefix + '/futures/usdt/price_orders', '', slPayload);
        ...
        const slData = await slResponse.json();
        console.log('🔥 FINAL GATE.IO v4 CORRECTED SL response status:', slResponse.status);
        console.log('🔥 FINAL GATE.IO v4 CORRECTED SL response:', slData);

        if (slResponse.status === 201) {
          slStatus = 'SUCCESS';
          slOrderId = slData.id;
          console.log('🔥 FINAL GATE.IO v4 CORRECTED SL order created:', slOrderId);
        } else {
          slStatus = 'ERROR';
          slError = slData.message || 'Unknown SL error';
          console.error('🔥 FINAL GATE.IO v4 CORRECTED SL error:', slError);
        }

      } catch (tpslError) {
        console.error('🔥 FINAL GATE.IO v4 CORRECTED TP/SL error:', tpslError);
        tpStatus = 'ERROR';
        slStatus = 'ERROR';
        tpError = tpslError.message;
        slError = tpslError.message;
      ...
      return {
        ...
        side: "long",
        status: 'LIVE',
        message: `Боевой ордер FINAL GATE.IO v4 с исправленными TP/SL: ${orderId}`,
        quantity: quantity,
        price: currentPrice,
        tp_price: tpPrice,
        sl_price: slPrice,
        ...
        sl_error: slError,
        exchange: 'GATE',
        api_url: 'FINAL_gateio.ws',
        debug_url: fullOrderUrl,
        api_version: 'FINAL_GATE_V4_WITH_CORRECTED_TP_SL',
        base_url: baseUrl,
        signature_format: 'FINAL_DOCUMENTATION_BASED',
        text_field_fix: 'CORRECT_T_PREFIX_APPLIED',
        tp_sl_implementation: 'CORRECTED_PRICE_ORDERS_API_WITH_PROPER_ROUNDING',
        tp_api_method: 'PRICE_ORDERS_API_TRIGGER_RULE_1_GTE_CORRECTED',
        sl_api_method: 'PRICE_ORDERS_API_TRIGGER_RULE_2_LTE_CORRECTED',
        price_rounding: 'GATE_TICK_SIZE_0_0001_APPLIED'
      };
    }
    ...
    console.error('🔥 FINAL GATE.IO Set TP/SL error:', error);
    throw error;
  }
}

// 🆕 ФИНАЛЬНАЯ РЕАЛИЗАЦИЯ ПОЛУЧЕНИЯ ПОЗИЦИЙ ДЛЯ GATE.IO
async function getPositions(apiKey: any, settings: any) {
  try {
    console.log('🔥 FINAL GATE.IO getPositions started for exchange:', apiKey.exchange);
    
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing FINAL GATE.IO API v4 positions request');
      ...
      console.log('🔥 FINAL GATE.IO v4 positions URL:', baseUrl + prefix + url);
      ...
      const data = await response.json();
      console.log('🔥 FINAL GATE.IO v4 positions response status:', response.status);
      console.log('🔥 FINAL GATE.IO v4 positions response:', data);

      if (response.status !== 200) {
        throw new Error(`FINAL GATE.IO v4 Positions API Error: ${data.message || 'Unknown error'} (Code: ${response.status})`);
      }

      // Фильтруем только открытые позиции
      const openPositions = data.filter((position: any) => parseFloat(position.size) !== 0);
      
      console.log('🔥 FINAL GATE.IO v4 open positions:', openPositions);
    ...
    console.error('🔥 FINAL GATE.IO Positions error:', error);
    throw new Error(`Ошибка получения позиций: ${error.message}`);
  }
}

// 🆕 ФИНАЛЬНАЯ РЕАЛИЗАЦИЯ ОТМЕНЫ ОРДЕРОВ ДЛЯ GATE.IO
async function cancelAllOrders(apiKey: any, settings: any) {
  try {
    console.log('🔥 FINAL GATE.IO cancelAllOrders started for exchange:', apiKey.exchange);
    
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing FINAL GATE.IO API v4 cancel orders request');
      ...
      try {
        ...
        const { signature: ordersSig, timestamp: ordersTs } = await createFinalGateSignature(apiKey.api_secret, 'DELETE', prefix + ordersUrl, '', '');
        
        console.log('🔥 FINAL GATE.IO v4 cancel regular orders URL:', baseUrl + prefix + ordersUrl);
        ...
        console.log('🔥 FINAL GATE.IO v4 cancel regular orders response status:', ordersResponse.status);
        console.log('🔥 FINAL GATE.IO v4 cancel regular orders response:', ordersData);
        ...
        console.error('🔥 FINAL GATE.IO v4 regular orders cancel error:', regularError);
        cancelResults.push({
          type: 'REGULAR_ORDERS',
          cancelled: 0,
          status: 'ERROR',
      ...
      // Задержка между типами ордеров
      await delay(2000);

      // 2. ОТМЕНА PRICE ORDERS (TP/SL)
      try {
        const priceOrdersUrl = `/futures/usdt/price_orders?contract=${symbol}&status=open`;
        const { signature: priceSig, timestamp: priceTs } = await createFinalGateSignature(apiKey.api_secret, 'DELETE', prefix + priceOrdersUrl, '', '');
        
        console.log('🔥 FINAL GATE.IO v4 cancel price orders URL:', baseUrl + prefix + priceOrdersUrl);
        ...
        const priceData = await priceResponse.json();
        console.log('🔥 FINAL GATE.IO v4 cancel price orders response status:', priceResponse.status);
        console.log('🔥 FINAL GATE.IO v4 cancel price orders response:', priceData);
        ...
        console.error('🔥 FINAL GATE.IO v4 price orders cancel error:', priceError);
        cancelResults.push({
          type: 'PRICE_ORDERS_TP_SL',
          cancelled: 0,
          status: 'ERROR',
      ...
      return {
        ...
        exchange: 'GATE',
        status: 'LIVE',
        message: `Отмена ордеров FINAL GATE.IO v4: ${totalCancelled} ордеров отменено`,
        api_version: 'FINAL_GATE_V4',
        symbol: symbol,
    ...
    console.error('🔥 FINAL GATE.IO Cancel orders error:', error);
    throw new Error(`Ошибка отмены ордеров: ${error.message}`);
  }
}

// 🆕 ФИНАЛЬНАЯ РЕАЛИЗАЦИЯ ЗАКРЫТИЯ ПОЗИЦИЙ ДЛЯ GATE.IO
async function closeAllPositions(apiKey: any, settings: any) {
  try {
    console.log('🔥 FINAL GATE.IO closeAllPositions started for exchange:', apiKey.exchange);
    
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing FINAL GATE.IO API v4 close positions request');
      ...
      const { signature: posSig, timestamp: posTs } = await createFinalGateSignature(apiKey.api_secret, 'GET', prefix + positionsUrl, '', '');
      
      console.log('🔥 FINAL GATE.IO v4 get positions URL:', baseUrl + prefix + positionsUrl);
      ...
      console.log('🔥 FINAL GATE.IO v4 positions response status:', positionsResponse.status);
      console.log('🔥 FINAL GATE.IO v4 positions response:', positionsData);

      if (positionsResponse.status !== 200) {
        throw new Error(`FINAL GATE.IO v4 Positions API Error: ${positionsData.message || 'Unknown error'} (Code: ${positionsResponse.status})`);
      }
      ...
      console.log('🔥 FINAL GATE.IO v4 open positions for', symbol, ':', openPositions);
      ...
      // 2. ФИНАЛЬНОЕ ЗАКРЫТИЕ ПОЗИЦИЙ С ПРАВИЛЬНЫМ TEXT (t- префикс)
      ...
      for (const position of openPositions) {
        try {
          ...
          // ФИНАЛЬНЫЙ СПОСОБ ЗАКРЫТИЯ ПОЗИЦИЙ ЧЕРЕЗ MARKET ORDER С ПРОТИВОПОЛОЖНЫМ РАЗМЕРОМ
          const closeSize = -positionSize; // Противоположный размер для закрытия
          
          console.log('🔥 FINAL GATE.IO v4 closing position:', { 
          ...
          const closeOrderData = {
            contract: position.contract,
            size: closeSize, // Противоположный размер
            price: "0", // Market order
            tif: "ioc",
            text: `t-${Date.now().toString().slice(-6)}` // 🔧 ПРАВИЛЬНЫЙ TEXT С t- ПРЕФИКСОМ ДЛЯ ЗАКРЫТИЯ
          };

          console.log('🔥 FINAL GATE.IO v4 close order data (CORRECT t- TEXT):', closeOrderData);

          const closePayload = JSON.stringify(closeOrderData);
          const { signature: closeSig, timestamp: closeTs } = await createFinalGateSignature(apiKey.api_secret, 'POST', prefix + '/futures/usdt/orders', '', closePayload);
          ...
          console.log('🔥 FINAL GATE.IO v4 close position response status:', closeResponse.status);
          console.log('🔥 FINAL GATE.IO v4 close position response:', closeData);
          ...
          if (closeResponse.status === 201) {
            ...
            console.log('🔥 FINAL GATE.IO v4 position closed successfully:', closeData.id);
          } else {
            closeResults.push({
              contract: position.contract,
              status: 'ERROR',
            ...
            console.error('🔥 FINAL GATE.IO v4 close position error:', closeData.message);
          }
          ...
          console.error('🔥 FINAL GATE.IO v4 close position error:', closeError);
          closeResults.push({
            contract: position.contract,
            status: 'ERROR',
      ...
      return {
        ...
        exchange: 'GATE',
        status: 'LIVE',
        message: `Закрытие позиций FINAL GATE.IO v4: ${closedPositions}/${openPositions.length} успешно`,
        api_version: 'FINAL_GATE_V4',
        symbol: symbol,
        close_method: 'FINAL_MARKET_ORDER_WITH_OPPOSITE_SIZE_CORRECT_T_TEXT',
        text_field_fix: 'CORRECT_T_PREFIX_FOR_CLOSE_ORDERS'
      };
    }
    ...
    console.error('🔥 FINAL GATE.IO Close positions error:', error);
