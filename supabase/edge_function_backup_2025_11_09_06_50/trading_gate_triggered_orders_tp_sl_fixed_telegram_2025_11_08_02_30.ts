...
async function scanFundingWithAutoNotifications(supabase: any, user_id: string, settings: any, apiKeys: any[]) {
  try {
    ...
    if (successfulResults.length > 0) {
      // 🔥 ИСПРАВЛЕННАЯ ОТПРАВКА TELEGRAM УВЕДОМЛЕНИЙ
...
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
    ...
    // 🔵 GATE.IO IMPLEMENTATION - ПРАВИЛЬНЫЕ TP/SL ЧЕРЕЗ TRIGGERED ORDERS
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing GATE.IO API v4 order with TRIGGERED ORDERS TP/SL');
      ...
      try {
        ...
        const tpTriggeredOrder = {
          ...
          trigger: {
            strategy_type: 0, // 0 для обычного triggered order
            price_type: 0, // 0 для mark price
            rule: 1, // 1 для >=
        ...
        console.log('🔥 GATE.IO v4 TP triggered order:', tpTriggeredOrder);

        const tpPayload = JSON.stringify(tpTriggeredOrder);
        ...
        console.log('🔥 GATE.IO v4 SL triggered order:', slTriggeredOrder);

        const slPayload = JSON.stringify(slTriggeredOrder);
...
<b>TP ID:</b> ${tpOrderId || 'N/A'}
<b>SL ID:</b> ${slOrderId || 'N/A'}
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
      `.trim();
      ...
      return {
        ...
        side: "long",
        status: 'LIVE',
        message: `Боевой ордер GATE.IO v4 с Triggered Orders TP/SL: ${orderId}`,
        quantity: quantity,
        price: currentPrice,
        tp_price: tpPrice,
        sl_price: slPrice,
        ...
        api_version: 'GATE_V4_TRIGGERED_ORDERS_TP_SL',
        base_url: baseUrl,
        signature_format: 'COMPLETE_DOCUMENTATION_BASED',
        text_field_fix: 'CORRECT_T_PREFIX_APPLIED',
        tp_sl_method: 'TRIGGERED_ORDERS_API'
      };
    }
