...
async function handlePlaceOrderWithTPSL(apiKeys: any, settings: any) {
  ...
  try {
    ...
    console.log('🟡 BINANCE FIXED: Symbol info:', { symbol, tickSize });
    ...
    console.log('🟡 BINANCE FIXED: Price calculation:', {
      currentPrice,
      tpPriceRaw,
      slPriceRaw,
      tpPrice,
...
async function handleCancelOrders(apiKeys: any) {
  ...
  try {
    ...
    const data = await response.json();
    console.log('🟡 BINANCE FIXED: Cancel orders response:', data);
