...
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
    ...
    // 🔵 GATE.IO С STOP ОРДЕРАМИ (НЕ LIMIT!)
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing GATE.IO API v4 order with STOP ORDERS (NOT LIMIT!)');
      ...
      console.log('🔥 GATE.IO v4 STOP ORDERS: Base URL:', baseUrl);
      console.log('🔥 GATE.IO v4 STOP ORDERS: Symbol:', symbol);
      
      // Получаем текущую цену
      const priceUrl = `/futures/usdt/tickers?contract=${symbol}`;
      const fullPriceUrl = baseUrl + prefix + priceUrl;
      console.log('🔥 GATE.IO v4 STOP ORDERS: Price URL:', fullPriceUrl);
      
      const priceResponse = await fetch(fullPriceUrl);
      const priceData = await priceResponse.json();
      
      console.log('🔥 GATE.IO v4 STOP ORDERS: Price response status:', priceResponse.status);
      console.log('🔥 GATE.IO v4 STOP ORDERS: Price data:', priceData);
      ...
      console.log('🔥 GATE.IO v4 STOP ORDERS current price:', currentPrice);
      ...
      console.log('🔥 GATE.IO v4 STOP ORDERS order params:', { 
        symbol, 
        quantity, 
        currentPrice,
        tpPriceNumber,
      ...
      console.log('🔥 GATE.IO v4 STOP ORDERS main order data:', orderData);
      ...
      console.log('🔥 GATE.IO v4 STOP ORDERS order request URL:', fullOrderUrl);
      ...
      const data = await response.json();
      console.log('🔥 GATE.IO v4 STOP ORDERS order response status:', response.status);
      console.log('🔥 GATE.IO v4 STOP ORDERS order response:', data);

      if (response.status !== 201) {
        throw new Error(`GATE.IO v4 STOP ORDERS API Error: ${data.message || 'Unknown error'} (Code: ${response.status})`);
      }

      const orderId = data.id;
      console.log('🔥 GATE.IO v4 STOP ORDERS main order created:', orderId);

      // 📱 ОТПРАВЛЯЕМ В ВАШ TELEGRAM БОТ (ТОЛЬКО ОСНОВНОЙ ОРДЕР)
      const telegramMessage = `
<b>🚀 ОРДЕР РАЗМЕЩЕН</b>
<b>Биржа:</b> Gate.io (Только основной ордер)
<b>Символ:</b> ${symbol}
<b>Сторона:</b> LONG
<b>Количество:</b> ${quantity}
<b>Цена входа:</b> $${currentPrice.toFixed(4)}
<b>Take Profit:</b> $${tpPriceString} ⚠️ УСТАНОВИТЕ ВРУЧНУЮ
<b>Stop Loss:</b> $${slPriceString} ⚠️ УСТАНОВИТЕ ВРУЧНУЮ
<b>ID ордера:</b> ${orderId}
<b>Примечание:</b> TP/SL нужно установить через интерфейс Gate.io
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
      `.trim();
      ...
      return {
        ...
        side: "long",
        status: 'LIVE',
        message: `Боевой ордер GATE.IO v4 (только основной): ${orderId}`,
        quantity: quantity,
        price: currentPrice,
        ...
        tp_status: 'NOT_SET',
        sl_status: 'NOT_SET',
        tp_error: 'STOP ордера слишком сложны - установите TP/SL вручную',
        sl_error: 'STOP ордера слишком сложны - установите TP/SL вручную',
        exchange: 'GATE',
        api_url: 'gateio.ws',
        debug_url: fullOrderUrl,
        api_version: 'GATE_V4_MAIN_ORDER_ONLY',
        base_url: baseUrl,
        signature_format: 'COMPLETE_DOCUMENTATION_BASED',
        tp_sl_method: 'MANUAL_SETUP_REQUIRED',
        approach: 'MAIN_ORDER_ONLY_MANUAL_TP_SL',
