...
serve(async (req) => {
  ...
  try {
    ...
    console.log('🎯 GATE.IO FIXED TEXT: Starting action:', action);
    ...
    console.log('🎯 GATE.IO FIXED TEXT: Keys loaded, settings:', !!settings);
    ...
    console.error('❌ GATE.IO FIXED TEXT Error:', error.message);
    return new Response(
...
async function handleGetBalance(apiKeys: any) {
  console.log('🎯 GATE.IO FIXED TEXT: Getting balance');
...
async function handlePlaceOrder(apiKeys: any, settings: any) {
  console.log('🎯 GATE.IO FIXED TEXT: Placing order');
  
  if (!settings) {
    console.log('❌ GATE.IO FIXED TEXT: No settings found');
    return new Response(
      JSON.stringify({
        success: false,
  ...
  console.log('🎯 GATE.IO FIXED TEXT: Contract:', contract);
  ...
  console.log('🎯 GATE.IO FIXED TEXT: Order params:', {
    contract,
    size,
    currentPrice,
    orderAmount,
  ...
  const orderData = {
    ...
    tif: 'gtc',
    text: 't-skywork_api_order' // ✅ ИСПРАВЛЕНО: добавлен префикс t-
  };
  ...
  console.log('🎯 GATE.IO FIXED TEXT: Placing order with data:', orderData);
  ...
  const responseData = await response.json();
  console.log('🎯 GATE.IO FIXED TEXT: Order response:', responseData);
...
async function handleGetPositions(apiKeys: any) {
  console.log('🎯 GATE.IO FIXED TEXT: Getting positions');
  ...
  const responseData = await response.json();
  console.log('🎯 GATE.IO FIXED TEXT: Positions response:', responseData);
...
async function handleCancelAllOrders(apiKeys: any) {
  console.log('🎯 GATE.IO FIXED TEXT: Cancelling all orders');
...
async function handleCloseAllPositions(apiKeys: any) {
  console.log('🎯 GATE.IO FIXED TEXT: Closing all positions');
  ...
  for (const position of openPositions) {
    try {
      ...
      const closeOrderData = {
        ...
        text: 't-close_position' // ✅ ИСПРАВЛЕНО: добавлен префикс t-
      };
