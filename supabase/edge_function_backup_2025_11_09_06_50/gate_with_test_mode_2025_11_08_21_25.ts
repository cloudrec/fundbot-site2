        ...
        return new Response(
          JSON.stringify({ 
            ...
            data: {
              ...
              symbol: `${settings.base_asset}_${settings.quote_asset}`,
              side: 'BUY',
              status: 'TEST MODE',
              message: 'ТЕСТОВЫЙ Gate.io ордер - добавьте API ключи для реальной торговли',
              quantity: '100',
              price: 0.5000,
              tp_price: '0.5050',
              sl_price: '0.4950',
              exchange: 'GATE.IO',
              note: 'ТЕСТОВЫЙ РЕЖИМ - ДОБАВЬТЕ API КЛЮЧИ! 🚨'
            }
          }),
          { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
