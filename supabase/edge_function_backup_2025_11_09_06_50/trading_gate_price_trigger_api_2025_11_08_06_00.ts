...
// 🚀 ОПТИМИЗИРОВАННОЕ КЕШИРОВАНИЕ И ЗАЩИТА
class HighPerformanceCache {
  private cache = new Map<string, { data: any, timestamp: number, ttl: number }>();
  private maxSize = 5000; // Максимальный размер кеша
...
const rateLimiter = new Map<string, { count: number, resetTime: number }>(); // Rate limiting
...
async function getUserDataOptimized(userId: string) {
  ...
  const [apiKeysResult, settingsResult] = await Promise.all([
    supabase.from('api_keys_dev').select('*').eq('user_id', userId).eq('is_active', true),
    supabase.from('trading_settings_dev').select('*').eq('user_id', userId).single()
  ]);
  
  if (apiKeysResult.error) throw new Error(`API Keys error: ${apiKeysResult.error.message}`);
  if (settingsResult.error) throw new Error(`Settings error: ${settingsResult.error.message}`);
  if (!apiKeysResult.data || apiKeysResult.data.length === 0) throw new Error('Нет API ключей');
  if (!settingsResult.data) throw new Error('Нет настроек');
  
  const userData = { apiKeys: apiKeysResult.data, settings: settingsResult.data };
...
serve(async (req) => {
  ...
  const startTime = Date.now(); // 🚀 МОНИТОРИНГ ПРОИЗВОДИТЕЛЬНОСТИ
  ...
  try {
    ...
    switch (action) {
      case 'get_balance':
        ...
        break;
      case 'get_positions':
        result = await getPositions(apiKey, settings);
        break;
      case 'place_order_with_tp_sl':
        result = await placeOrderWithTPSL(apiKey, settings, getSupabaseClient(), user_id);
        break;
      case 'cancel_all_orders':
      case 'cancel_orders':
        result = await cancelAllOrders(apiKey, settings, getSupabaseClient(), user_id);
        break;

      case 'close_all_positions':
      case 'close_positions':
        result = await closeAllPositions(apiKey, settings, getSupabaseClient(), user_id);
        break;
      case 'scan_funding':
    ...
    return new Response(JSON.stringify({
      ...
      performance: {
        ...
        cache_stats: {
          user_data_cache_size: userDataCache.size(),
          price_cache_size: priceCache.size(),
          balance_cache_size: balanceCache.size()
        },
...
async function getPriceOptimized(symbol: string, baseUrl: string): Promise<number> {
  ...
  try {
    ...
    // Сохраняем stale копию для fallback
    priceCache.set(`${cacheKey}_stale`, price, 300000); // 5 минут
...
// Функция получения цены с кешированием (старая версия)
...
// 🚀 ОПТИМИЗИРОВАННАЯ ФУНКЦИЯ БАЛАНСА
async function getBalanceOptimized(apiKey: any, settings: any, userId: string) {
  ...
  if (cached) {
    console.log('🚀 BALANCE CACHE HIT for', apiKey.exchange);
    return { ...cached, cached: true };
  }
  
  console.log('🚀 BALANCE CACHE MISS: Fetching fresh data for', apiKey.exchange);
  ...
  // Fallback к обычной функции
  const result = await getBalance(apiKey, settings);
  balanceCache.set(cacheKey, result, CACHE_TTL.BALANCE);
  return result;
}

// 🚀 ОПТИМИЗИРОВАННАЯ GATE.IO БАЛАНС
async function getGateBalanceOptimized(apiKey: any) {
  ...
  const { signature, timestamp } = await createCompleteGateSignature(apiKey.api_secret, 'GET', prefix + url, '', '');
...
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
    ...
    // 🔵 GATE.IO С PRICE TRIGGER API (ПРАВИЛЬНЫЕ STOP ОРДЕРА)
    if (apiKey.exchange === 'gate') {
      console.log('🚀 Processing GATE.IO API v4 order with OPTIMIZED CONDITIONAL ORDERS');
      
      const symbol = `${settings.base_asset}_${settings.quote_asset}`;
      const baseUrl = apiKey.is_testnet ? 'https://fx-api-testnet.gateio.ws' : 'https://api.gateio.ws';
      const prefix = '/api/v4';
      
      console.log('🚀 GATE.IO v4 OPTIMIZED: Base URL:', baseUrl);
      console.log('🚀 GATE.IO v4 OPTIMIZED: Symbol:', symbol);
      ...
      console.log('🚀 GATE.IO v4 OPTIMIZED current price:', currentPrice);
      ...
      const tpPriceString = roundGatePrice(tpPriceNumber);
      const slPriceString = roundGatePrice(slPriceNumber);
      
      console.log('🔥 GATE.IO v4 PRICE TRIGGER order params:', { 
        symbol, 
        quantity, 
        currentPrice,
        tpPriceNumber,
      ...
      // 1. ОСНОВНОЙ MARKET ОРДЕР
      const orderData = {
        contract: symbol,
        size: quantity,
        price: "0", // Market order
        tif: "ioc",
        text: `t-main-${Date.now().toString().slice(-6)}`
      };
      
      console.log('🔥 GATE.IO v4 PRICE TRIGGER main order data:', orderData);
      ...
      console.log('🔥 GATE.IO v4 PRICE TRIGGER order request URL:', fullOrderUrl);
      ...
      const data = await response.json();
      console.log('🔥 GATE.IO v4 PRICE TRIGGER order response status:', response.status);
      console.log('🔥 GATE.IO v4 PRICE TRIGGER order response:', data);

      if (response.status !== 201) {
        throw new Error(`GATE.IO v4 PRICE TRIGGER API Error: ${data.message || 'Unknown error'} (Code: ${response.status})`);
      }

      const orderId = data.id;
      console.log('🔥 GATE.IO v4 PRICE TRIGGER main order created:', orderId);

      // Задержка перед установкой TP/SL
      console.log('🔥 Waiting 7 seconds before setting CONDITIONAL ORDERS TP/SL...');
      await delay(7000);

      // 2. ПРОСТЫЕ УСЛОВНЫЕ ОРДЕРА ЧЕРЕЗ ОБЫЧНЫЙ ENDPOINT (БЕЗ PRICE TRIGGER API)
      let tpOrderId = null;
      let slOrderId = null;
      let tpStatus = 'NOT_SET';
      let slStatus = 'NOT_SET';
      ...
      try {
        // TAKE PROFIT ЧЕРЕЗ ПРОСТОЙ LIMIT ОРДЕР С REDUCE_ONLY
        const tpOrderData = {
          contract: symbol,
          size: quantity,              // Положительный размер для SHORT ордера
          price: tpPriceString,        // Цена TP
          tif: "gtc",                  // Good Till Cancelled
          text: `t-tp-simple-${Date.now().toString().slice(-6)}`,
          reduce_only: true            // Обязательно для закрытия позиции
        };

        console.log('🚀 GATE.IO v4 SIMPLE TP data:', tpOrderData);

        const tpOrderUrl = '/futures/usdt/orders';
        const tpPayload = JSON.stringify(tpOrderData);
        const { signature: tpSig, timestamp: tpTs } = await createCompleteGateSignature(apiKey.api_secret, 'POST', prefix + tpOrderUrl, '', tpPayload);

        const tpResponse = await fetch(baseUrl + prefix + tpOrderUrl, {
          method: 'POST',
          headers: {
            'KEY': apiKey.api_key,
            'SIGN': tpSig,
        ...
        const tpData = await tpResponse.json();
        console.log('🚀 GATE.IO v4 SIMPLE TP response status:', tpResponse.status);
        console.log('🚀 GATE.IO v4 SIMPLE TP response:', tpData);

        if (tpResponse.status === 201) {
          tpOrderId = tpData.id;
          tpStatus = 'SUCCESS';
          console.log('🚀 GATE.IO v4 SIMPLE TP order created:', tpOrderId);
        } else {
          tpStatus = 'ERROR';
          tpError = tpData.message || 'Unknown TP error';
          console.error('🚀 GATE.IO v4 SIMPLE TP error:', tpData.message);
        }

        // Задержка между TP и SL
        await delay(5000);

        // STOP LOSS ЧЕРЕЗ ПРОСТОЙ LIMIT ОРДЕР С REDUCE_ONLY
        const slOrderData = {
          contract: symbol,
          size: quantity,              // Положительный размер для SHORT ордера
          price: slPriceString,        // Цена SL
          tif: "gtc",                  // Good Till Cancelled
          text: `t-sl-simple-${Date.now().toString().slice(-6)}`,
          reduce_only: true            // Обязательно для закрытия позиции
        };

        console.log('🚀 GATE.IO v4 SIMPLE SL data:', slOrderData);

        const slOrderUrl = '/futures/usdt/orders';
        const slPayload = JSON.stringify(slOrderData);
        const { signature: slSig, timestamp: slTs } = await createCompleteGateSignature(apiKey.api_secret, 'POST', prefix + slOrderUrl, '', slPayload);

        const slResponse = await fetch(baseUrl + prefix + slOrderUrl, {
          method: 'POST',
          headers: {
            'KEY': apiKey.api_key,
            'SIGN': slSig,
        ...
        const slData = await slResponse.json();
        console.log('🚀 GATE.IO v4 SIMPLE SL response status:', slResponse.status);
        console.log('🚀 GATE.IO v4 SIMPLE SL response:', slData);

        if (slResponse.status === 201) {
          slOrderId = slData.id;
          slStatus = 'SUCCESS';
          console.log('🚀 GATE.IO v4 SIMPLE SL order created:', slOrderId);
        } else {
          slStatus = 'ERROR';
          slError = slData.message || 'Unknown SL error';
          console.error('🚀 GATE.IO v4 SIMPLE SL error:', slData.message);
        }

      } catch (tpslError) {
        console.error('🚀 GATE.IO v4 SIMPLE TP/SL error:', tpslError);
        tpStatus = 'ERROR';
        slStatus = 'ERROR';
        tpError = tpslError.message;
        slError = tpslError.message;
      ...
      const telegramMessage = `
<b>🚀 ОРДЕР РАЗМЕЩЕН</b>
<b>Биржа:</b> Gate.io (Simple Limit Orders)
<b>Символ:</b> ${symbol}
<b>Сторона:</b> LONG
<b>Количество:</b> ${quantity}
<b>Цена входа:</b> $${currentPrice.toFixed(4)}
<b>Take Profit:</b> $${tpPriceString} ${tpStatus === 'SUCCESS' ? '✅' : '❌'}
<b>Stop Loss:</b> $${slPriceString} ${slStatus === 'SUCCESS' ? '✅' : '❌'}
<b>ID ордера:</b> ${orderId}
<b>TP Order ID:</b> ${tpOrderId || 'Не установлен'}
<b>SL Order ID:</b> ${slOrderId || 'Не установлен'}
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
      `.trim();
      ...
      return {
        ...
        side: "long",
        status: 'LIVE',
        message: `Боевой ордер GATE.IO v4 с Simple Limit TP/SL: ${orderId}`,
        quantity: quantity,
        price: currentPrice,
        ...
        api_version: 'GATE_V4_SIMPLE_LIMIT_ORDERS',
        base_url: baseUrl,
        signature_format: 'COMPLETE_DOCUMENTATION_BASED',
        tp_sl_method: 'SIMPLE_LIMIT_ORDERS_WITH_REDUCE_ONLY',
        approach: 'MAIN_ORDER_PLUS_SIMPLE_LIMIT_TP_SL',
        note: 'TP/SL установлены через простые limit ордера с reduce_only: true'
      };
    }
