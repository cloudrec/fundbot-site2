...
serve(async (req) => {
  ...
  try {
    ...
    console.log('📊 UPDATED POSITIONS: Getting positions for user:', user_id);
    ...
    try {
      ...
      console.log(`📊 UPDATED POSITIONS: Calling ${functionName} for ${currentExchange}`);
      ...
      if (error) {
        ...
        console.log(`📊 UPDATED POSITIONS: Got ${positions.length} positions from ${currentExchange}`);
    ...
    console.log('📊 UPDATED POSITIONS: Total open positions found:', allPositions.length);
    ...
    console.error('❌ UPDATED POSITIONS Error:', error.message);
    return new Response(
