...
serve(async (req) => {
  ...
  try {
    ...
    if (!gateApiKey || !gateApiSecret) {
      console.log('🚫 GATE.IO: API ключи не найдены');
        ...
        return new Response(
          JSON.stringify({ 
            ...
            data: {
              ...
              currency: 'USDT',
              status: '🔑 НАСТРОЙТЕ API КЛЮЧИ',
              exchange: 'GATE.IO',
              note: 'Добавьте GATE_API_KEY и GATE_API_SECRET в настройки API ключей',
              setup_required: true
            }
          }),
      ...
      if (action === 'place_order_with_tp_sl') {
        return new Response(
          JSON.stringify({ 
            success: false, 
            error: '🔑 Для торговли на Gate.io необходимо настроить API ключи.\n\n📋 Инструкция:\n1. Перейдите на вкладку "API Ключи"\n2. Добавьте GATE_API_KEY и GATE_API_SECRET\n3. Получите ключи на gate.io в разделе API Management\n\n⚠️ Убедитесь, что у ключей есть права на торговлю фьючерсами!'
          }),
          { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }
      
      throw new Error('🔑 Gate.io API ключи не настроены. Перейдите на вкладку "API Ключи" и добавьте GATE_API_KEY и GATE_API_SECRET.');
    }

    console.log('🟢 GATE.IO: API ключи найдены, выполняем действие:', action);
