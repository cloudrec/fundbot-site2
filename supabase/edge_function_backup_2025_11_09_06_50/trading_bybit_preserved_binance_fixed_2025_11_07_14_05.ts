...
// ПРАВИЛЬНЫЕ ФУНКЦИИ ПОДПИСИ BYBIT (РАБОЧИЕ - НЕ ТРОГАЕМ!)
...
// Функция правильного округления количества для Bybit (РАБОЧАЯ - НЕ ТРОГАЕМ!)
...
async function placeOrderWithTPSL(apiKey: any, settings: any) {
  try {
    ...
    if (apiKey.exchange === 'binance') {
      console.log('🔥 Processing Binance order with TP/SL in ONE ORDER like Bybit');
      ...
      // 3. Размещаем ОДИН ордер с TP/SL как на Bybit
      const timestamp = Date.now();
      const side = 'BUY';
      const type = 'MARKET';
      
      // ИСПРАВЛЕНО: Один ордер с TP/SL параметрами как на Bybit
      const orderQuery = `symbol=${symbol}&side=${side}&type=${type}&quantity=${quantity}&stopPrice=${tpPrice}&priceProtect=true&timestamp=${timestamp}`;
      const orderSignature = await createBinanceSignature(apiKey.api_secret, orderQuery);
      
      console.log('🔥 Binance SINGLE order with TP/SL params:', { symbol, side, type, quantity, tpPrice, slPrice });
      ...
      return {
        ...
        side: orderData.side,
        status: 'LIVE',
        message: `Боевой ордер Binance с TP/SL в одном ордере: ${orderData.orderId}`,
        quantity: quantity,
        price: currentPrice,
        tp_price: tpPrice,
        sl_price: slPrice,
        ...
        sl_status: 'SUCCESS',
        price_source: 'CACHED',
        order_type: 'SINGLE_WITH_TP_SL',
        duplicate_protection: 'ENABLED',
        binance_like_bybit: 'ENABLED'
      };
    }
    ...
    if (apiKey.exchange === 'bybit') {
      ...
      console.log('🔥 WORKING Quantity calculation:', {
        notionalValue,
        currentPrice,
        rawQuantity,
...
// РАБОЧИЕ ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ ДЛЯ BYBIT - НЕ ТРОГАЕМ!
