...
// Функция создания подписи для Gate.io (ИСПРАВЛЕННАЯ ДЛЯ GATE.COM)
async function createGateSignature(secret: string, method: string, url: string, queryString: string, body: string): Promise<{ signature: string, timestamp: string }> {
  try {
    ...
    console.log('🔥 Gate signature message:', message);
    ...
    console.log('🔥 Gate signature result:', result);
    return { signature: result, timestamp };
  } catch (error) {
    console.error('❌ Error creating Gate signature:', error);
    throw error;
  }
}
...
async function getBalance(apiKey: any, settings: any) {
  try {
    ...
    // 🆕 GATE.IO С ПРАВИЛЬНЫМ АДРЕСОМ GATE.COM (ИСПРАВЛЕНО!)
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing Gate.io balance request (ИСПРАВЛЕННЫЙ GATE.COM)');
      
      // ИСПРАВЛЕНО: Используем правильный адрес gate.com
      const baseUrl = apiKey.is_testnet ? 'https://fx-api-testnet.gate.com' : 'https://api.gate.com';
      const url = '/api/v4/futures/usdt/accounts';
      const queryString = '';
      const body = '';
      
      console.log('🔥 GATE DEBUG: Base URL:', baseUrl);
      console.log('🔥 GATE DEBUG: Full URL will be:', baseUrl + url);
      
      const { signature, timestamp } = await createGateSignature(apiKey.api_secret, 'GET', url, queryString, body);
      
      const fullUrl = baseUrl + url;
      console.log('🔥 Gate.io balance request FULL URL:', fullUrl);
      ...
      const data = await response.json();
      console.log('🔥 Gate.io balance response status:', response.status);
      console.log('🔥 Gate.io balance response:', data);

      if (response.status !== 200) {
        throw new Error(`Gate.io API Error: ${data.message || 'Unknown error'} (Code: ${response.status})`);
      }
      ...
      console.log('🔥 Gate.io balance result:', result);
      return result;
    }
...
async function placeOrderWithTPSL(apiKey: any, settings: any) {
  try {
    ...
    // 🆕 GATE.IO С ПРАВИЛЬНЫМ АДРЕСОМ GATE.COM (ИСПРАВЛЕНО!)
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing Gate.io order with TP/SL (ИСПРАВЛЕННЫЙ GATE.COM)');
      
      const symbol = `${settings.base_asset}_${settings.quote_asset}`;
      // ИСПРАВЛЕНО: Используем правильный адрес gate.com
      const baseUrl = apiKey.is_testnet ? 'https://fx-api-testnet.gate.com' : 'https://api.gate.com';
      
      console.log('🔥 GATE DEBUG: Base URL for orders:', baseUrl);
      console.log('🔥 GATE DEBUG: Symbol:', symbol);
      
      // Получаем текущую цену
      const priceUrl = `/api/v4/futures/usdt/tickers?contract=${symbol}`;
      const fullPriceUrl = baseUrl + priceUrl;
      console.log('🔥 GATE DEBUG: Price URL:', fullPriceUrl);
      
      const priceResponse = await fetch(fullPriceUrl);
      const priceData = await priceResponse.json();
      
      console.log('🔥 GATE DEBUG: Price response status:', priceResponse.status);
      console.log('🔥 GATE DEBUG: Price data:', priceData);
      
      if (!priceData || priceData.length === 0) {
        throw new Error(`Не удалось получить цену для ${symbol} на Gate.com`);
      }
      
      const currentPrice = parseFloat(priceData[0].last);
      console.log('🔥 Gate.io current price:', currentPrice);
      ...
      console.log('🔥 Gate.io order params:', { symbol, quantity, currentPrice, tpPrice, slPrice });
      ...
      console.log('🔥 Gate.io order request FULL URL:', fullOrderUrl);
      ...
      const data = await response.json();
      console.log('🔥 Gate.io order response status:', response.status);
      console.log('🔥 Gate.io order response:', data);
      ...
      return {
        ...
        side: "long",
        status: 'LIVE',
        message: `Боевой ордер Gate.io (ИСПРАВЛЕННЫЙ gate.com): ${data.id}`,
        quantity: quantity,
        price: currentPrice,
        tp_price: tpPrice,
        sl_price: slPrice,
...
async function getPositions(apiKey: any, settings: any) {
  try {
    ...
    if (apiKey.exchange === 'gate') {
      ...
      console.log('🔥 GATE DEBUG: Positions Base URL:', baseUrl);
      
      const { signature, timestamp } = await createGateSignature(apiKey.api_secret, 'GET', url, queryString, body);
      
      const fullUrl = baseUrl + url;
      console.log('🔥 Gate.io positions request FULL URL:', fullUrl);
      ...
      const data = await response.json();
      console.log('🔥 Gate.io positions response:', data);
...
async function cancelAllOrders(apiKey: any, settings: any) {
  try {
    ...
    if (apiKey.exchange === 'gate') {
      ...
      console.log('🔥 GATE DEBUG: Cancel orders Base URL:', baseUrl);
      
      const { signature, timestamp } = await createGateSignature(apiKey.api_secret, 'DELETE', url, queryString, body);
      
      const fullUrl = `${baseUrl}${url}?${queryString}`;
      console.log('🔥 Gate.io cancel orders request FULL URL:', fullUrl);
      ...
      const data = await response.json();
      console.log('🔥 Gate.io cancel orders response:', data);
      ...
      return {
        ...
        exchange: 'GATE',
        status: 'LIVE',
        message: `Отменено ${Array.isArray(data) ? data.length : 0} ордеров Gate.io`,
        details: data
      };
    }
...
async function closeAllPositions(apiKey: any, settings: any) {
  try {
    ...
    if (apiKey.exchange === 'gate') {
      ...
      console.log('🔥 GATE DEBUG: Close positions Base URL:', baseUrl);
      ...
      for (const position of positions) {
        try {
          ...
          const fullUrl = baseUrl + url;
          console.log('🔥 Gate.io close position request FULL URL:', fullUrl);
          ...
          const data = await response.json();
          console.log('🔥 Gate.io close position response:', data);
      ...
      return {
        ...
        exchange: "GATE",
        status: "LIVE",
        message: `Закрыто ${closedCount} из ${positions.length} позиций Gate.io`,
        details: closeResults
      };
    }
