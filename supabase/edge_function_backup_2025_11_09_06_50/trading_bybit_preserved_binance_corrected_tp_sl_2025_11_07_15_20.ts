...
async function placeOrderWithTPSL(apiKey: any, settings: any) {
  try {
    ...
    if (apiKey.exchange === 'binance') {
      console.log('🔥 Processing Binance order with CORRECTED TP/SL logic');
      ...
      console.log('🔥 Waiting 5 seconds before setting TP/SL...');
      await delay(5000);

      // 4. ИСПРАВЛЕННЫЕ TP/SL ОРДЕРА
      let tpOrderId = null;
      let slOrderId = null;
      ...
      try {
        // Take Profit ордер (LIMIT вместо TAKE_PROFIT_MARKET)
        ...
        // Stop Loss ордер (STOP_MARKET с правильными параметрами)
      ...
      return {
        ...
        message: `Боевой ордер Binance с исправленными TP/SL: ${orderData.orderId}`,
        quantity: quantity,
        price: currentPrice,
        tp_price: tpPrice,
        sl_price: slPrice,
        ...
        binance_version: 'CORRECTED_TP_SL_LOGIC'
      };
    }
