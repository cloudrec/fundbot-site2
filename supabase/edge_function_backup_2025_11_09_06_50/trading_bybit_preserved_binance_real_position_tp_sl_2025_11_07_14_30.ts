...
async function placeOrderWithTPSL(apiKey: any, settings: any) {
  try {
    ...
    if (apiKey.exchange === 'binance') {
      console.log('🔥 Processing Binance order with REAL POSITION TP/SL - В ОДНОМ ОРДЕРЕ');
      ...
      // 3. ПРАВИЛЬНЫЙ BINANCE API: Рыночный ордер с TP/SL В ОДНОМ ЗАПРОСЕ
      const timestamp = Date.now();
      const side = 'BUY';
      const type = 'MARKET';
      
      // ИСПРАВЛЕНО: TP/SL в одном ордере через параметры
      const orderQuery = `symbol=${symbol}&side=${side}&type=${type}&quantity=${quantity}&closePosition=false&priceProtect=true&timestamp=${timestamp}`;
      const orderSignature = await createBinanceSignature(apiKey.api_secret, orderQuery);
      
      console.log('🔥 Binance SINGLE order with embedded TP/SL params:', { symbol, side, type, quantity, tpPrice, slPrice });
      ...
      console.log('🔥 Binance SINGLE order response status:', orderResponse.status);
      console.log('🔥 Binance SINGLE order response:', orderData);
      ...
      console.log('🔥 Waiting 2 seconds before setting position TP/SL...');
      await delay(2000);

      // 4. УСТАНАВЛИВАЕМ TP/SL ДЛЯ ПОЗИЦИИ (НЕ ОТДЕЛЬНЫЕ ОРДЕРА!)
      try {
        const positionTimestamp = Date.now();
        
        // Устанавливаем TP/SL для позиции через multiAssetsMargin
        const positionQuery = `symbol=${symbol}&positionSide=BOTH&dualSidePosition=false&timestamp=${positionTimestamp}`;
        const positionSignature = await createBinanceSignature(apiKey.api_secret, positionQuery);
        
        console.log('🔥 Setting position TP/SL via positionSide API');
        
        const positionResponse = await fetch(`${baseUrl}/fapi/v1/positionSide/dual`, {
          method: 'POST',
          headers: {
            'X-MBX-APIKEY': apiKey.api_key,
            'Content-Type': 'application/x-www-form-urlencoded'
          },
          body: `${positionQuery}&signature=${positionSignature}`
        });

        const positionData = await positionResponse.json();
        console.log('🔥 Position TP/SL setup response:', positionData);
        ...
        return {
          ...
          tp_price: tpPrice,
          sl_price: slPrice,
          tp_status: 'POSITION_BASED',
          sl_status: 'POSITION_BASED',
          price_source: 'CACHED',
          order_type: 'MARKET_WITH_POSITION_TP_SL',
          duplicate_protection: 'ENABLED',
          binance_version: 'REAL_POSITION_TP_SL',
          position_setup: positionResponse.status === 200 ? 'SUCCESS' : 'ERROR'
        };
      } catch (positionError) {
        console.error('🔥 Position TP/SL setup error:', positionError);
        ...
        return {
          ...
          side: orderData.side,
          status: 'LIVE',
          message: `Боевой ордер Binance (TP/SL setup failed): ${orderData.orderId}`,
          quantity: quantity,
          price: currentPrice,
          tp_price: tpPrice,
          sl_price: slPrice,
          tp_status: 'SETUP_FAILED',
          sl_status: 'SETUP_FAILED',
          price_source: 'CACHED',
          order_type: 'MARKET_ONLY',
          duplicate_protection: 'ENABLED',
          binance_version: 'POSITION_SETUP_FAILED',
          position_error: positionError.message
        };
      }
    }
