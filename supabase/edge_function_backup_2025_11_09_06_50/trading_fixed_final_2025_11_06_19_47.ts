...
async function placeTestOrder(apiKey: any, settings: any) {
  try {
    ...
    await validateSymbol(symbol, apiKey.is_testnet);
    ...
    const orderData = {
      ...
      side: "Buy",
      orderType: "Market",
      qty: "0.01", // Увеличиваем минимальный размер
      timeInForce: "IOC"
    };
    ...
    return {
      ...
      exchange: "BYBIT",
      mode: apiKey.is_testnet ? "TESTNET" : "LIVE",
      note: `🔥 ${apiKey.is_testnet ? 'Тестовый' : 'Реальный'} ордер размещен на Bybit`
    };
  } catch (error) {
...
async function placeOrderWithTPSL(apiKey: any, settings: any) {
  try {
    ...
    // Проверяем доступность символа и получаем текущую цену
    ...
    const orderAmount = (orderAmountUSD / currentPrice).toFixed(3);
    ...
    return {
      ...
      exchange: "BYBIT",
      mode: apiKey.is_testnet ? "TESTNET" : "LIVE",
      note: `🔥 ${apiKey.is_testnet ? 'Тестовый' : 'Реальный'} ордер с TP/SL размещен на Bybit`
    };
  } catch (error) {
...
async function validateSymbol(symbol: string, isTestnet: boolean = false): Promise<void> {
  try {
    const baseUrl = isTestnet ? 'https://api-testnet.bybit.com' : 'https://api.bybit.com';
    ...
    console.log('🔥 Symbol validated:', symbol);
  } catch (error) {
