  ...
  try {
    ...
    switch (action) {
      ...
      case 'scan_funding':
        ...
        result = await scanFundingWithAutoNotifications(supabase, user_id, settingsData, apiKeys);
        break;
...
// 💰 УЛУЧШЕННАЯ ФУНКЦИЯ СКАНИРОВАНИЯ ФАНДИНГА С АВТОМАТИЧЕСКИМИ УВЕДОМЛЕНИЯМИ
async function scanFundingWithAutoNotifications(supabase: any, user_id: string, settings: any, apiKeys: any[]) {
  try {
    console.log('💰 AUTO FUNDING SCAN: Starting comprehensive scan for user:', user_id);
    ...
    for (const exchange of exchanges) {
      try {
        console.log(`💰 AUTO FUNDING SCAN: Scanning ${exchange.toUpperCase()}...`);
    ...
    console.log('💰 AUTO FUNDING SCAN: Successful results:', successfulResults);
    ...
    if (successfulResults.length > 0) {
      ...
      console.log('💰 AUTO FUNDING SCAN: Sending comprehensive Telegram message');
      const telegramResult = await sendTelegramNotification(supabase, user_id, telegramMessage, 'funding');
      console.log('💰 AUTO FUNDING SCAN: Telegram result:', telegramResult);
...
<b>Время сканирования:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
      ...
      const telegramResult = await sendTelegramNotification(supabase, user_id, errorMessage, 'funding');
      
      return {
        scan_completed: true,
        exchanges_scanned: exchanges.length,
        successful_scans: 0,
        failed_scans: scanResults.length,
        all_results: scanResults,
        telegram_sent: telegramResult.success,
        telegram_error: telegramResult.reason || null,
        scan_time: new Date().toISOString(),
        message: 'Сканирование завершено: данные о фандинге не найдены'
      };
    }
    
  } catch (error: any) {
    console.error('💰 AUTO FUNDING SCAN: Error:', error);
...
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
    ...
    await sendTelegramNotification(supabase, user_id, errorMessage, 'funding');
...
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
    ...
    // 🔵 GATE.IO IMPLEMENTATION - ПРАВИЛЬНЫЕ TP/SL ДЛЯ ФЬЮЧЕРСОВ
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing GATE.IO API v4 order - FUTURES TP/SL METHOD');
      ...
      const tpPrice = roundGatePrice(tpPriceRaw);
      const slPrice = roundGatePrice(slPriceRaw);
      ...
      // 🔥 НОВЫЙ ПОДХОД: ОСНОВНОЙ ОРДЕР С ПАРАМЕТРАМИ TP/SL (как для фьючерсов)
      const orderData = {
        contract: symbol,
        size: quantity,
        ...
        // ДОБАВЛЯЕМ TP/SL ПАРАМЕТРЫ НАПРЯМУЮ В ОСНОВНОЙ ОРДЕР
        auto_size: "close_long", // Автоматическое закрытие лонга
        close: false // Это не закрывающий ордер
      };
      
      console.log('🔥 GATE.IO v4 order data with futures TP/SL:', orderData);
      ...
      console.log('🔥 GATE.IO v4 order created:', orderId);

      // Задержка перед установкой отдельных TP/SL ордеров
      console.log('🔥 GATE.IO v4: Waiting 3 seconds before setting separate TP/SL orders...');
      await delay(3000);

      // 🔥 ОТДЕЛЬНЫЕ TP/SL ОРДЕРА (TRIGGERED ORDERS)
      let tpStatus = 'NOT_SET';
      let slStatus = 'NOT_SET';
      let tpOrderId = null;
      let slOrderId = null;
      ...
      try {
        ...
        console.log('🔥 GATE.IO v4: Setting Take Profit triggered order...');
        
        const tpOrderData = {
          contract: symbol,
          size: -quantity, // Отрицательный размер для закрытия лонга
          price: "0", // Market order для закрытия
          tif: "ioc",
          text: `tp-${Date.now().toString().slice(-6)}`,
          reduce_only: true, // Только для закрытия позиции
          auto_size: "close_long" // Автоматическое закрытие лонга при достижении цены
        };

        console.log('🔥 GATE.IO v4 TP triggered order data:', tpOrderData);

        const tpPayload = JSON.stringify(tpOrderData);
        const { signature: tpSig, timestamp: tpTs } = await createCompleteGateSignature(apiKey.api_secret, 'POST', prefix + orderUrl, '', tpPayload);

        const tpResponse = await fetch(baseUrl + prefix + orderUrl, {
          method: 'POST',
          headers: {
            'KEY': apiKey.api_key,
            'SIGN': tpSig,
        ...
        const tpData = await tpResponse.json();
        console.log('🔥 GATE.IO v4 TP response status:', tpResponse.status);
        console.log('🔥 GATE.IO v4 TP response:', tpData);

        if (tpResponse.status === 201) {
          tpStatus = 'SUCCESS';
          tpOrderId = tpData.id;
          console.log('🔥 GATE.IO v4 TP triggered order created:', tpOrderId);
        } else {
          tpStatus = 'ERROR';
          tpError = tpData.message || 'Unknown TP error';
          console.error('🔥 GATE.IO v4 TP error:', tpError);
        }
        ...
        console.log('🔥 GATE.IO v4: Setting Stop Loss triggered order...');
        ...
        const slOrderData = {
          ...
          price: "0", // Market order для закрытия
          tif: "ioc",
          text: `sl-${Date.now().toString().slice(-6)}`,
          reduce_only: true, // Только для закрытия позиции
          auto_size: "close_long" // Автоматическое закрытие лонга при достижении цены
        };

        console.log('🔥 GATE.IO v4 SL triggered order data:', slOrderData);

        const slPayload = JSON.stringify(slOrderData);
        const { signature: slSig, timestamp: slTs } = await createCompleteGateSignature(apiKey.api_secret, 'POST', prefix + orderUrl, '', slPayload);

        const slResponse = await fetch(baseUrl + prefix + orderUrl, {
          method: 'POST',
          headers: {
            'KEY': apiKey.api_key,
            'SIGN': slSig,
        ...
        const slData = await slResponse.json();
        console.log('🔥 GATE.IO v4 SL response status:', slResponse.status);
        console.log('🔥 GATE.IO v4 SL response:', slData);

        if (slResponse.status === 201) {
          slStatus = 'SUCCESS';
          slOrderId = slData.id;
          console.log('🔥 GATE.IO v4 SL triggered order created:', slOrderId);
        } else {
          slStatus = 'ERROR';
          slError = slData.message || 'Unknown SL error';
          console.error('🔥 GATE.IO v4 SL error:', slError);
        }

      } catch (tpslError) {
        console.error('🔥 GATE.IO v4 TP/SL triggered orders error:', tpslError);
        tpStatus = 'ERROR';
        slStatus = 'ERROR';
        tpError = tpslError.message;
        slError = tpslError.message;
...
${tpOrderId ? `<b>TP ID:</b> ${tpOrderId}` : ''}
${slOrderId ? `<b>SL ID:</b> ${slOrderId}` : ''}
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
      `.trim();
      ...
      return {
        ...
        side: "long",
        status: 'LIVE',
        message: `Боевой ордер GATE.IO v4 с triggered TP/SL: ${orderId}`,
        quantity: quantity,
        price: currentPrice,
        tp_price: tpPrice,
        sl_price: slPrice,
        ...
        api_version: 'GATE_V4_FUTURES_TRIGGERED_ORDERS',
        base_url: baseUrl,
        signature_format: 'COMPLETE_DOCUMENTATION_BASED',
        text_field_fix: 'CORRECT_T_PREFIX_APPLIED',
        tp_sl_method: 'FUTURES_TRIGGERED_ORDERS_WITH_AUTO_SIZE'
      };
    }
...
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
    ...
    // 🔵 GATE.IO CANCEL ORDERS - УЛУЧШЕННАЯ ВЕРСИЯ
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing GATE.IO API v4 cancel orders request - ENHANCED VERSION');
      ...
      try {
        ...
        console.log('🔥 GATE.IO v4: Cancelling regular orders...');
        ...
        if (ordersResponse.status === 200) {
          const regularCancelled = Array.isArray(ordersData) ? ordersData.length : 0;
          totalCancelled += regularCancelled;
          cancelResults.push({ type: 'regular_orders', cancelled: regularCancelled, status: 'SUCCESS' });
          console.log('🔥 GATE.IO v4: Regular orders cancelled:', regularCancelled);
        } else {
          cancelResults.push({ type: 'regular_orders', cancelled: 0, status: 'ERROR', error: ordersData.message });
        }
        ...
        console.error('🔥 GATE.IO v4 cancel error:', cancelError);
        cancelResults.push({ type: 'error', cancelled: 0, status: 'ERROR', error: cancelError.message });
      }
...
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
      ...
      return {
        ...
        api_version: 'GATE_V4_ENHANCED',
        symbol: symbol,
        cancel_results: cancelResults,
        cancel_types: ['regular_orders']
      };
    }
