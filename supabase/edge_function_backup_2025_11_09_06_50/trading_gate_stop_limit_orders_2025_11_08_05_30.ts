...
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
    ...
    // 🔵 GATE.IO С STOP-LIMIT ОРДЕРАМИ
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing GATE.IO API v4 order with STOP-LIMIT ORDERS');
      ...
      console.log('🔥 GATE.IO v4 STOP-LIMIT: Base URL:', baseUrl);
      console.log('🔥 GATE.IO v4 STOP-LIMIT: Symbol:', symbol);
      
      // Получаем текущую цену
      const priceUrl = `/futures/usdt/tickers?contract=${symbol}`;
      const fullPriceUrl = baseUrl + prefix + priceUrl;
      console.log('🔥 GATE.IO v4 STOP-LIMIT: Price URL:', fullPriceUrl);
      
      const priceResponse = await fetch(fullPriceUrl);
      const priceData = await priceResponse.json();
      
      console.log('🔥 GATE.IO v4 STOP-LIMIT: Price response status:', priceResponse.status);
      console.log('🔥 GATE.IO v4 STOP-LIMIT: Price data:', priceData);
      ...
      console.log('🔥 GATE.IO v4 STOP-LIMIT current price:', currentPrice);
      ...
      console.log('🔥 GATE.IO v4 STOP-LIMIT order params:', { 
        symbol, 
        quantity, 
        currentPrice,
        tpPriceNumber,
      ...
      console.log('🔥 GATE.IO v4 STOP-LIMIT main order data:', orderData);
      ...
      console.log('🔥 GATE.IO v4 STOP-LIMIT order request URL:', fullOrderUrl);
      ...
      const data = await response.json();
      console.log('🔥 GATE.IO v4 STOP-LIMIT order response status:', response.status);
      console.log('🔥 GATE.IO v4 STOP-LIMIT order response:', data);

      if (response.status !== 201) {
        throw new Error(`GATE.IO v4 STOP-LIMIT API Error: ${data.message || 'Unknown error'} (Code: ${response.status})`);
      }

      const orderId = data.id;
      console.log('🔥 GATE.IO v4 STOP-LIMIT main order created:', orderId);

      // Задержка перед установкой TP/SL
      console.log('🔥 Waiting 5 seconds before setting STOP-LIMIT TP/SL...');
      await delay(5000);

      // 2. STOP-LIMIT ОРДЕРА TP/SL
      let tpOrderId = null;
      let slOrderId = null;
      let tpStatus = 'NOT_SET';
      let slStatus = 'NOT_SET';
      ...
      try {
        // TAKE PROFIT STOP-LIMIT ОРДЕР
        const tpOrderData = {
          contract: symbol,
          size: -quantity, // Противоположный размер для закрытия
          price: tpPriceString, // Limit цена
          tif: "gtc",
          text: `t-tp-${Date.now().toString().slice(-6)}`,
          reduce_only: true, // Только для закрытия позиции
          // Добавляем stop параметры для STOP-LIMIT
          stop_price: tpPriceString, // Цена активации
          stop_type: "limit" // Тип stop ордера
        };

        console.log('🔥 GATE.IO v4 STOP-LIMIT TP order data:', tpOrderData);
        ...
        const tpResponse = await fetch(fullOrderUrl, {
          method: 'POST',
          headers: {
            'KEY': apiKey.api_key,
            'SIGN': tpSig,
        ...
        const tpData = await tpResponse.json();
        console.log('🔥 GATE.IO v4 STOP-LIMIT TP response status:', tpResponse.status);
        console.log('🔥 GATE.IO v4 STOP-LIMIT TP response:', tpData);

        if (tpResponse.status === 201) {
          tpOrderId = tpData.id;
          tpStatus = 'SUCCESS';
          console.log('🔥 GATE.IO v4 STOP-LIMIT TP order created:', tpOrderId);
        } else {
          tpStatus = 'ERROR';
          tpError = tpData.message || 'Unknown TP error';
          console.error('🔥 GATE.IO v4 STOP-LIMIT TP error:', tpData.message);
        }

        // Задержка между TP и SL
        await delay(3000);

        // STOP LOSS STOP-LIMIT ОРДЕР
        const slOrderData = {
          contract: symbol,
          size: -quantity, // Противоположный размер для закрытия
          price: slPriceString, // Limit цена
          tif: "gtc",
          text: `t-sl-${Date.now().toString().slice(-6)}`,
          reduce_only: true, // Только для закрытия позиции
          // Добавляем stop параметры для STOP-LIMIT
          stop_price: slPriceString, // Цена активации
          stop_type: "limit" // Тип stop ордера
        };

        console.log('🔥 GATE.IO v4 STOP-LIMIT SL order data:', slOrderData);
        ...
        const slResponse = await fetch(fullOrderUrl, {
          method: 'POST',
          headers: {
            'KEY': apiKey.api_key,
            'SIGN': slSig,
        ...
        const slData = await slResponse.json();
        console.log('🔥 GATE.IO v4 STOP-LIMIT SL response status:', slResponse.status);
        console.log('🔥 GATE.IO v4 STOP-LIMIT SL response:', slData);

        if (slResponse.status === 201) {
          slOrderId = slData.id;
          slStatus = 'SUCCESS';
          console.log('🔥 GATE.IO v4 STOP-LIMIT SL order created:', slOrderId);
        } else {
          slStatus = 'ERROR';
          slError = slData.message || 'Unknown SL error';
          console.error('🔥 GATE.IO v4 STOP-LIMIT SL error:', slData.message);
        }

      } catch (tpslError) {
        console.error('🔥 GATE.IO v4 STOP-LIMIT TP/SL error:', tpslError);
        tpStatus = 'ERROR';
        slStatus = 'ERROR';
        tpError = tpslError.message;
        slError = tpslError.message;
      ...
      const telegramMessage = `
<b>🚀 ОРДЕР РАЗМЕЩЕН</b>
<b>Биржа:</b> Gate.io (Stop-Limit ордера)
<b>Символ:</b> ${symbol}
<b>Сторона:</b> LONG
<b>Количество:</b> ${quantity}
<b>Цена входа:</b> $${currentPrice.toFixed(4)}
...
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
      ...
      return {
        ...
        side: "long",
        status: 'LIVE',
        message: `Боевой ордер GATE.IO v4 с Stop-Limit TP/SL: ${orderId}`,
        quantity: quantity,
        price: currentPrice,
        ...
        api_version: 'GATE_V4_STOP_LIMIT_ORDERS',
        base_url: baseUrl,
        signature_format: 'COMPLETE_DOCUMENTATION_BASED',
        tp_sl_method: 'STOP_LIMIT_ORDERS',
        approach: 'MAIN_ORDER_PLUS_STOP_LIMIT_TP_SL',
        note: 'TP/SL установлены как Stop-Limit ордера с reduce_only=true'
      };
    }
