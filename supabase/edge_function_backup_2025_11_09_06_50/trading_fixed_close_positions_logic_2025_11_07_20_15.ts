...
serve(async (req) => {
  ...
  try {
    console.log('🔥 FIXED CLOSE POSITIONS Gate.io API v4 Function started');
...
// 🆕 ИСПРАВЛЕННАЯ ФУНКЦИЯ СОЗДАНИЯ ПОДПИСИ ДЛЯ GATE.IO API V4 С ПРАВИЛЬНЫМ ЗАКРЫТИЕМ ПОЗИЦИЙ
async function createGateSignatureFixed(secret: string, method: string, url: string, queryString: string = '', payloadString: string = ''): Promise<{ signature: string, timestamp: string }> {
  try {
    ...
    console.log('🔥 FIXED CLOSE POSITIONS Gate.io v4 signature inputs:');
    ...
    console.log('🔥 FIXED CLOSE POSITIONS Gate.io v4 hashed payload:', hashedPayload);
    
    // РАБОЧИЙ ФОРМАТ СТРОКИ ПОДПИСИ ПО ОФИЦИАЛЬНОМУ ПРИМЕРУ:
    // method + '\n' + url + '\n' + query_string + '\n' + hashed_payload + '\n' + timestamp
    const signatureString = `${method}\n${url}\n${queryString}\n${hashedPayload}\n${timestamp}`;
    
    console.log('🔥 FIXED CLOSE POSITIONS Gate.io v4 signature string:');
    ...
    console.log('🔥 FIXED CLOSE POSITIONS Gate.io v4 signature result:', result);
    return { signature: result, timestamp };
  } catch (error) {
    console.error('❌ Error creating FIXED CLOSE POSITIONS Gate.io v4 signature:', error);
    throw error;
  }
}
...
async function getBalance(apiKey: any, settings: any) {
  try {
    console.log('🔥 FIXED CLOSE POSITIONS getBalance started for exchange:', apiKey.exchange);
    ...
    // 🆕 ИСПРАВЛЕННАЯ GATE.IO API V4 РЕАЛИЗАЦИЯ С ПРАВИЛЬНЫМ ЗАКРЫТИЕМ ПОЗИЦИЙ
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing FIXED CLOSE POSITIONS Gate.io API v4 balance request');
      ...
      console.log('🔥 FIXED CLOSE POSITIONS Gate.io v4: Base URL:', baseUrl);
      console.log('🔥 FIXED CLOSE POSITIONS Gate.io v4: Prefix:', prefix);
      console.log('🔥 FIXED CLOSE POSITIONS Gate.io v4: URL path:', url);
      console.log('🔥 FIXED CLOSE POSITIONS Gate.io v4: Full URL:', baseUrl + prefix + url);
      ...
      console.log('🔥 FIXED CLOSE POSITIONS Gate.io v4 balance request URL:', fullUrl);
      ...
      const data = await response.json();
      console.log('🔥 FIXED CLOSE POSITIONS Gate.io v4 balance response status:', response.status);
      console.log('🔥 FIXED CLOSE POSITIONS Gate.io v4 balance response:', data);

      if (response.status !== 200) {
        throw new Error(`FIXED CLOSE POSITIONS Gate.io v4 API Error: ${data.message || 'Unknown error'} (Code: ${response.status})`);
      }
      ...
      const result = {
        ...
        result: data,
        debug_url: fullUrl,
        api_version: 'FIXED_CLOSE_POSITIONS_GATE_V4',
        base_url: baseUrl,
        signature_format: 'OFFICIAL_EXAMPLE_BASED'
      };
      
      console.log('🔥 FIXED CLOSE POSITIONS Gate.io v4 balance result:', result);
      return result;
    }
    ...
    console.error('🔥 FIXED CLOSE POSITIONS Balance error:', error);
...
async function placeOrderWithTPSL(apiKey: any, settings: any) {
  try {
    console.log('🔥 FIXED CLOSE POSITIONS placeOrderWithTPSL started for exchange:', apiKey.exchange);
    ...
    // 🆕 ИСПРАВЛЕННАЯ GATE.IO API V4 РЕАЛИЗАЦИЯ С УПРОЩЕННЫМИ TP/SL
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing FIXED CLOSE POSITIONS Gate.io API v4 order with SIMPLIFIED TP/SL');
      ...
      console.log('🔥 FIXED CLOSE POSITIONS Gate.io v4 ORDER: Base URL:', baseUrl);
      console.log('🔥 FIXED CLOSE POSITIONS Gate.io v4 ORDER: Symbol:', symbol);
      
      // Получаем текущую цену
      const priceUrl = `/futures/usdt/tickers?contract=${symbol}`;
      const fullPriceUrl = baseUrl + prefix + priceUrl;
      console.log('🔥 FIXED CLOSE POSITIONS Gate.io v4 ORDER: Price URL:', fullPriceUrl);
      
      const priceResponse = await fetch(fullPriceUrl);
      const priceData = await priceResponse.json();
      
      console.log('🔥 FIXED CLOSE POSITIONS Gate.io v4 ORDER: Price response status:', priceResponse.status);
      console.log('🔥 FIXED CLOSE POSITIONS Gate.io v4 ORDER: Price data:', priceData);
      
      if (!priceData || priceData.length === 0) {
        throw new Error(`Не удалось получить цену для ${symbol} на FIXED CLOSE POSITIONS Gate.io v4`);
      }
      
      const currentPrice = parseFloat(priceData[0].last);
      console.log('🔥 FIXED CLOSE POSITIONS Gate.io v4 current price:', currentPrice);
      
      // Рассчитываем количество (Gate.io использует контракты)
      const calculatedQuantity = Math.floor(settings.order_amount_usd / currentPrice);
      const quantity = calculatedQuantity;
      
      // 🔧 УПРОЩЕННЫЕ TP/SL: ТОЛЬКО ОСНОВНОЙ ОРДЕР БЕЗ АВТОМАТИЧЕСКИХ TP/SL
      console.log('🔥 FIXED CLOSE POSITIONS Gate.io v4 order params (SIMPLIFIED):', { 
        symbol, 
        quantity, 
        currentPrice
      });
      
      // 1. ТОЛЬКО ОСНОВНОЙ ОРДЕР (БЕЗ TP/SL ДЛЯ УПРОЩЕНИЯ)
      const orderData = {
        contract: symbol,
        size: quantity,
      ...
      console.log('🔥 FIXED CLOSE POSITIONS Gate.io v4 SIMPLIFIED main order data:', orderData);
      ...
      console.log('🔥 FIXED CLOSE POSITIONS Gate.io v4 order request URL:', fullOrderUrl);
      ...
      const data = await response.json();
      console.log('🔥 FIXED CLOSE POSITIONS Gate.io v4 order response status:', response.status);
      console.log('🔥 FIXED CLOSE POSITIONS Gate.io v4 order response:', data);
      ...
      console.log('🔥 FIXED CLOSE POSITIONS Gate.io v4 SIMPLIFIED main order created:', orderId);

      // 🔧 УПРОЩЕННЫЙ ПОДХОД: НЕ УСТАНАВЛИВАЕМ TP/SL АВТОМАТИЧЕСКИ
      // Пользователь может установить их вручную через интерфейс Gate.io
      ...
      return {
        ...
        side: "long",
        status: 'LIVE',
        message: `Боевой ордер FIXED CLOSE POSITIONS Gate.io v4 (УПРОЩЕННЫЙ): ${orderId}`,
        quantity: quantity,
        price: currentPrice,
        ...
        sl_error: null,
        exchange: 'GATE',
        api_url: 'FIXED_CLOSE_POSITIONS_gateio.ws',
        debug_url: fullOrderUrl,
        api_version: 'FIXED_CLOSE_POSITIONS_GATE_V4_SIMPLIFIED',
        base_url: baseUrl,
        signature_format: 'OFFICIAL_EXAMPLE_BASED',
        text_field_fix: 'APPLIED',
        tp_sl_implementation: 'SIMPLIFIED_MANUAL_SETUP_REQUIRED',
        note: 'TP/SL нужно установить вручную через интерфейс Gate.io или отдельными API вызовами'
      };
    }
    ...
    console.error('🔥 FIXED CLOSE POSITIONS Set TP/SL error:', error);
    throw error;
  }
}
...
async function getPositions(apiKey: any, settings: any) {
  try {
    console.log('🔥 FIXED CLOSE POSITIONS getPositions started for exchange:', apiKey.exchange);
    
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing FIXED CLOSE POSITIONS Gate.io API v4 positions request');
      ...
      console.log('🔥 FIXED CLOSE POSITIONS Gate.io v4 positions URL:', baseUrl + prefix + url);
      ...
      const data = await response.json();
      console.log('🔥 FIXED CLOSE POSITIONS Gate.io v4 positions response status:', response.status);
      console.log('🔥 FIXED CLOSE POSITIONS Gate.io v4 positions response:', data);

      if (response.status !== 200) {
        throw new Error(`FIXED CLOSE POSITIONS Gate.io v4 Positions API Error: ${data.message || 'Unknown error'} (Code: ${response.status})`);
      }

      // Фильтруем только открытые позиции
      const openPositions = data.filter((position: any) => parseFloat(position.size) !== 0);
      
      console.log('🔥 FIXED CLOSE POSITIONS Gate.io v4 open positions:', openPositions);
    ...
    console.error('🔥 FIXED CLOSE POSITIONS Positions error:', error);
...
async function cancelAllOrders(apiKey: any, settings: any) {
  try {
    console.log('🔥 FIXED CLOSE POSITIONS cancelAllOrders started for exchange:', apiKey.exchange);
    
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing FIXED CLOSE POSITIONS Gate.io API v4 cancel orders request');
      ...
      console.log('🔥 FIXED CLOSE POSITIONS Gate.io v4 cancel orders URL:', baseUrl + prefix + ordersUrl);
      ...
      console.log('🔥 FIXED CLOSE POSITIONS Gate.io v4 cancel orders response status:', ordersResponse.status);
      console.log('🔥 FIXED CLOSE POSITIONS Gate.io v4 cancel orders response:', ordersData);
      ...
      console.log('🔥 FIXED CLOSE POSITIONS Gate.io v4 cancel price orders URL:', baseUrl + prefix + priceOrdersUrl);
      ...
      const priceData = await priceResponse.json();
      console.log('🔥 FIXED CLOSE POSITIONS Gate.io v4 cancel price orders response status:', priceResponse.status);
      console.log('🔥 FIXED CLOSE POSITIONS Gate.io v4 cancel price orders response:', priceData);
      ...
      return {
        ...
        exchange: 'GATE',
        status: 'LIVE',
        message: `Отмена ордеров FIXED CLOSE POSITIONS Gate.io v4: ${cancelledOrders} обычных + ${cancelledPriceOrders} TP/SL`,
        api_version: 'FIXED_CLOSE_POSITIONS_GATE_V4',
        symbol: symbol
      };
    }
    ...
    console.error('🔥 FIXED CLOSE POSITIONS Cancel orders error:', error);
    throw new Error(`Ошибка отмены ордеров: ${error.message}`);
  }
}

// 🆕 ИСПРАВЛЕННАЯ РЕАЛИЗАЦИЯ ЗАКРЫТИЯ ПОЗИЦИЙ ДЛЯ GATE.IO С ПРАВИЛЬНОЙ ЛОГИКОЙ
async function closeAllPositions(apiKey: any, settings: any) {
  try {
    console.log('🔥 FIXED CLOSE POSITIONS closeAllPositions started for exchange:', apiKey.exchange);
    
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing FIXED CLOSE POSITIONS Gate.io API v4 close positions request');
      ...
      console.log('🔥 FIXED CLOSE POSITIONS Gate.io v4 get positions URL:', baseUrl + prefix + positionsUrl);
      ...
      console.log('🔥 FIXED CLOSE POSITIONS Gate.io v4 positions response status:', positionsResponse.status);
      console.log('🔥 FIXED CLOSE POSITIONS Gate.io v4 positions response:', positionsData);

      if (positionsResponse.status !== 200) {
        throw new Error(`FIXED CLOSE POSITIONS Gate.io v4 Positions API Error: ${positionsData.message || 'Unknown error'} (Code: ${positionsResponse.status})`);
      }
      ...
      console.log('🔥 FIXED CLOSE POSITIONS Gate.io v4 open positions for', symbol, ':', openPositions);
      ...
      // 2. Закрываем каждую позицию С ПРАВИЛЬНОЙ ЛОГИКОЙ
      ...
      for (const position of openPositions) {
        try {
          ...
          // 🔧 ИСПРАВЛЕНА ЛОГИКА ЗАКРЫТИЯ ПОЗИЦИЙ:
          // Если позиция положительная (лонг), закрываем отрицательным размером
          // Если позиция отрицательная (шорт), закрываем положительным размером
          const closeSize = -positionSize; // Всегда противоположный знак
          
          console.log('🔥 FIXED CLOSE POSITIONS Gate.io v4 closing position CORRECTLY:', { 
            contract: position.contract, 
            originalSize: positionSize, 
            closeSize: closeSize,
            positionType: positionSize > 0 ? 'LONG' : 'SHORT',
            closeAction: positionSize > 0 ? 'SELL_TO_CLOSE_LONG' : 'BUY_TO_CLOSE_SHORT'
          });

          const closeOrderData = {
            contract: position.contract,
            size: closeSize, // 🔧 ИСПРАВЛЕНО: правильный размер для закрытия
            price: "0", // Market order
            tif: "ioc",
            text: `t-${Date.now()}-close-position-fixed`
          };

          console.log('🔥 FIXED CLOSE POSITIONS Gate.io v4 close order data:', closeOrderData);
          ...
          console.log('🔥 FIXED CLOSE POSITIONS Gate.io v4 close position response status:', closeResponse.status);
          console.log('🔥 FIXED CLOSE POSITIONS Gate.io v4 close position response:', closeData);
          ...
          if (closeResponse.status === 201) {
            ...
            console.log('🔥 FIXED CLOSE POSITIONS Gate.io v4 position closed successfully:', closeData.id);
          } else {
            closeResults.push({
              contract: position.contract,
              status: 'ERROR',
            ...
            console.error('🔥 FIXED CLOSE POSITIONS Gate.io v4 close position error:', closeData.message);
          }
          ...
          console.error('🔥 FIXED CLOSE POSITIONS Gate.io v4 close position error:', closeError);
          closeResults.push({
            contract: position.contract,
            status: 'ERROR',
      ...
      return {
        ...
        exchange: 'GATE',
        status: 'LIVE',
        message: `Закрытие позиций FIXED CLOSE POSITIONS Gate.io v4: ${closedPositions}/${openPositions.length} успешно`,
        api_version: 'FIXED_CLOSE_POSITIONS_GATE_V4',
        symbol: symbol,
        close_logic_fix: 'APPLIED_CORRECT_SIZE_CALCULATION'
      };
    }
    ...
    console.error('🔥 FIXED CLOSE POSITIONS Close positions error:', error);
