...
async function handlePlaceOrder(apiKeys: any, settings: any) {
  console.log('🟡 BINANCE SIMPLE: Placing order');
  ...
  // ✅ ПРОСТОЙ ПОДХОД: Фиксированное количество для тестирования
  const symbol = 'BTCUSDT'; // Фиксированный символ
  const quantity = '0.001'; // Фиксированное количество BTC
  
  console.log('🟡 BINANCE SIMPLE: Order params:', { 
    symbol, 
    quantity,
    side: 'BUY',
    type: 'MARKET'
  ...
  if (response.ok) {
    return new Response(
      JSON.stringify({
        success: true,
        data: {
          message: `Binance ордер размещен: ${symbol} количество ${quantity}`,
