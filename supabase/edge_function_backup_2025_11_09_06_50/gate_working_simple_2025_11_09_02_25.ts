...
serve(async (req) => {
  ...
  try {
    ...
    console.log('🎯 GATE.IO WORKING: Starting action:', action);
    ...
    console.log('🎯 GATE.IO WORKING: Keys loaded, settings:', !!settings);
...
async function handleGetBalance(apiKeys: any) {
  console.log('🎯 GATE.IO WORKING: Getting balance');
...
// Размещение простого ордера (как раньше работало)
async function handlePlaceOrderWithTPSL(apiKeys: any, settings: any) {
  console.log('🎯 GATE.IO WORKING: Placing simple order');
  
  if (!settings) {
    console.log('❌ GATE.IO WORKING: No settings found');
    return new Response(
      JSON.stringify({
        success: false,
  ...
  console.log('🎯 GATE.IO WORKING: Contract:', contract);
  ...
  console.log('🎯 GATE.IO WORKING: Order params:', {
    contract,
    size,
    currentPrice,
    orderAmount,
  ...
  console.log('🎯 GATE.IO WORKING: Placing order with data:', orderData);
  ...
  const responseData = await response.json();
  console.log('🎯 GATE.IO WORKING: Order response:', responseData);
...
async function handleGetPositions(apiKeys: any) {
  console.log('🎯 GATE.IO WORKING: Getting positions');
  ...
  const responseData = await response.json();
  console.log('🎯 GATE.IO WORKING: Positions response:', responseData);
...
async function handleCancelAllOrders(apiKeys: any) {
  console.log('🎯 GATE.IO WORKING: Cancelling all orders');
  ...
  const responseData = await response.json();
  console.log('🎯 GATE.IO WORKING: Cancel orders response:', responseData);
...
async function handleCloseAllPositions(apiKeys: any) {
  console.log('🎯 GATE.IO WORKING: Closing all positions with reduce_only');
  ...
  console.log('🎯 GATE.IO WORKING: Found positions:', allPositions.length, 'open:', openPositions.length);
  ...
  for (const position of openPositions) {
    try {
      ...
      console.log(`🎯 GATE.IO WORKING: Closing position ${position.contract}, size: ${positionSize} -> ${closeSize}`);
      ...
      console.log('🎯 GATE.IO WORKING: Placing reduce_only order:', closeOrderData);
      ...
      const responseData = await response.json();
      console.log('🎯 GATE.IO WORKING: Close order response:', responseData);
