...
async function placeOrderWithTPSL(apiKey: any, settings: any) {
  try {
    ...
    if (apiKey.exchange === 'binance') {
      ...
      // Для TP используем TAKE_PROFIT_MARKET с правильными параметрами
      const tpQuery = `symbol=${symbol}&side=SELL&type=TAKE_PROFIT_MARKET&quantity=${quantity}&stopPrice=${tpPrice}&timeInForce=GTC&timestamp=${tpTimestamp}`;
