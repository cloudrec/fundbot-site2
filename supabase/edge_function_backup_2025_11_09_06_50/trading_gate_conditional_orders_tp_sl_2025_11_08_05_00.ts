...
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
    ...
    // 🔵 GATE.IO С ОБЫЧНЫМИ УСЛОВНЫМИ ОРДЕРАМИ
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing GATE.IO API v4 order with CONDITIONAL ORDERS');
      ...
      console.log('🔥 GATE.IO v4 CONDITIONAL: Base URL:', baseUrl);
      console.log('🔥 GATE.IO v4 CONDITIONAL: Symbol:', symbol);
      
      // Получаем текущую цену
      const priceUrl = `/futures/usdt/tickers?contract=${symbol}`;
      const fullPriceUrl = baseUrl + prefix + priceUrl;
      console.log('🔥 GATE.IO v4 CONDITIONAL: Price URL:', fullPriceUrl);
      
      const priceResponse = await fetch(fullPriceUrl);
      const priceData = await priceResponse.json();
      
      console.log('🔥 GATE.IO v4 CONDITIONAL: Price response status:', priceResponse.status);
      console.log('🔥 GATE.IO v4 CONDITIONAL: Price data:', priceData);
      ...
      console.log('🔥 GATE.IO v4 CONDITIONAL current price:', currentPrice);
      ...
      console.log('🔥 GATE.IO v4 CONDITIONAL order params:', { 
        symbol, 
        quantity, 
        currentPrice,
        tpPriceNumber,
      ...
      console.log('🔥 GATE.IO v4 CONDITIONAL main order data:', orderData);
      ...
      console.log('🔥 GATE.IO v4 CONDITIONAL order request URL:', fullOrderUrl);
      ...
      const data = await response.json();
      console.log('🔥 GATE.IO v4 CONDITIONAL order response status:', response.status);
      console.log('🔥 GATE.IO v4 CONDITIONAL order response:', data);

      if (response.status !== 201) {
        throw new Error(`GATE.IO v4 CONDITIONAL API Error: ${data.message || 'Unknown error'} (Code: ${response.status})`);
      }

      const orderId = data.id;
      console.log('🔥 GATE.IO v4 CONDITIONAL main order created:', orderId);

      // Задержка перед установкой условных ордеров
      console.log('🔥 Waiting 3 seconds before setting CONDITIONAL TP/SL...');
      await delay(3000);

      // 2. УСЛОВНЫЕ ОРДЕРА TP/SL через /futures/usdt/orders (НЕ price_orders!)
      let tpOrderId = null;
      let slOrderId = null;
      let tpStatus = 'NOT_SET';
      let slStatus = 'NOT_SET';
      ...
      try {
        ...
        console.log('🔥 GATE.IO v4 CONDITIONAL TP order data:', tpOrderData);
        ...
        const tpData = await tpResponse.json();
        console.log('🔥 GATE.IO v4 CONDITIONAL TP response status:', tpResponse.status);
        console.log('🔥 GATE.IO v4 CONDITIONAL TP response:', tpData);

        if (tpResponse.status === 201) {
          tpOrderId = tpData.id;
          tpStatus = 'SUCCESS';
          console.log('🔥 GATE.IO v4 CONDITIONAL TP order created:', tpOrderId);
        } else {
          tpStatus = 'ERROR';
          tpError = tpData.message || 'Unknown TP error';
          console.error('🔥 GATE.IO v4 CONDITIONAL TP error:', tpData.message);
        }
        ...
        console.log('🔥 GATE.IO v4 CONDITIONAL SL order data:', slOrderData);
        ...
        const slData = await slResponse.json();
        console.log('🔥 GATE.IO v4 CONDITIONAL SL response status:', slResponse.status);
        console.log('🔥 GATE.IO v4 CONDITIONAL SL response:', slData);

        if (slResponse.status === 201) {
          slOrderId = slData.id;
          slStatus = 'SUCCESS';
          console.log('🔥 GATE.IO v4 CONDITIONAL SL order created:', slOrderId);
        } else {
          slStatus = 'ERROR';
          slError = slData.message || 'Unknown SL error';
          console.error('🔥 GATE.IO v4 CONDITIONAL SL error:', slData.message);
        }

      } catch (tpslError) {
        console.error('🔥 GATE.IO v4 CONDITIONAL TP/SL error:', tpslError);
        tpStatus = 'ERROR';
        slStatus = 'ERROR';
        tpError = tpslError.message;
        slError = tpslError.message;
      ...
      const telegramMessage = `
<b>🚀 ОРДЕР РАЗМЕЩЕН</b>
<b>Биржа:</b> Gate.io (Условные ордера)
<b>Символ:</b> ${symbol}
<b>Сторона:</b> LONG
<b>Количество:</b> ${quantity}
<b>Цена входа:</b> $${currentPrice.toFixed(4)}
...
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
      ...
      return {
        ...
        side: "long",
        status: 'LIVE',
        message: `Боевой ордер GATE.IO v4 с условными TP/SL: ${orderId}`,
        quantity: quantity,
        price: currentPrice,
        ...
        api_version: 'GATE_V4_CONDITIONAL_ORDERS',
        base_url: baseUrl,
        signature_format: 'COMPLETE_DOCUMENTATION_BASED',
        tp_sl_method: 'CONDITIONAL_LIMIT_ORDERS',
        approach: 'MAIN_ORDER_PLUS_CONDITIONAL_TP_SL',
        note: 'TP/SL установлены как условные LIMIT ордера с reduce_only=true'
      };
    }
