...
async function placeOrderWithTPSL(apiKey: any, settings: any) {
  try {
    ...
    if (apiKey.exchange === 'binance') {
      console.log('🔥 Processing Binance order - ТОЛЬКО РЫНОЧНЫЙ ОРДЕР БЕЗ TP/SL');
      ...
      // 2. Размещаем ТОЛЬКО рыночный ордер (как первый шаг)
      const timestamp = Date.now();
      const side = 'BUY';
      const type = 'MARKET';
      
      // ИСПРАВЛЕНО: Только рыночный ордер без TP/SL параметров
      ...
      console.log('🔥 Binance MARKET order only params:', { symbol, side, type, quantity });
      ...
      console.log('🔥 Binance MARKET order response status:', orderResponse.status);
      console.log('🔥 Binance MARKET order response:', orderData);
      ...
      return {
        ...
        side: orderData.side,
        status: 'LIVE',
        message: `Боевой рыночный ордер Binance: ${orderData.orderId}`,
        quantity: quantity,
        price: currentPrice,
        tp_price: tpPrice,
        sl_price: slPrice,
        ...
        note: 'TP/SL не установлены - только рыночный ордер'
      };
    }
