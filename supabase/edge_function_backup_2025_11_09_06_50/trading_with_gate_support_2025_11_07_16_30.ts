...
// Функция создания подписи для Gate.io
...
async function getBalance(apiKey: any, settings: any) {
  try {
    ...
    // 🆕 НОВАЯ ПОДДЕРЖКА GATE.IO
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing Gate.io balance request');
...
async function placeOrderWithTPSL(apiKey: any, settings: any) {
  try {
    ...
    // 🆕 НОВАЯ ПОДДЕРЖКА GATE.IO
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing Gate.io order with TP/SL');
      ...
      return {
        ...
        side: "long",
        status: 'LIVE',
        message: `Боевой ордер Gate.io: ${data.id}`,
        quantity: quantity,
        price: currentPrice,
        tp_price: tpPrice,
        sl_price: slPrice,
