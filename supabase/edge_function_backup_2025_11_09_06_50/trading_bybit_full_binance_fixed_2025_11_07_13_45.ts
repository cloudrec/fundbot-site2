...
async function placeOrderWithTPSL(apiKey: any, settings: any) {
  try {
    ...
    if (apiKey.exchange === 'binance') {
      ...
      // 3. Рассчитываем цены TP/SL с правильной точностью
      ...
      // 4. Take Profit ордер - используем правильные параметры Binance Futures
      const tpTimestamp = Date.now();
      
      const tpQuery = `symbol=${symbol}&side=SELL&type=TAKE_PROFIT&quantity=${quantity}&price=${tpPrice}&stopPrice=${tpPrice}&timeInForce=GTC&workingType=CONTRACT_PRICE&timestamp=${tpTimestamp}`;
      ...
      // 5. Stop Loss ордер - используем правильные параметры
      const slTimestamp = Date.now();
      const slQuery = `symbol=${symbol}&side=SELL&type=STOP&quantity=${quantity}&price=${slPrice}&stopPrice=${slPrice}&timeInForce=GTC&workingType=CONTRACT_PRICE&timestamp=${slTimestamp}`;
    ...
    // ПОЛНЫЙ РАБОЧИЙ КОД BYBIT С TP/SL
    if (apiKey.exchange === 'bybit') {
      ...
      const leveragedValue = notionalValue * settings.leverage;
      const quantity = (leveragedValue / currentPrice).toFixed(symbolInfo.qtyStep);
      ...
      const tpPrice = (currentPrice * (1 + settings.long_tp_offset_percent / 100)).toFixed(symbolInfo.priceStep);
      const slPrice = (currentPrice * (1 - settings.long_stop_loss_percent / 100)).toFixed(symbolInfo.priceStep);
      
      console.log('🔥 TP/SL prices:', { tpPrice, slPrice, priceStep: symbolInfo.priceStep });
      ...
      const params = `category=linear&symbol=${symbol}&side=Buy&orderType=Market&qty=${quantity}&takeProfit=${tpPrice}&stopLoss=${slPrice}&tpTriggerBy=LastPrice&slTriggerBy=LastPrice&timeInForce=IOC&positionIdx=0&timestamp=${timestamp}`;
      const signature = await createBybitSignature(apiKey.api_secret, timestamp.toString(), apiKey.api_key, params);
    ...
    if (apiKey.exchange === 'bybit') {
      ...
      for (const position of positions) {
        try {
          ...
          const params = `category=linear&symbol=${position.symbol}&side=${side}&orderType=Market&qty=${Math.abs(size).toString()}&timeInForce=IOC&reduceOnly=true&positionIdx=0&timestamp=${timestamp}`;
          const signature = await createBybitSignature(apiKey.api_secret, timestamp.toString(), apiKey.api_key, params);
