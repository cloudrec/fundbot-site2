...
// Order duplication protection
const recentOrders = new Map();
const ORDER_COOLDOWN = 3000; // 3 seconds
...
serve(async (req) => {
  ...
  try {
    console.log('🔴 BYBIT FIXED SIGNATURE FUNCTION STARTED');
    ...
    switch (action) {
      ...
      case 'place_order_with_tp_sl':
        result = await placeBybitOrderWithTPSL(apiKeys, settings, user_id);
        break;
...
async function getBybitBalance(apiKeys: any, settings: any) {
  ...
  const { signature } = await createBybitSignatureGET(apiKeys.api_key, apiKeys.api_secret, timestamp, recvWindow, queryParams.toString());
...
async function placeBybitOrderWithTPSL(apiKeys: any, settings: any, user_id: string) {
  console.log('🔴 BYBIT: Placing order with TP/SL...');
  
  // Order duplication protection
  const orderKey = `${user_id}_bybit_order`;
  const now = Date.now();
  const lastOrder = recentOrders.get(orderKey) || 0;
  
  if (now - lastOrder < ORDER_COOLDOWN) {
    throw new Error(`Защита от дублирования: подождите ${Math.ceil((ORDER_COOLDOWN - (now - lastOrder)) / 1000)} секунд`);
  }
  
  recentOrders.set(orderKey, now);

  // Получаем текущую цену
  const currentPrice = await getCurrentBybitPrice(apiKeys, settings);
  
  // Рассчитываем количество
  const quantity = Math.floor(settings.order_amount_usd / currentPrice);
  
  // Рассчитываем TP/SL цены
  const tpPrice = (currentPrice * (1 + settings.short_tp_offset_percent / 100)).toFixed(4);
  const slPrice = (currentPrice * (1 - settings.short_stop_loss_percent / 100)).toFixed(4);
  
  console.log('🔴 BYBIT: Order params:', { quantity, currentPrice, tpPrice, slPrice });
  ...
  // ИСПРАВЛЕННАЯ ПОДПИСЬ ДЛЯ POST ЗАПРОСОВ
  const bodyString = JSON.stringify(orderParams);
  const { signature } = await createBybitSignaturePOST(apiKeys.api_key, apiKeys.api_secret, timestamp, recvWindow, bodyString);
  
  const baseUrl = apiKeys.is_testnet ? 'https://api-testnet.bybit.com' : 'https://api.bybit.com';
  
  console.log('🔴 BYBIT: Making POST request with body:', bodyString);
  ...
  return {
    ...
    side: 'BUY',
    status: 'LIVE',
    message: `РЕАЛЬНЫЙ BYBIT ордер с TP/SL: ${orderId}`,
    quantity: quantity.toString(),
    price: currentPrice,
    tp_price: tpPrice,
    sl_price: slPrice,
    exchange: 'BYBIT',
    note: 'BYBIT ОРДЕР РАЗМЕЩЕН! ⚡'
  };
}
...
async function getCurrentBybitPrice(apiKeys: any, settings: any) {
...
async function createBybitSignatureGET(apiKey: string, apiSecret: string, timestamp: string, recvWindow: string, queryString: string) {
...
async function createBybitSignaturePOST(apiKey: string, apiSecret: string, timestamp: string, recvWindow: string, bodyString: string) {
  ...
  console.log('🔴 BYBIT: Generated signature:', signatureHex);
