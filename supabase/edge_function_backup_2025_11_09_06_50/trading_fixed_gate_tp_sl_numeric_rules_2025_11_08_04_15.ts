...
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
    ...
    // 🔵 ИСПРАВЛЕННЫЙ GATE.IO TP/SL ПОДХОД
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing GATE.IO API v4 order with FIXED TP/SL APPROACH');
      ...
      console.log('🔥 GATE.IO v4 FIXED TP/SL: Base URL:', baseUrl);
      console.log('🔥 GATE.IO v4 FIXED TP/SL: Symbol:', symbol);
      
      // Получаем текущую цену
      const priceUrl = `/futures/usdt/tickers?contract=${symbol}`;
      const fullPriceUrl = baseUrl + prefix + priceUrl;
      console.log('🔥 GATE.IO v4 FIXED TP/SL: Price URL:', fullPriceUrl);
      
      const priceResponse = await fetch(fullPriceUrl);
      const priceData = await priceResponse.json();
      
      console.log('🔥 GATE.IO v4 FIXED TP/SL: Price response status:', priceResponse.status);
      console.log('🔥 GATE.IO v4 FIXED TP/SL: Price data:', priceData);
      ...
      console.log('🔥 GATE.IO v4 FIXED TP/SL current price:', currentPrice);
      ...
      console.log('🔥 GATE.IO v4 FIXED TP/SL order params:', { 
        symbol, 
        quantity, 
        currentPrice,
        tpPrice,
      ...
      console.log('🔥 GATE.IO v4 FIXED TP/SL main order data:', orderData);
      ...
      console.log('🔥 GATE.IO v4 FIXED TP/SL order request URL:', fullOrderUrl);
      ...
      const data = await response.json();
      console.log('🔥 GATE.IO v4 FIXED TP/SL order response status:', response.status);
      console.log('🔥 GATE.IO v4 FIXED TP/SL order response:', data);

      if (response.status !== 201) {
        throw new Error(`GATE.IO v4 FIXED TP/SL API Error: ${data.message || 'Unknown error'} (Code: ${response.status})`);
      }

      const orderId = data.id;
      console.log('🔥 GATE.IO v4 FIXED TP/SL main order created:', orderId);

      // Задержка перед установкой триггерных ордеров
      await delay(3000);

      // 2. ИСПРАВЛЕННЫЙ TAKE PROFIT TRIGGER ORDER
      let tpStatus = 'NOT_SET';
      let slStatus = 'NOT_SET';
      let tpOrderId = null;
      let slOrderId = null;
      ...
      try {
        // ИСПРАВЛЕННАЯ СТРУКТУРА TP TRIGGER ORDER
        const tpTriggerData = {
          initial: {
            contract: symbol,
          ...
          trigger: {
            ...
            rule: 1, // >= (greater than or equal) - ИСПРАВЛЕНО: используем число вместо строки
            trigger_price: tpPrice,
            expiration: 86400 // 24 часа
          }
        };

        console.log('🔥 GATE.IO v4 FIXED TP trigger data:', JSON.stringify(tpTriggerData, null, 2));
        ...
        console.log('🔥 GATE.IO v4 FIXED TP trigger response status:', tpResponse.status);
        console.log('🔥 GATE.IO v4 FIXED TP trigger response:', tpResult);

        if (tpResponse.status === 201) {
          tpStatus = 'SUCCESS';
          tpOrderId = tpResult.id;
          console.log('🔥 GATE.IO v4 FIXED TP trigger order created:', tpOrderId);
        } else {
          tpStatus = 'ERROR';
          tpError = tpResult.message || 'Unknown TP error';
          console.error('🔥 GATE.IO v4 FIXED TP trigger error:', tpResult.message);
        }

        // Задержка между TP и SL
        await delay(2000);

        // 3. ИСПРАВЛЕННЫЙ STOP LOSS TRIGGER ORDER
        const slTriggerData = {
          initial: {
            contract: symbol,
            size: -quantity, // Противоположный размер для закрытия
          ...
          trigger: {
            strategy_type: 0, // По цене
            price_type: 0, // Последняя цена
            rule: 2, // <= (less than or equal) - ИСПРАВЛЕНО: используем число вместо строки
            trigger_price: slPrice,
            expiration: 86400 // 24 часа
          }
        };

        console.log('🔥 GATE.IO v4 FIXED SL trigger data:', JSON.stringify(slTriggerData, null, 2));
        ...
        console.log('🔥 GATE.IO v4 FIXED SL trigger response status:', slResponse.status);
        console.log('🔥 GATE.IO v4 FIXED SL trigger response:', slResult);

        if (slResponse.status === 201) {
          slStatus = 'SUCCESS';
          slOrderId = slResult.id;
          console.log('🔥 GATE.IO v4 FIXED SL trigger order created:', slOrderId);
        } else {
          slStatus = 'ERROR';
          slError = slResult.message || 'Unknown SL error';
          console.error('🔥 GATE.IO v4 FIXED SL trigger error:', slResult.message);
        }

      } catch (triggerError) {
        console.error('🔥 GATE.IO v4 FIXED trigger orders error:', triggerError);
        tpStatus = 'ERROR';
        slStatus = 'ERROR';
      ...
      const telegramMessage = `
<b>🚀 ОРДЕР РАЗМЕЩЕН</b>
<b>Биржа:</b> Gate.io (Fixed Trigger Orders)
<b>Символ:</b> ${symbol}
<b>Сторона:</b> LONG
<b>Количество:</b> ${quantity}
<b>Цена входа:</b> $${currentPrice.toFixed(4)}
...
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
      ...
      return {
        ...
        side: "long",
        status: 'LIVE',
        message: `Боевой ордер GATE.IO v4 с Fixed Trigger Orders: ${orderId}`,
        quantity: quantity,
        price: currentPrice,
        tp_price: tpPrice,
        sl_price: slPrice,
        ...
        api_version: 'GATE_V4_FIXED_TRIGGER_ORDERS',
        base_url: baseUrl,
        signature_format: 'COMPLETE_DOCUMENTATION_BASED',
        tp_sl_method: 'FIXED_TRIGGER_ORDERS_APPROACH',
        trigger_orders_endpoint: '/futures/usdt/price_orders',
        fix_applied: 'NUMERIC_RULE_VALUES_INSTEAD_OF_STRINGS'
      };
    }
