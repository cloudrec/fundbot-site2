...
async function placeOrderWithTPSL(apiKey: any, settings: any) {
  try {
    ...
    if (apiKey.exchange === 'binance') {
      console.log('🔥 Processing Binance order with POSITION TP/SL - ПРАВИЛЬНЫЙ API');
      ...
      // 2. Размещаем основной рыночный ордер
      const timestamp = Date.now();
      const side = 'BUY';
      const type = 'MARKET';
      ...
      console.log('🔥 Waiting 2 seconds before placing position TP/SL...');
      await delay(2000);
      ...
      // 4. ПРАВИЛЬНЫЙ BINANCE API: Take Profit для позиции - TAKE_PROFIT_MARKET
      const tpTimestamp = Date.now();
      
      const tpQuery = `symbol=${symbol}&side=SELL&type=TAKE_PROFIT_MARKET&quantity=${quantity}&stopPrice=${tpPrice}&timeInForce=GTE_GTC&timestamp=${tpTimestamp}`;
      const tpSignature = await createBinanceSignature(apiKey.api_secret, tpQuery);
      
      console.log('🔥 TP position order query (TAKE_PROFIT_MARKET):', tpQuery);
      ...
      console.log('🔥 Binance TP position order response status:', tpResponse.status);
      console.log('🔥 Binance TP position order response:', tpData);

      await delay(1500);

      // 5. ПРАВИЛЬНЫЙ BINANCE API: Stop Loss для позиции - STOP_MARKET
      const slTimestamp = Date.now();
      const slQuery = `symbol=${symbol}&side=SELL&type=STOP_MARKET&quantity=${quantity}&stopPrice=${slPrice}&timeInForce=GTE_GTC&timestamp=${slTimestamp}`;
      const slSignature = await createBinanceSignature(apiKey.api_secret, slQuery);
      
      console.log('🔥 SL position order query (STOP_MARKET):', slQuery);
      ...
      console.log('🔥 Binance SL position order response status:', slResponse.status);
      console.log('🔥 Binance SL position order response:', slData);
      ...
      return {
        ...
        message: `Боевой ордер Binance с позиционными TP/SL: ${mainOrderData.orderId}`,
        quantity: quantity,
        price: currentPrice,
        tp_price: tpPrice,
        sl_price: slPrice,
        ...
        binance_version: 'POSITION_TP_SL_API'
      };
    }
