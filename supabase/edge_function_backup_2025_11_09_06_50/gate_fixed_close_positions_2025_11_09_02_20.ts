...
serve(async (req) => {
  ...
  try {
    ...
    console.log('🎯 GATE.IO FIXED CLOSE: Starting action:', action);
    ...
    console.log('🎯 GATE.IO FIXED CLOSE: Keys loaded, settings:', !!settings);
    ...
    console.error('❌ GATE.IO FIXED CLOSE Error:', error.message);
    return new Response(
...
async function handleGetBalance(apiKeys: any) {
  console.log('🎯 GATE.IO FIXED CLOSE: Getting balance');
...
async function handlePlaceOrderWithTPSL(apiKeys: any, settings: any) {
  console.log('🎯 GATE.IO FIXED CLOSE: Placing order with TP/SL');
  
  if (!settings) {
    console.log('❌ GATE.IO FIXED CLOSE: No settings found');
    return new Response(
      JSON.stringify({
        success: false,
  ...
  console.log('🎯 GATE.IO FIXED CLOSE: Contract:', contract);
  ...
  console.log('🎯 GATE.IO FIXED CLOSE: Order params:', {
    contract,
    size,
    currentPrice,
    orderAmount,
  ...
  try {
    ...
    if (!mainOrderResult.success) {
      return new Response(
        JSON.stringify({
          success: false,
          error: `Ошибка размещения основного ордера: ${mainOrderResult.error}`
        }),
        { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
    ...
    const tpOrderData = {
      contract: contract,
      size: -size, // Противоположное направление
      price: tpPrice.toFixed(4),
      tif: 'gtc',
      text: 't-skywork_tp_order'
    };
    ...
    return new Response(
      JSON.stringify({
        success: true,
        data: {
          message: `GATE.IO ордер с TP/SL размещен: ${contract}`,
    ...
    return new Response(
      JSON.stringify({
        success: false,
        error: `Ошибка размещения ордера с TP/SL: ${error.message}`
      }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    );
  }
...
async function placeOrder(apiKeys: any, orderData: any) {
  ...
  console.log('🎯 GATE.IO FIXED CLOSE: Placing order:', orderData);
  ...
  const responseData = await response.json();
  console.log('🎯 GATE.IO FIXED CLOSE: Order response:', responseData);
...
async function handleGetPositions(apiKeys: any) {
  console.log('🎯 GATE.IO FIXED CLOSE: Getting positions');
  ...
  const responseData = await response.json();
  console.log('🎯 GATE.IO FIXED CLOSE: Positions response:', responseData);

  if (response.status === 200) {
    // Фильтруем только открытые позиции
    const openPositions = responseData.filter((pos: any) => {
      const size = parseFloat(pos.size || 0);
      return size !== 0;
    });

    console.log('🎯 GATE.IO FIXED CLOSE: Open positions found:', openPositions.length);
...
async function handleCancelAllOrders(apiKeys: any) {
  console.log('🎯 GATE.IO FIXED CLOSE: Cancelling all orders');
  ...
  const responseData = await response.json();
  console.log('🎯 GATE.IO FIXED CLOSE: Cancel orders response:', responseData);
...
// Закрытие всех позиций (ИСПРАВЛЕНО)
async function handleCloseAllPositions(apiKeys: any) {
  console.log('🎯 GATE.IO FIXED CLOSE: Closing all positions');
  ...
  for (const position of openPositions) {
    try {
      ...
      // ✅ ИСПРАВЛЕНО: Используем рыночный ордер без цены
      const closeOrderData = {
        contract: position.contract,
        size: side,
        tif: 'ioc', // Immediate or Cancel
        text: 't-close_position'
        // НЕ указываем price для рыночного ордера
      };

      console.log('🎯 GATE.IO FIXED CLOSE: Closing position:', closeOrderData);

      // Используем специальную функцию для рыночных ордеров
      const closeResult = await placeMarketOrder(apiKeys, closeOrderData);
      
      results.push({
        contract: position.contract,
        status: closeResult.success ? 'closed' : 'error',
        order_id: closeResult.success ? closeResult.data.order_id : null,
        error: closeResult.success ? null : closeResult.error
      });
...
// ✅ НОВАЯ ФУНКЦИЯ: Размещение рыночного ордера для закрытия позиций
async function placeMarketOrder(apiKeys: any, orderData: any) {
  // Для рыночных ордеров Gate.io использует другой endpoint
  const method = 'POST';
  const url = '/api/v4/futures/usdt/orders';
  const queryString = '';
  
  // Для рыночного ордера не указываем price, но указываем type
  const marketOrderData = {
    ...orderData,
    // price не указываем для рыночного ордера
    // Gate.io автоматически определит что это рыночный ордер
  };
  
  const payloadString = JSON.stringify(marketOrderData);
  ...
  console.log('🎯 GATE.IO FIXED CLOSE: Placing market order:', marketOrderData);
  ...
  const responseData = await response.json();
  console.log('🎯 GATE.IO FIXED CLOSE: Market order response:', responseData);
