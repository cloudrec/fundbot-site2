...
serve(async (req) => {
  ...
  try {
    console.log('🔥 GATE TP/SL WITH RULE Function started');
  ...
  try {
    ...
    if (apiKey.exchange === 'gate') {
      ...
      const result = {
        ...
        result: data,
        debug_url: fullUrl,
        api_version: 'GATE_V4_WITH_RULE_TP_SL',
        base_url: baseUrl,
...
async function placeOrderWithTPSL(apiKey: any, settings: any) {
  try {
    ...
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing GATE.IO API v4 order WITH PROPER TP/SL PRICE ORDERS');
      ...
      try {
        ...
        console.log('🔥 GATE.IO v4: Creating TP Price Order with rule >=');
        ...
        console.log('🔥 GATE.IO v4: Creating SL Price Order with rule <=');
      ...
      return {
        ...
        side: "long",
        status: 'LIVE',
        message: `Боевой ордер GATE.IO v4 с правильными TP/SL: ${orderId}`,
        quantity: quantity,
        price: currentPrice,
        tp_price: tpPrice,
        sl_price: slPrice,
        ...
        api_version: 'GATE_V4_WITH_PROPER_PRICE_ORDERS_RULE',
        base_url: baseUrl,
        signature_format: 'COMPLETE_DOCUMENTATION_BASED',
