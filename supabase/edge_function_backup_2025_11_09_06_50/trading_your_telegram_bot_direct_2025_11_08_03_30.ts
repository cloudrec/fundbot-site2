  ...
  try {
    ...
    switch (action) {
      ...
      case 'scan_funding':
        ...
        result = await scanFundingWithDirectTelegram(settingsData);
        break;
...
// 💰 СКАНИРОВАНИЕ ФАНДИНГА С ПРЯМОЙ ОТПРАВКОЙ В ВАШ TELEGRAM
async function scanFundingWithDirectTelegram(settings: any) {
  try {
    console.log('💰 DIRECT FUNDING SCAN: Starting comprehensive scan');
    ...
    for (const exchange of exchanges) {
      try {
        console.log(`💰 DIRECT FUNDING SCAN: Scanning ${exchange.toUpperCase()}...`);
    ...
    console.log('💰 DIRECT FUNDING SCAN: Successful results:', successfulResults);
    ...
    try {
      if (successfulResults.length > 0) {
        ...
        console.log('💰 DIRECT FUNDING SCAN: Sending comprehensive Telegram message to YOUR bot');
        telegramResult = await sendToYourTelegram(telegramMessage, 'funding');
        console.log('💰 DIRECT FUNDING SCAN: Telegram result:', telegramResult);
...
<b>Время сканирования:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
      ...
      console.error('💰 DIRECT FUNDING SCAN: Telegram error:', telegramError);
    ...
    return {
      ...
      message: `Сканирование завершено: найдено ${successfulResults.length} фандингов, Telegram: ${telegramResult.success ? 'ОТПРАВЛЕНО В ВАШ БОТ' : 'ОШИБКА'}`,
      your_bot_token: YOUR_TELEGRAM_BOT_TOKEN.substring(0, 10) + '...',
      your_chat_id: YOUR_TELEGRAM_CHAT_ID
    };
    
  } catch (error: any) {
    console.error('💰 DIRECT FUNDING SCAN: Error:', error);
...
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
      ...
      console.error('💰 DIRECT FUNDING SCAN: Failed to send error notification:', telegramError);
    }
...
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
    ...
    if (apiKey.exchange === 'gate') {
      ...
      if (response.status !== 201) {
        ...
        // 📱 ОТПРАВЛЯЕМ В ВАШ TELEGRAM БОТ ДЛЯ ПРОСТОГО ОРДЕРА
        const telegramMessage = `
<b>🚀 ОРДЕР РАЗМЕЩЕН</b>
...
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
      ...
      // 📱 ОТПРАВЛЯЕМ В ВАШ TELEGRAM БОТ ДЛЯ УСПЕШНОГО ОРДЕРА
      const telegramMessage = `
<b>🚀 ОРДЕР РАЗМЕЩЕН</b>
<b>Биржа:</b> Gate.io (ИСПРАВЛЕННЫЙ t- префикс)
<b>Символ:</b> ${symbol}
