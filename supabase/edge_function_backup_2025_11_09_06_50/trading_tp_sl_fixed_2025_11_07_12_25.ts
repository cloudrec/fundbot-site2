...
async function placeOrderWithTPSL(apiKey: any, settings: any) {
  try {
    ...
    if (apiKey.exchange === 'binance') {
      ...
      // 3. Размещаем Take Profit ордер (ИСПРАВЛЕНО: используем правильный тип)
      ...
      // ИСПРАВЛЕНО: Используем LIMIT ордер вместо TAKE_PROFIT_MARKET
      ...
      console.log('🔥 TP order params:', { tpPrice, quantity, type: 'LIMIT' });
      ...
      // Задержка перед размещением SL
      console.log('🔥 Waiting 2 seconds before placing SL order...');
      await delay(2000);

      // 4. Размещаем Stop Loss ордер (ИСПРАВЛЕНО: используем правильный тип)
      ...
      // ИСПРАВЛЕНО: Используем STOP_MARKET с правильными параметрами
      ...
      console.log('🔥 SL order params:', { slPrice, quantity, type: 'STOP_MARKET' });
  ...
  try {
    ...
    if (apiKey.exchange === 'binance') {
      ...
      return {
        ...
        exchange: 'BINANCE',
        status: 'LIVE',
        message: `Отменено ${Array.isArray(data) ? data.length : 0} ордеров Binance`
      };
    }
