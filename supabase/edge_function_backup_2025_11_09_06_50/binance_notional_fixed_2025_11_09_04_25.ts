...
serve(async (req) => {
  ...
  try {
    ...
    console.log('🟡 BINANCE NOTIONAL: Starting action:', action);
    ...
    console.log('🟡 BINANCE NOTIONAL: Keys loaded, settings:', !!settings);
    ...
    console.error('❌ BINANCE NOTIONAL Error:', error.message);
    return new Response(
...
async function handleGetBalance(apiKeys: any) {
  console.log('🟡 BINANCE NOTIONAL: Getting balance');
...
async function handlePlaceOrder(apiKeys: any, settings: any) {
  console.log('🟡 BINANCE NOTIONAL: Placing order');
  ...
  // ✅ ИСПРАВЛЕНО: Используем quoteOrderQty вместо quantity
  // Это позволяет указать сумму в USDT напрямую
  console.log('🟡 BINANCE NOTIONAL: Order params:', { 
    symbol, 
  ...
  const data = await response.json();
  console.log('🟡 BINANCE NOTIONAL: Order response:', data);
  ...
  if (response.ok) {
    return new Response(
      JSON.stringify({
        success: true,
        data: {
          message: `Binance ордер размещен: ${symbol} на $${orderAmountUSD}`,
