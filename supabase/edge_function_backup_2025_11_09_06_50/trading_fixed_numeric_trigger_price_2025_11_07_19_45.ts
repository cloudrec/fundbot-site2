...
serve(async (req) => {
  ...
  try {
    console.log('🔥 FIXED TRIGGER_PRICE Gate.io API v4 Function started');
...
// 🆕 ИСПРАВЛЕННАЯ ФУНКЦИЯ СОЗДАНИЯ ПОДПИСИ ДЛЯ GATE.IO API V4 С ЧИСЛОВЫМИ TRIGGER_PRICE
async function createGateSignatureFixed(secret: string, method: string, url: string, queryString: string = '', payloadString: string = ''): Promise<{ signature: string, timestamp: string }> {
  try {
    // Используем текущее время в секундах (как в официальном примере)
    const timestamp = (Date.now() / 1000).toString();
    
    console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 signature inputs:');
    ...
    console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 hashed payload:', hashedPayload);
    
    // РАБОЧИЙ ФОРМАТ СТРОКИ ПОДПИСИ ПО ОФИЦИАЛЬНОМУ ПРИМЕРУ:
    // method + '\n' + url + '\n' + query_string + '\n' + hashed_payload + '\n' + timestamp
    const signatureString = `${method}\n${url}\n${queryString}\n${hashedPayload}\n${timestamp}`;
    
    console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 signature string:');
    ...
    console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 signature result:', result);
    return { signature: result, timestamp };
  } catch (error) {
    console.error('❌ Error creating FIXED TRIGGER_PRICE Gate.io v4 signature:', error);
    throw error;
  }
}
...
async function getBalance(apiKey: any, settings: any) {
  try {
    console.log('🔥 FIXED TRIGGER_PRICE getBalance started for exchange:', apiKey.exchange);
    ...
    // 🆕 ИСПРАВЛЕННАЯ GATE.IO API V4 РЕАЛИЗАЦИЯ С ЧИСЛОВЫМИ TRIGGER_PRICE
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing FIXED TRIGGER_PRICE Gate.io API v4 balance request');
      ...
      console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4: Base URL:', baseUrl);
      console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4: Prefix:', prefix);
      console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4: URL path:', url);
      console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4: Full URL:', baseUrl + prefix + url);
      
      // Создаем подпись по официальному примеру
      const { signature, timestamp } = await createGateSignatureFixed(apiKey.api_secret, 'GET', prefix + url, queryString, payloadString);
      
      const fullUrl = baseUrl + prefix + url;
      console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 balance request URL:', fullUrl);
      ...
      const data = await response.json();
      console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 balance response status:', response.status);
      console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 balance response:', data);

      if (response.status !== 200) {
        throw new Error(`FIXED TRIGGER_PRICE Gate.io v4 API Error: ${data.message || 'Unknown error'} (Code: ${response.status})`);
      }
      ...
      const result = {
        ...
        result: data,
        debug_url: fullUrl,
        api_version: 'FIXED_TRIGGER_PRICE_GATE_V4',
        base_url: baseUrl,
        signature_format: 'OFFICIAL_EXAMPLE_BASED'
      };
      
      console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 balance result:', result);
      return result;
    }
    ...
    console.error('🔥 FIXED TRIGGER_PRICE Balance error:', error);
...
async function placeOrderWithTPSL(apiKey: any, settings: any) {
  try {
    console.log('🔥 FIXED TRIGGER_PRICE placeOrderWithTPSL started for exchange:', apiKey.exchange);
    ...
    // 🆕 ИСПРАВЛЕННАЯ GATE.IO API V4 РЕАЛИЗАЦИЯ С ЧИСЛОВЫМИ TRIGGER_PRICE
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing FIXED TRIGGER_PRICE Gate.io API v4 order with TP/SL');
      ...
      console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 ORDER: Base URL:', baseUrl);
      console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 ORDER: Symbol:', symbol);
      
      // Получаем текущую цену
      const priceUrl = `/futures/usdt/tickers?contract=${symbol}`;
      const fullPriceUrl = baseUrl + prefix + priceUrl;
      console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 ORDER: Price URL:', fullPriceUrl);
      
      const priceResponse = await fetch(fullPriceUrl);
      const priceData = await priceResponse.json();
      
      console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 ORDER: Price response status:', priceResponse.status);
      console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 ORDER: Price data:', priceData);
      
      if (!priceData || priceData.length === 0) {
        throw new Error(`Не удалось получить цену для ${symbol} на FIXED TRIGGER_PRICE Gate.io v4`);
      }
      
      const currentPrice = parseFloat(priceData[0].last);
      console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 current price:', currentPrice);
      
      // Рассчитываем количество (Gate.io использует контракты)
      const calculatedQuantity = Math.floor(settings.order_amount_usd / currentPrice);
      const quantity = calculatedQuantity;
      
      // 🔧 ИСПРАВЛЕНО: ЧИСЛОВЫЕ ЗНАЧЕНИЯ ДЛЯ TP/SL (НЕ СТРОКИ!)
      ...
      console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 order params:', { 
        symbol, 
        quantity, 
        currentPrice, 
        tpPriceNumber, 
        slPriceNumber,
        tpPriceType: typeof tpPriceNumber,
        slPriceType: typeof slPriceNumber
      });
      ...
      console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 main order data:', orderData);
      
      const orderUrl = '/futures/usdt/orders';
      const payloadString = JSON.stringify(orderData);
      const { signature, timestamp } = await createGateSignatureFixed(apiKey.api_secret, 'POST', prefix + orderUrl, '', payloadString);
      
      const fullOrderUrl = baseUrl + prefix + orderUrl;
      console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 order request URL:', fullOrderUrl);
      ...
      const data = await response.json();
      console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 order response status:', response.status);
      console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 order response:', data);
      ...
      console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 main order created:', orderId);

      // 2. УСТАНАВЛИВАЕМ TP/SL ДЛЯ ПОЗИЦИИ С ЧИСЛОВЫМИ TRIGGER_PRICE
      let tpStatus = 'NOT_SET';
      let slStatus = 'NOT_SET';
      let tpError = null;
      let slError = null;
      ...
      try {
        ...
        // 🔧 ИСПРАВЛЕНО: УСТАНАВЛИВАЕМ TAKE PROFIT С ЧИСЛОВЫМ TRIGGER_PRICE
        const tpData = {
          contract: symbol,
          trigger: {
          ...
          trigger_price: tpPriceNumber // 🔧 ИСПРАВЛЕНО: ЧИСЛОВОЕ ЗНАЧЕНИЕ!
        };

        console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 TP data:', tpData);
        console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 TP trigger_price type:', typeof tpData.trigger_price);

        const tpPayload = JSON.stringify(tpData);
        const tpSig = await createGateSignatureFixed(apiKey.api_secret, 'POST', prefix + '/futures/usdt/price_orders', '', tpPayload);
        ...
        console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 TP response status:', tpResponse.status);
        console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 TP response:', tpResult);

        if (tpResponse.status === 201) {
          tpStatus = 'SUCCESS';
          console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 TP order created:', tpResult.id);
        } else {
          tpStatus = 'ERROR';
          tpError = tpResult.message || 'Unknown TP error';
          console.error('🔥 FIXED TRIGGER_PRICE Gate.io v4 TP error:', tpError);
        }

        // Задержка между TP и SL
        await delay(2000);

        // 🔧 ИСПРАВЛЕНО: УСТАНАВЛИВАЕМ STOP LOSS С ЧИСЛОВЫМ TRIGGER_PRICE
        const slData = {
          contract: symbol,
          trigger: {
            strategy_type: 0, // 0 = by_price
          ...
          trigger_price: slPriceNumber // 🔧 ИСПРАВЛЕНО: ЧИСЛОВОЕ ЗНАЧЕНИЕ!
        };

        console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 SL data:', slData);
        console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 SL trigger_price type:', typeof slData.trigger_price);

        const slPayload = JSON.stringify(slData);
        const slSig = await createGateSignatureFixed(apiKey.api_secret, 'POST', prefix + '/futures/usdt/price_orders', '', slPayload);
        ...
        console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 SL response status:', slResponse.status);
        console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 SL response:', slResult);

        if (slResponse.status === 201) {
          slStatus = 'SUCCESS';
          console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 SL order created:', slResult.id);
        } else {
          slStatus = 'ERROR';
          slError = slResult.message || 'Unknown SL error';
          console.error('🔥 FIXED TRIGGER_PRICE Gate.io v4 SL error:', slError);
        }

      } catch (tpslError) {
        console.error('🔥 FIXED TRIGGER_PRICE Gate.io v4 TP/SL error:', tpslError);
        tpStatus = 'ERROR';
        slStatus = 'ERROR';
        tpError = tpslError.message;
        slError = tpslError.message;
      ...
      return {
        ...
        side: "long",
        status: 'LIVE',
        message: `Боевой ордер FIXED TRIGGER_PRICE Gate.io v4 с TP/SL: ${orderId}`,
        quantity: quantity,
        price: currentPrice,
        ...
        sl_error: slError,
        exchange: 'GATE',
        api_url: 'FIXED_TRIGGER_PRICE_gateio.ws',
        debug_url: fullOrderUrl,
        api_version: 'FIXED_TRIGGER_PRICE_GATE_V4_WITH_TP_SL',
        base_url: baseUrl,
        signature_format: 'OFFICIAL_EXAMPLE_BASED',
        text_field_fix: 'APPLIED',
        tp_sl_implementation: 'PRICE_ORDERS_API_WITH_NUMERIC_TRIGGER_PRICE',
        trigger_price_fix: 'NUMERIC_VALUES_APPLIED'
      };
    }
    ...
    console.error('🔥 FIXED TRIGGER_PRICE Set TP/SL error:', error);
    throw error;
  }
}
...
async function getPositions(apiKey: any, settings: any) {
  try {
    console.log('🔥 FIXED TRIGGER_PRICE getPositions started for exchange:', apiKey.exchange);
    
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing FIXED TRIGGER_PRICE Gate.io API v4 positions request');
      ...
      console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 positions URL:', baseUrl + prefix + url);
      ...
      const data = await response.json();
      console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 positions response status:', response.status);
      console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 positions response:', data);

      if (response.status !== 200) {
        throw new Error(`FIXED TRIGGER_PRICE Gate.io v4 Positions API Error: ${data.message || 'Unknown error'} (Code: ${response.status})`);
      }

      // Фильтруем только открытые позиции
      const openPositions = data.filter((position: any) => parseFloat(position.size) !== 0);
      
      console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 open positions:', openPositions);
    ...
    console.error('🔥 FIXED TRIGGER_PRICE Positions error:', error);
...
async function cancelAllOrders(apiKey: any, settings: any) {
  try {
    console.log('🔥 FIXED TRIGGER_PRICE cancelAllOrders started for exchange:', apiKey.exchange);
    
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing FIXED TRIGGER_PRICE Gate.io API v4 cancel orders request');
      ...
      const { signature: ordersSig, timestamp: ordersTs } = await createGateSignatureFixed(apiKey.api_secret, 'DELETE', prefix + ordersUrl, '', '');
      
      console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 cancel orders URL:', baseUrl + prefix + ordersUrl);
      ...
      console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 cancel orders response status:', ordersResponse.status);
      console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 cancel orders response:', ordersData);
      ...
      const { signature: priceSig, timestamp: priceTs } = await createGateSignatureFixed(apiKey.api_secret, 'DELETE', prefix + priceOrdersUrl, '', '');
      
      console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 cancel price orders URL:', baseUrl + prefix + priceOrdersUrl);
      ...
      const priceData = await priceResponse.json();
      console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 cancel price orders response status:', priceResponse.status);
      console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 cancel price orders response:', priceData);
      ...
      return {
        ...
        exchange: 'GATE',
        status: 'LIVE',
        message: `Отмена ордеров FIXED TRIGGER_PRICE Gate.io v4: ${cancelledOrders} обычных + ${cancelledPriceOrders} TP/SL`,
        api_version: 'FIXED_TRIGGER_PRICE_GATE_V4',
        symbol: symbol
      };
    }
    ...
    console.error('🔥 FIXED TRIGGER_PRICE Cancel orders error:', error);
...
async function closeAllPositions(apiKey: any, settings: any) {
  try {
    console.log('🔥 FIXED TRIGGER_PRICE closeAllPositions started for exchange:', apiKey.exchange);
    
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing FIXED TRIGGER_PRICE Gate.io API v4 close positions request');
      ...
      const { signature: posSig, timestamp: posTs } = await createGateSignatureFixed(apiKey.api_secret, 'GET', prefix + positionsUrl, '', '');
      
      console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 get positions URL:', baseUrl + prefix + positionsUrl);
      ...
      console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 positions response status:', positionsResponse.status);
      console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 positions response:', positionsData);

      if (positionsResponse.status !== 200) {
        throw new Error(`FIXED TRIGGER_PRICE Gate.io v4 Positions API Error: ${positionsData.message || 'Unknown error'} (Code: ${positionsResponse.status})`);
      }
      ...
      console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 open positions for', symbol, ':', openPositions);
      ...
      for (const position of openPositions) {
        try {
          ...
          console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 closing position:', { contract: position.contract, size: positionSize, closeSize });
          ...
          const { signature: closeSig, timestamp: closeTs } = await createGateSignatureFixed(apiKey.api_secret, 'POST', prefix + '/futures/usdt/orders', '', closePayload);
          ...
          console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 close position response status:', closeResponse.status);
          console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 close position response:', closeData);
          ...
          if (closeResponse.status === 201) {
            ...
            console.log('🔥 FIXED TRIGGER_PRICE Gate.io v4 position closed successfully:', closeData.id);
          } else {
            closeResults.push({
              contract: position.contract,
              status: 'ERROR',
            ...
            console.error('🔥 FIXED TRIGGER_PRICE Gate.io v4 close position error:', closeData.message);
          }
          ...
          console.error('🔥 FIXED TRIGGER_PRICE Gate.io v4 close position error:', closeError);
          closeResults.push({
            contract: position.contract,
            status: 'ERROR',
      ...
      return {
        ...
        exchange: 'GATE',
        status: 'LIVE',
        message: `Закрытие позиций FIXED TRIGGER_PRICE Gate.io v4: ${closedPositions}/${openPositions.length} успешно`,
        api_version: 'FIXED_TRIGGER_PRICE_GATE_V4',
        symbol: symbol
      };
    }
    ...
    console.error('🔥 FIXED TRIGGER_PRICE Close positions error:', error);
