...
serve(async (req) => {
  ...
  try {
    console.log('🔧 FIXED Gate.io Function started');
    ...
    if (action === 'scan_funding') {
      ...
      // 🔧 ПРАВИЛЬНО: Получаем API ключи из api_keys_dev с правильной структурой
      console.log('🔧 Getting API keys from api_keys_dev table for exchange:', settings.exchange);
...
async function scanFunding(supabase: any) {
  ...
  try {
    ...
    return {
      message: 'FIXED: Ошибка сканирования фандинга',
      opportunities: [],
...
// 🔧 ИСПРАВЛЕННАЯ Gate.io баланс с правильной подписью
async function getGateBalance(apiKeys: any) {
  console.log('🔧 Getting Gate.io balance with FIXED signature');
  ...
  console.log('🔧 Gate.io API details:', {
    baseUrl,
    path,
  ...
  // 🔧 ИСПРАВЛЕННАЯ подпись для Gate.io
  const { signature, timestamp } = await createGateSignatureFixed(
    apiKeys.api_secret, 'GET', path, '', ''
  );
  
  console.log('🔧 Gate.io signature created:', {
  ...
  const data = await response.json();
  console.log('🔧 Gate.io balance data received:', data);
...
async function placeTestOrder(apiKeys: any, settings: any) {
  ...
  return {
    message: 'Test order simulation - FIXED',
    exchange: settings.exchange,
...
async function placeBinanceOrderWithTPSL(apiKeys: any, settings: any) {
  ...
  return {
    ...
    message: `FIXED: Binance ордер размещен: ${orderData.orderId}`,
...
async function placeBybitOrderWithTPSL(apiKeys: any, settings: any) {
  ...
  return {
    ...
    side: "Buy",
    status: 'NEW',
    message: `FIXED: Bybit ордер с TP/SL размещен: ${result.result.orderId}`,
    quantity: quantity,
    price: currentPrice,
    tp_price: tpPrice,
    sl_price: slPrice,
...
async function placeGateOrderWithTPSL(apiKeys: any, settings: any) {
  ...
  return {
    ...
    message: `FIXED: Gate.io ордер размещен (без TP/SL): ${orderResult.id}`,
    quantity: quantity.toString(),
    price: currentPrice,
    tp_price: tpPrice,
    sl_price: slPrice,
...
async function cancelAllOrders(apiKeys: any, settings: any) {
  ...
  return {
    message: `FIXED: Отмена ордеров на ${settings.exchange}`,
    exchange: settings.exchange
  };
}
...
async function closeAllPositions(apiKeys: any, settings: any) {
  ...
  return {
    message: `FIXED: Закрытие позиций на ${settings.exchange}`,
    exchange: settings.exchange
  };
}
...
// 🔧 ИСПРАВЛЕННАЯ подпись для Gate.io
async function createGateSignatureFixed(secret: string, method: string, path: string, query: string, body: string) {
  console.log('🔧 Creating FIXED Gate.io signature');
  
  // 🔧 КРИТИЧЕСКИ ВАЖНО: правильный формат timestamp для Gate.io
  const timestamp = Math.floor(Date.now() / 1000).toString();
  
  // 🔧 КРИТИЧЕСКИ ВАЖНО: правильный порядок и формат сообщения для Gate.io
  const message = `${method}\n${path}\n${query}\n${body}\n${timestamp}`;
  
  console.log('🔧 Gate.io signature message:', {
    method,
    path,
    query,
    body,
  ...
  // 🔧 КРИТИЧЕСКИ ВАЖНО: Gate.io использует SHA-512, не SHA-256
  const cryptoKey = await crypto.subtle.importKey(
  ...
  console.log('🔧 Gate.io FIXED signature created:', {
    signature_length: result.length,
    timestamp,
    signature_preview: result.substring(0, 16) + '...'
  });
