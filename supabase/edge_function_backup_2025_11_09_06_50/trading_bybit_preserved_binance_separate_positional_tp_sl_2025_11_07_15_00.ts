...
async function placeOrderWithTPSL(apiKey: any, settings: any) {
  try {
    ...
    if (apiKey.exchange === 'binance') {
      console.log('🔥 Processing Binance order with SEPARATE POSITIONAL TP/SL orders');
      ...
      // 4. ПОЗИЦИОННЫЕ TP/SL ОРДЕРА (отдельно)
      let tpOrderId = null;
      let slOrderId = null;
      ...
      try {
        ...
        console.log('🔥 TP order query (TAKE_PROFIT_MARKET):', tpQuery);
        ...
        console.log('🔥 SL order query (STOP_MARKET):', slQuery);
      ...
      return {
        ...
        binance_version: 'SEPARATE_POSITIONAL_TP_SL'
      };
    }
