...
serve(async (req) => {
  ...
  try {
    ...
    console.log('🟢 GATE.IO FIXED ACTION:', action, 'for user:', user_id);
    ...
    console.log('🔑 GATE.IO: Поиск по UUID result:', { found: !!apiKeys, error: keysError?.message });

    // Если не нашли по UUID, пробуем по email
    if (keysError || !apiKeys) {
      console.log('🔍 GATE.IO: Не нашли по UUID, пробуем по email...');
      
      // Получаем email пользователя
      const { data: userData } = await supabase.auth.admin.getUserById(user_id);
      const userEmail = userData.user?.email;
      
      console.log('📧 GATE.IO: User email:', userEmail);
      
      if (userEmail) {
        const { data: apiKeysByEmail, error: emailKeysError } = await supabase
          .from('api_keys')
          .select('*')
        ...
        console.log('🔑 GATE.IO: Поиск по email result:', { found: !!apiKeysByEmail, error: emailKeysError?.message });
        
        if (!emailKeysError && apiKeysByEmail) {
          console.log('✅ GATE.IO: Нашли API ключи по email!');
          apiKeys = apiKeysByEmail;
          keysError = null;
        }
      }
    }
    ...
    if (keysError || !apiKeys) {
      console.log('🚫 GATE.IO: API ключи не найдены ни по UUID, ни по email');
    ...
    console.log('🟢 GATE.IO: API ключи найдены, длина:', {
      key_length: gateApiKey?.length || 0,
      secret_length: gateApiSecret?.length || 0,
      key_preview: gateApiKey?.substring(0, 10) + '...',
      secret_preview: gateApiSecret?.substring(0, 10) + '...'
    });
