...
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
    ...
    // 🔵 GATE.IO IMPLEMENTATION - ПРОСТЫЕ ОРДЕРА БЕЗ СЛОЖНЫХ TP/SL
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing GATE.IO API v4 order - SIMPLE VERSION WITHOUT COMPLEX TP/SL');
      ...
      // 🔥 ПРОСТОЙ MARKET ОРДЕР БЕЗ СЛОЖНЫХ ПАРАМЕТРОВ
      const orderData = {
        contract: symbol,
        size: quantity,
      ...
      console.log('🔥 GATE.IO v4 simple order data:', orderData);
      ...
      console.log('🔥 GATE.IO v4 simple order created:', orderId);
...
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
      ...
      return {
        ...
        side: "long",
        status: 'LIVE',
        message: `Боевой ордер GATE.IO v4 (простой): ${orderId}`,
        quantity: quantity,
        price: currentPrice,
        tp_price: tpPrice,
        sl_price: slPrice,
        ...
        api_version: 'GATE_V4_SIMPLE_NO_COMPLEX_TP_SL',
        base_url: baseUrl,
        signature_format: 'COMPLETE_DOCUMENTATION_BASED',
        text_field_fix: 'CORRECT_T_PREFIX_APPLIED',
        tp_sl_method: 'MANUAL_SET_REQUIRED'
      };
    }
