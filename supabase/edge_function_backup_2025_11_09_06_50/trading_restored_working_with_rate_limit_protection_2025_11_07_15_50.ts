...
async function placeOrderWithTPSL(apiKey: any, settings: any) {
  try {
    ...
    if (apiKey.exchange === 'binance') {
      console.log('🔥 Processing Binance order with RESTORED WORKING TP/SL logic');
      ...
      console.log('🔥 Waiting 8 seconds before setting TP/SL (rate limit protection)...');
      await delay(8000);

      // 4. ВОССТАНОВЛЕННЫЕ РАБОЧИЕ TP/SL ОРДЕРА
      let tpOrderId = null;
      let slOrderId = null;
      ...
      try {
        // Take Profit ордер (LIMIT с reduceOnly)
        const tpTimestamp = Date.now();
        const tpQuery = `symbol=${symbol}&side=SELL&type=LIMIT&quantity=${quantity}&price=${tpPrice}&timeInForce=GTC&reduceOnly=true&timestamp=${tpTimestamp}`;
        const tpSignature = await createBinanceSignature(apiKey.api_secret, tpQuery);
        
        console.log('🔥 TP order query (LIMIT with reduceOnly):', tpQuery);
        ...
        if (tpResponse.status === 200) {
          ...
          tpStatus = 'ERROR';
          tpError = tpData.msg;
          console.error('🔥 TP order error:', tpData.msg);
        }

        // Задержка между TP и SL (увеличенная для rate limit)
        await delay(8000);

        // Stop Loss ордер (STOP_MARKET с reduceOnly)
        const slTimestamp = Date.now();
        const slQuery = `symbol=${symbol}&side=SELL&type=STOP_MARKET&quantity=${quantity}&stopPrice=${slPrice}&reduceOnly=true&timestamp=${slTimestamp}`;
        const slSignature = await createBinanceSignature(apiKey.api_secret, slQuery);
        
        console.log('🔥 SL order query (STOP_MARKET with reduceOnly):', slQuery);
        ...
        if (slResponse.status === 200) {
          ...
          slStatus = 'ERROR';
          slError = slData.msg;
          console.error('🔥 SL order error:', slData.msg);
        }

      } catch (tpslError) {
        console.error('🔥 TP/SL orders error:', tpslError);
        tpStatus = 'ERROR';
        slStatus = 'ERROR';
      ...
      return {
        ...
        message: `Боевой ордер Binance с восстановленными TP/SL: ${orderData.orderId}`,
        quantity: quantity,
        price: currentPrice,
        tp_price: tpPrice,
        sl_price: slPrice,
        ...
        binance_version: 'RESTORED_WORKING_TP_SL_WITH_RATE_LIMIT_PROTECTION'
      };
    }
