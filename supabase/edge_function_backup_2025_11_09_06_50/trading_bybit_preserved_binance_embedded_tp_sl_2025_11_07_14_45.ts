...
async function placeOrderWithTPSL(apiKey: any, settings: any) {
  try {
    ...
    if (apiKey.exchange === 'binance') {
      console.log('🔥 Processing Binance order with EMBEDDED TP/SL PARAMETERS');
      ...
      // 3. ПРАВИЛЬНЫЙ BINANCE API: Рыночный ордер с ВСТРОЕННЫМИ TP/SL параметрами
      const timestamp = Date.now();
      const side = 'BUY';
      const type = 'MARKET';
      
      // ИСПРАВЛЕНО: TP/SL как параметры в ордере
      const orderParams = new URLSearchParams({
        symbol: symbol,
        side: side,
        type: type,
        quantity: quantity,
        stopPrice: slPrice,           // Stop Loss цена
        priceProtect: 'true',         // Защита от плохих цен
        workingType: 'CONTRACT_PRICE', // Тип цены для TP/SL
        timestamp: timestamp.toString()
      });
      
      // Добавляем Take Profit если поддерживается
      orderParams.append('takeProfitPrice', tpPrice);
      
      const orderQuery = orderParams.toString();
      const orderSignature = await createBinanceSignature(apiKey.api_secret, orderQuery);
      
      console.log('🔥 Binance order with EMBEDDED TP/SL params:', { 
      ...
      console.log('🔥 Binance EMBEDDED TP/SL order response status:', orderResponse.status);
      console.log('🔥 Binance EMBEDDED TP/SL order response:', orderData);

      if (orderResponse.status !== 200) {
        // Если не поддерживается встроенный TP/SL, пробуем без него
        if (orderData.code === -1102 || orderData.msg?.includes('takeProfitPrice')) {
          console.log('🔥 Embedded TP/SL not supported, trying basic order');
          
          const basicOrderParams = new URLSearchParams({
            symbol: symbol,
            side: side,
            type: type,
            quantity: quantity,
          ...
          const basicOrderQuery = basicOrderParams.toString();
          const basicOrderSignature = await createBinanceSignature(apiKey.api_secret, basicOrderQuery);
          
          const basicOrderResponse = await fetch(`${baseUrl}/fapi/v1/order`, {
            method: 'POST',
            headers: {
              'X-MBX-APIKEY': apiKey.api_key,
              'Content-Type': 'application/x-www-form-urlencoded'
            },
            body: `${basicOrderQuery}&signature=${basicOrderSignature}`
          });

          const basicOrderData = await basicOrderResponse.json();
          console.log('🔥 Binance basic order response:', basicOrderData);

          if (basicOrderResponse.status !== 200) {
            throw new Error(`Binance Order Error: ${basicOrderData.msg || 'Unknown error'} (Code: ${basicOrderData.code || basicOrderResponse.status})`);
          }

          return {
            order_id: basicOrderData.orderId,
            symbol: basicOrderData.symbol,
            side: basicOrderData.side,
            status: 'LIVE',
            message: `Боевой ордер Binance (без TP/SL): ${basicOrderData.orderId}`,
            quantity: quantity,
            price: currentPrice,
            tp_price: tpPrice,
            sl_price: slPrice,
            tp_status: 'NOT_SUPPORTED',
            sl_status: 'NOT_SUPPORTED',
            price_source: 'CACHED',
            order_type: 'MARKET_ONLY',
            duplicate_protection: 'ENABLED',
            binance_version: 'BASIC_ORDER_NO_TP_SL'
          };
        }
      ...
      return {
        ...
        side: orderData.side,
        status: 'LIVE',
        message: `Боевой ордер Binance с встроенными TP/SL: ${orderData.orderId}`,
        quantity: quantity,
        price: currentPrice,
        tp_price: tpPrice,
        sl_price: slPrice,
        ...
        sl_status: 'EMBEDDED',
        price_source: 'CACHED',
        order_type: 'MARKET_WITH_EMBEDDED_TP_SL',
        duplicate_protection: 'ENABLED',
        binance_version: 'EMBEDDED_TP_SL_PARAMETERS'
      };
    }
