...
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
    ...
    // 🔵 GATE.IO С PRICE TRIGGER API (ИСПРАВЛЕННЫЕ ЧИСЛОВЫЕ ПРАВИЛА)
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing GATE.IO API v4 order with PRICE TRIGGER API (NUMERIC RULES FIXED)');
      ...
      console.log('🔥 GATE.IO v4 NUMERIC RULES: Base URL:', baseUrl);
      console.log('🔥 GATE.IO v4 NUMERIC RULES: Symbol:', symbol);
      
      // Получаем текущую цену
      const priceUrl = `/futures/usdt/tickers?contract=${symbol}`;
      const fullPriceUrl = baseUrl + prefix + priceUrl;
      console.log('🔥 GATE.IO v4 NUMERIC RULES: Price URL:', fullPriceUrl);
      
      const priceResponse = await fetch(fullPriceUrl);
      const priceData = await priceResponse.json();
      
      console.log('🔥 GATE.IO v4 NUMERIC RULES: Price response status:', priceResponse.status);
      console.log('🔥 GATE.IO v4 NUMERIC RULES: Price data:', priceData);
      ...
      console.log('🔥 GATE.IO v4 NUMERIC RULES current price:', currentPrice);
      ...
      console.log('🔥 GATE.IO v4 NUMERIC RULES order params:', { 
        symbol, 
        quantity, 
        currentPrice,
        tpPriceNumber,
      ...
      console.log('🔥 GATE.IO v4 NUMERIC RULES main order data:', orderData);
      ...
      console.log('🔥 GATE.IO v4 NUMERIC RULES order request URL:', fullOrderUrl);
      ...
      const data = await response.json();
      console.log('🔥 GATE.IO v4 NUMERIC RULES order response status:', response.status);
      console.log('🔥 GATE.IO v4 NUMERIC RULES order response:', data);

      if (response.status !== 201) {
        throw new Error(`GATE.IO v4 NUMERIC RULES API Error: ${data.message || 'Unknown error'} (Code: ${response.status})`);
      }

      const orderId = data.id;
      console.log('🔥 GATE.IO v4 NUMERIC RULES main order created:', orderId);

      // Задержка перед установкой TP/SL
      console.log('🔥 Waiting 10 seconds before setting NUMERIC RULES PRICE TRIGGER TP/SL...');
      await delay(10000);

      // 2. PRICE TRIGGER API С ЧИСЛОВЫМИ ПРАВИЛАМИ (ИСПРАВЛЕНО!)
      let tpOrderId = null;
      let slOrderId = null;
      let tpStatus = 'NOT_SET';
      let slStatus = 'NOT_SET';
      ...
      try {
        // TAKE PROFIT ЧЕРЕЗ PRICE TRIGGER API С ЧИСЛОВЫМ ПРАВИЛОМ
        const tpTriggerData = {
          trigger: {
            price: tpPriceString,        // Цена активации
            rule: 1,                     // ✅ ИСПРАВЛЕНО: 1 = gte (больше или равно)
            expiration: 86400            // Срок действия в секундах (24 часа)
          },
          put: {
            type: "limit",               // Тип ордера после активации
            side: "short",               // Сторона (short для закрытия long позиции)
            contract: symbol,            // Контракт
            size: quantity,              // Размер
            price: tpPriceString,        // Цена исполнения
            tif: "gtc",                  // Time in force
            text: `t-tp-trigger-${Date.now().toString().slice(-6)}`,
            reduce_only: true            // Только для закрытия позиции
          }
        };

        console.log('🔥 GATE.IO v4 NUMERIC RULES TP data (rule=1 for gte):', tpTriggerData);

        const tpTriggerUrl = '/futures/usdt/price_orders';
        const tpPayload = JSON.stringify(tpTriggerData);
        const { signature: tpSig, timestamp: tpTs } = await createCompleteGateSignature(apiKey.api_secret, 'POST', prefix + tpTriggerUrl, '', tpPayload);

        const tpResponse = await fetch(baseUrl + prefix + tpTriggerUrl, {
          method: 'POST',
          headers: {
            'KEY': apiKey.api_key,
            'SIGN': tpSig,
        ...
        const tpData = await tpResponse.json();
        console.log('🔥 GATE.IO v4 NUMERIC RULES TP response status:', tpResponse.status);
        console.log('🔥 GATE.IO v4 NUMERIC RULES TP response:', tpData);

        if (tpResponse.status === 201) {
          tpOrderId = tpData.id;
          tpStatus = 'SUCCESS';
          console.log('🔥 GATE.IO v4 NUMERIC RULES TP order created:', tpOrderId);
        } else {
          tpStatus = 'ERROR';
          tpError = tpData.message || 'Unknown TP error';
          console.error('🔥 GATE.IO v4 NUMERIC RULES TP error:', tpData.message);
        }

        // Задержка между TP и SL
        await delay(5000);

        // STOP LOSS ЧЕРЕЗ PRICE TRIGGER API С ЧИСЛОВЫМ ПРАВИЛОМ
        const slTriggerData = {
          trigger: {
            price: slPriceString,        // Цена активации
            rule: 2,                     // ✅ ИСПРАВЛЕНО: 2 = lte (меньше или равно)
            expiration: 86400            // Срок действия в секундах (24 часа)
          },
          put: {
            type: "limit",               // Тип ордера после активации
            ...
            contract: symbol,            // Контракт
            size: quantity,              // Размер
            price: slPriceString,        // Цена исполнения
            tif: "gtc",                  // Time in force
            text: `t-sl-trigger-${Date.now().toString().slice(-6)}`,
            reduce_only: true            // Только для закрытия позиции
          }
        };

        console.log('🔥 GATE.IO v4 NUMERIC RULES SL data (rule=2 for lte):', slTriggerData);

        const slPayload = JSON.stringify(slTriggerData);
        const { signature: slSig, timestamp: slTs } = await createCompleteGateSignature(apiKey.api_secret, 'POST', prefix + tpTriggerUrl, '', slPayload);

        const slResponse = await fetch(baseUrl + prefix + tpTriggerUrl, {
          method: 'POST',
          headers: {
            'KEY': apiKey.api_key,
            'SIGN': slSig,
        ...
        const slData = await slResponse.json();
        console.log('🔥 GATE.IO v4 NUMERIC RULES SL response status:', slResponse.status);
        console.log('🔥 GATE.IO v4 NUMERIC RULES SL response:', slData);

        if (slResponse.status === 201) {
          slOrderId = slData.id;
          slStatus = 'SUCCESS';
          console.log('🔥 GATE.IO v4 NUMERIC RULES SL order created:', slOrderId);
        } else {
          slStatus = 'ERROR';
          slError = slData.message || 'Unknown SL error';
          console.error('🔥 GATE.IO v4 NUMERIC RULES SL error:', slData.message);
        }

      } catch (tpslError) {
        console.error('🔥 GATE.IO v4 NUMERIC RULES TP/SL error:', tpslError);
        tpStatus = 'ERROR';
        slStatus = 'ERROR';
        tpError = tpslError.message;
        slError = tpslError.message;
      ...
      const telegramMessage = `
<b>🚀 ОРДЕР РАЗМЕЩЕН</b>
<b>Биржа:</b> Gate.io (Numeric Rules Fixed)
<b>Символ:</b> ${symbol}
<b>Сторона:</b> LONG
<b>Количество:</b> ${quantity}
<b>Цена входа:</b> $${currentPrice.toFixed(4)}
...
<b>TP Trigger ID:</b> ${tpOrderId || 'Не установлен'}
<b>SL Trigger ID:</b> ${slOrderId || 'Не установлен'}
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
      `.trim();
      ...
      return {
        ...
        side: "long",
        status: 'LIVE',
        message: `Боевой ордер GATE.IO v4 с Numeric Rules Price Trigger TP/SL: ${orderId}`,
        quantity: quantity,
        price: currentPrice,
        ...
        api_version: 'GATE_V4_PRICE_TRIGGER_NUMERIC_RULES_FIXED',
        base_url: baseUrl,
        signature_format: 'COMPLETE_DOCUMENTATION_BASED',
        tp_sl_method: 'PRICE_TRIGGER_NUMERIC_RULES',
        approach: 'MAIN_ORDER_PLUS_NUMERIC_PRICE_TRIGGER_TP_SL',
        note: 'TP/SL установлены через Price Trigger API с числовыми правилами: 1=gte, 2=lte'
      };
    }
