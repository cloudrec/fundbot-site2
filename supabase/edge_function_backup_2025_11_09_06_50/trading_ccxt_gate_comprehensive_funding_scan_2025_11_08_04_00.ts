...
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
    ...
    // 🔵 GATE.IO CCXT-ПОДОБНЫЙ ПОДХОД ДЛЯ TP/SL
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing GATE.IO API v4 order with CCXT TRIGGER ORDERS APPROACH');
      ...
      console.log('🔥 GATE.IO v4 CCXT TRIGGER: Base URL:', baseUrl);
      console.log('🔥 GATE.IO v4 CCXT TRIGGER: Symbol:', symbol);
      
      // Получаем текущую цену
      const priceUrl = `/futures/usdt/tickers?contract=${symbol}`;
      const fullPriceUrl = baseUrl + prefix + priceUrl;
      console.log('🔥 GATE.IO v4 CCXT TRIGGER: Price URL:', fullPriceUrl);
      
      const priceResponse = await fetch(fullPriceUrl);
      const priceData = await priceResponse.json();
      
      console.log('🔥 GATE.IO v4 CCXT TRIGGER: Price response status:', priceResponse.status);
      console.log('🔥 GATE.IO v4 CCXT TRIGGER: Price data:', priceData);
      ...
      console.log('🔥 GATE.IO v4 CCXT TRIGGER current price:', currentPrice);
      ...
      console.log('🔥 GATE.IO v4 CCXT TRIGGER order params:', { 
        symbol, 
        quantity, 
        currentPrice,
        tpPrice,
      ...
      console.log('🔥 GATE.IO v4 CCXT TRIGGER main order data:', orderData);
      ...
      console.log('🔥 GATE.IO v4 CCXT TRIGGER order request URL:', fullOrderUrl);
      ...
      const data = await response.json();
      console.log('🔥 GATE.IO v4 CCXT TRIGGER order response status:', response.status);
      console.log('🔥 GATE.IO v4 CCXT TRIGGER order response:', data);

      if (response.status !== 201) {
        throw new Error(`GATE.IO v4 CCXT TRIGGER API Error: ${data.message || 'Unknown error'} (Code: ${response.status})`);
      }

      const orderId = data.id;
      console.log('🔥 GATE.IO v4 CCXT TRIGGER main order created:', orderId);

      // Задержка перед установкой триггерных ордеров
      await delay(2000);

      // 2. CCXT-ПОДОБНЫЙ TAKE PROFIT TRIGGER ORDER
      let tpStatus = 'NOT_SET';
      let slStatus = 'NOT_SET';
      let tpOrderId = null;
      let slOrderId = null;
      ...
      try {
        // Take Profit Trigger Order (CCXT approach)
        const tpTriggerData = {
          initial: {
            contract: symbol,
          ...
          trigger: {
            ...
            rule: 1, // >= (greater than or equal)
            trigger_price: tpPrice,
            expiration: 86400 // 24 часа
          }
        };

        console.log('🔥 GATE.IO v4 CCXT TP trigger data:', tpTriggerData);
        ...
        console.log('🔥 GATE.IO v4 CCXT TP trigger response status:', tpResponse.status);
        console.log('🔥 GATE.IO v4 CCXT TP trigger response:', tpResult);

        if (tpResponse.status === 201) {
          tpStatus = 'SUCCESS';
          tpOrderId = tpResult.id;
          console.log('🔥 GATE.IO v4 CCXT TP trigger order created:', tpOrderId);
        } else {
          tpStatus = 'ERROR';
          console.error('🔥 GATE.IO v4 CCXT TP trigger error:', tpResult.message);
        }

        // Задержка между TP и SL
        await delay(1000);

        // 3. CCXT-ПОДОБНЫЙ STOP LOSS TRIGGER ORDER
        const slTriggerData = {
          initial: {
            contract: symbol,
            size: -quantity, // Противоположный размер для закрытия
          ...
          trigger: {
            strategy_type: 0, // По цене
            price_type: 0, // Последняя цена
            rule: 2, // <= (less than or equal)
            trigger_price: slPrice,
            expiration: 86400 // 24 часа
          }
        };

        console.log('🔥 GATE.IO v4 CCXT SL trigger data:', slTriggerData);
        ...
        console.log('🔥 GATE.IO v4 CCXT SL trigger response status:', slResponse.status);
        console.log('🔥 GATE.IO v4 CCXT SL trigger response:', slResult);

        if (slResponse.status === 201) {
          slStatus = 'SUCCESS';
          slOrderId = slResult.id;
          console.log('🔥 GATE.IO v4 CCXT SL trigger order created:', slOrderId);
        } else {
          slStatus = 'ERROR';
          console.error('🔥 GATE.IO v4 CCXT SL trigger error:', slResult.message);
        }

      } catch (triggerError) {
        console.error('🔥 GATE.IO v4 CCXT trigger orders error:', triggerError);
        tpStatus = 'ERROR';
        slStatus = 'ERROR';
      }
      ...
      const telegramMessage = `
<b>🚀 ОРДЕР РАЗМЕЩЕН</b>
<b>Биржа:</b> Gate.io (CCXT Trigger Orders)
<b>Символ:</b> ${symbol}
<b>Сторона:</b> LONG
<b>Количество:</b> ${quantity}
<b>Цена входа:</b> $${currentPrice.toFixed(4)}
...
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
      ...
      return {
        ...
        side: "long",
        status: 'LIVE',
        message: `Боевой ордер GATE.IO v4 с CCXT Trigger Orders: ${orderId}`,
        quantity: quantity,
        price: currentPrice,
        tp_price: tpPrice,
        sl_price: slPrice,
        ...
        api_version: 'GATE_V4_CCXT_TRIGGER_ORDERS',
        base_url: baseUrl,
        signature_format: 'COMPLETE_DOCUMENTATION_BASED',
        tp_sl_method: 'CCXT_TRIGGER_ORDERS_APPROACH',
