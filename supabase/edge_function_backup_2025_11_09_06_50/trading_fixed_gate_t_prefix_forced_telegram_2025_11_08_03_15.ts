  ...
  try {
    ...
    switch (action) {
      ...
      case 'scan_funding':
        ...
        result = await scanFundingWithForcedTelegram(supabase, user_id, settingsData, apiKeys);
        break;
...
// 📱 ПРИНУДИТЕЛЬНАЯ ФУНКЦИЯ ОТПРАВКИ TELEGRAM УВЕДОМЛЕНИЙ
async function forceSendTelegramNotification(supabase: any, user_id: string, message: string, type: 'order' | 'close' | 'cancel' | 'funding' = 'order') {
  try {
    console.log('📱 FORCE Sending Telegram notification for user:', user_id, 'type:', type);
    ...
    console.log('📱 FORCE Telegram settings query result:', { telegramSettings, telegramError });

    if (telegramError || !telegramSettings) {
      console.log('📱 FORCE No Telegram settings found for user:', user_id);
      throw new Error('Telegram настройки не найдены или неактивны');
    }

    const telegramBotToken = Deno.env.get('TELEGRAM_BOT_TOKEN');
    if (!telegramBotToken) {
      console.error('📱 FORCE TELEGRAM_BOT_TOKEN not found in environment');
      throw new Error('TELEGRAM_BOT_TOKEN не настроен в окружении');
    }

    console.log('📱 FORCE Using bot token:', telegramBotToken ? 'Found' : 'Not found');
    console.log('📱 FORCE Chat ID:', telegramSettings.chat_id);
    ...
    console.log('📱 FORCE Sending message:', fullMessage);
    console.log('📱 FORCE Telegram URL:', telegramUrl);
    ...
    console.log('📱 FORCE Telegram response status:', telegramResponse.status);
    console.log('📱 FORCE Telegram response data:', telegramData);

    if (telegramResponse.ok) {
      console.log('📱 FORCE Telegram notification sent successfully');
      return { success: true, message_id: telegramData.result?.message_id };
    } else {
      console.error('📱 FORCE Telegram notification failed:', telegramData);
      throw new Error(`Telegram API Error: ${telegramData.description || 'Unknown error'}`);
    }

  } catch (error) {
    console.error('📱 FORCE Telegram notification error:', error);
    throw error;
  }
}

// 💰 ПРИНУДИТЕЛЬНАЯ ФУНКЦИЯ СКАНИРОВАНИЯ ФАНДИНГА С ОБЯЗАТЕЛЬНЫМИ TELEGRAM УВЕДОМЛЕНИЯМИ
async function scanFundingWithForcedTelegram(supabase: any, user_id: string, settings: any, apiKeys: any[]) {
  try {
    console.log('💰 FORCED FUNDING SCAN: Starting comprehensive scan for user:', user_id);
    ...
    for (const exchange of exchanges) {
      try {
        console.log(`💰 FORCED FUNDING SCAN: Scanning ${exchange.toUpperCase()}...`);
        
        const symbol = exchange === 'gate' ? 
          `${settings.base_asset}_${settings.quote_asset}` : 
          `${settings.base_asset}USDT`;
        ...
        if (exchange === 'binance') {
          try {
            ...
            console.log(`💰 ${exchange.toUpperCase()} funding response:`, data);
            
            if (response.ok && data.lastFundingRate) {
            ...
            console.error(`💰 ${exchange.toUpperCase()} API error:`, error);
          }
          ...
          try {
            ...
            if (response.ok && data.result && data.result.list && data.result.list.length > 0) {
          ...
          try {
            const gateSymbol = `${settings.base_asset}_${settings.quote_asset}`;
            const response = await fetch(`https://api.gateio.ws/api/v4/futures/usdt/contracts/${gateSymbol}`);
            const data = await response.json();
            console.log(`💰 ${exchange.toUpperCase()} funding response:`, data);
            
            if (response.ok && data.funding_rate) {
              ...
              exchangeUrl = `https://www.gate.io/futures_trade/USDT/${gateSymbol}`;
              fundingSuccess = true;
            }
          } catch (error) {
            console.error(`💰 ${exchange.toUpperCase()} API error:`, error);
        ...
        if (fundingSuccess) {
          ...
          console.log(`💰 ${exchange.toUpperCase()} funding found: ${fundingRate.toFixed(4)}%`);
        } else {
          scanResults.push({
            exchange: exchange.toUpperCase(),
            symbol: symbol,
        ...
        console.error(`💰 Error scanning ${exchange}:`, exchangeError);
        scanResults.push({
          exchange: exchange.toUpperCase(),
    ...
    // Фильтруем успешные результаты и сортируем по абсолютной ставке фандинга
    const successfulResults = scanResults.filter(r => r.success);
    successfulResults.sort((a, b) => b.funding_rate_abs - a.funding_rate_abs);
    
    console.log('💰 FORCED FUNDING SCAN: Successful results:', successfulResults);
    
    // 🔥 ПРИНУДИТЕЛЬНАЯ ОТПРАВКА TELEGRAM УВЕДОМЛЕНИЙ
    let telegramResult = { success: false, reason: 'No attempt made' };
    
    try {
      if (successfulResults.length > 0) {
        // Отправляем сводное уведомление со всеми найденными фандингами
        let telegramMessage = `<b>💰 СКАНИРОВАНИЕ ФАНДИНГА ЗАВЕРШЕНО</b>\n\n`;
        
        successfulResults.forEach((result, index) => {
          telegramMessage += `<b>${index + 1}. ${result.exchange}</b>\n`;
          telegramMessage += `<b>Символ:</b> ${result.symbol}\n`;
        ...
        console.log('💰 FORCED FUNDING SCAN: Sending comprehensive Telegram message');
        telegramResult = await forceSendTelegramNotification(supabase, user_id, telegramMessage, 'funding');
        console.log('💰 FORCED FUNDING SCAN: Telegram result:', telegramResult);
        
      } else {
        // Отправляем уведомление об отсутствии данных
        const errorMessage = `
<b>💰 СКАНИРОВАНИЕ ФАНДИНГА</b>
<b>Статус:</b> Не удалось получить данные ни с одной биржи
<b>Проверенные биржи:</b> Binance, Bybit, Gate.io
<b>Время сканирования:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
        `.trim();

        telegramResult = await forceSendTelegramNotification(supabase, user_id, errorMessage, 'funding');
      }
    } catch (telegramError) {
      console.error('💰 FORCED FUNDING SCAN: Telegram error:', telegramError);
    ...
    return {
      ...
      successful_scans: successfulResults.length,
      failed_scans: scanResults.length - successfulResults.length,
      best_funding: successfulResults[0] || null,
      all_results: scanResults,
      telegram_sent: telegramResult.success,
      telegram_error: telegramResult.reason || null,
      scan_time: new Date().toISOString(),
      message: `Сканирование завершено: найдено ${successfulResults.length} фандингов, Telegram: ${telegramResult.success ? 'ОТПРАВЛЕНО' : 'ОШИБКА'}`
    };
    
  } catch (error: any) {
    console.error('💰 FORCED FUNDING SCAN: Error:', error);
    
    // Пытаемся отправить уведомление об ошибке
    try {
      const errorMessage = `
<b>💰 ОШИБКА СКАНИРОВАНИЯ ФАНДИНГА</b>
<b>Ошибка:</b> ${error.message}
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
      `.trim();

      await forceSendTelegramNotification(supabase, user_id, errorMessage, 'funding');
    } catch (telegramError) {
      console.error('💰 FORCED FUNDING SCAN: Failed to send error notification:', telegramError);
    }
    
    throw new Error(`Ошибка автоматического сканирования фандинга: ${error.message}`);
  }
}
...
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
      ...
      await forceSendTelegramNotification(supabase, user_id, telegramMessage, 'order');
...
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
    ...
    // 🔵 GATE.IO IMPLEMENTATION - ИСПРАВЛЕННЫЙ ПРЕФИКС t-
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing GATE.IO API v4 order with FIXED t- PREFIX');
      ...
      console.log('🔥 GATE.IO v4 FIXED PREFIX: Base URL:', baseUrl);
      console.log('🔥 GATE.IO v4 FIXED PREFIX: Symbol:', symbol);
      
      // Получаем текущую цену
      const priceUrl = `/futures/usdt/tickers?contract=${symbol}`;
      const fullPriceUrl = baseUrl + prefix + priceUrl;
      console.log('🔥 GATE.IO v4 FIXED PREFIX: Price URL:', fullPriceUrl);
      
      const priceResponse = await fetch(fullPriceUrl);
      const priceData = await priceResponse.json();
      
      console.log('🔥 GATE.IO v4 FIXED PREFIX: Price response status:', priceResponse.status);
      console.log('🔥 GATE.IO v4 FIXED PREFIX: Price data:', priceData);
      ...
      console.log('🔥 GATE.IO v4 FIXED PREFIX current price:', currentPrice);
      ...
      const tpPrice = roundGatePrice(currentPrice * (1 + settings.long_tp_offset_percent / 100));
      const slPrice = roundGatePrice(currentPrice * (1 - settings.long_stop_loss_percent / 100));
      
      console.log('🔥 GATE.IO v4 FIXED PREFIX order params:', { 
        symbol, 
        quantity, 
        currentPrice,
        tpPrice,
      ...
      // 1. ОСНОВНОЙ MARKET ОРДЕР С ИСПРАВЛЕННЫМ ПРЕФИКСОМ t-
      const orderData = {
        contract: symbol,
        size: quantity,
        price: "0", // Market order
        tif: "ioc",
        text: `t-ccxt-${Date.now().toString().slice(-6)}`, // ✅ ИСПРАВЛЕННЫЙ ПРЕФИКС t-
        // CCXT-подобные параметры для TP/SL
      ...
      console.log('🔥 GATE.IO v4 FIXED PREFIX main order data:', orderData);
      ...
      console.log('🔥 GATE.IO v4 FIXED PREFIX order request URL:', fullOrderUrl);
      ...
      const data = await response.json();
      console.log('🔥 GATE.IO v4 FIXED PREFIX order response status:', response.status);
      console.log('🔥 GATE.IO v4 FIXED PREFIX order response:', data);

      if (response.status !== 201) {
        // Если основной подход не работает, пробуем простой ордер без TP/SL
        console.log('🔥 GATE.IO v4 FIXED PREFIX: Main approach failed, trying simple order...');
        ...
        const simpleOrderData = {
          ...
          price: "0", // Market order
          tif: "ioc",
          text: `t-simple-${Date.now().toString().slice(-6)}` // ✅ ИСПРАВЛЕННЫЙ ПРЕФИКС t-
        };
        
        const simplePayload = JSON.stringify(simpleOrderData);
        const simpleSig = await createCompleteGateSignature(apiKey.api_secret, 'POST', prefix + orderUrl, '', simplePayload);
        
        const simpleResponse = await fetch(fullOrderUrl, {
          method: 'POST',
          headers: {
            'KEY': apiKey.api_key,
            'SIGN': simpleSig.signature,
            'Timestamp': simpleSig.timestamp,
            'Content-Type': 'application/json'
          },
          body: simplePayload
        });
        ...
        console.log('🔥 GATE.IO v4 FIXED PREFIX simple order response status:', simpleResponse.status);
        console.log('🔥 GATE.IO v4 FIXED PREFIX simple order response:', simpleData);

        if (simpleResponse.status !== 201) {
          throw new Error(`GATE.IO v4 FIXED PREFIX API Error: ${simpleData.message || data.message || 'Unknown error'} (Code: ${simpleResponse.status})`);
        }

        const orderId = simpleData.id;
        console.log('🔥 GATE.IO v4 FIXED PREFIX simple order created:', orderId);

        // 📱 ОТПРАВЛЯЕМ TELEGRAM УВЕДОМЛЕНИЕ ДЛЯ ПРОСТОГО ОРДЕРА
        const telegramMessage = `
<b>🚀 ОРДЕР РАЗМЕЩЕН</b>
<b>Биржа:</b> Gate.io (ИСПРАВЛЕННЫЙ t- префикс)
<b>Символ:</b> ${symbol}
<b>Сторона:</b> LONG
<b>Количество:</b> ${quantity}
<b>Цена входа:</b> $${currentPrice.toFixed(4)}
<b>Take Profit:</b> $${tpPrice} ⚠️ (Установите вручную)
<b>Stop Loss:</b> $${slPrice} ⚠️ (Установите вручную)
...
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
        ...
        return {
          ...
          side: "long",
          status: 'LIVE',
          message: `Боевой ордер GATE.IO v4 (ИСПРАВЛЕННЫЙ t- префикс, простой): ${orderId}`,
          quantity: quantity,
          price: currentPrice,
          tp_price: tpPrice,
          sl_price: slPrice,
          ...
          api_version: 'GATE_V4_FIXED_T_PREFIX_SIMPLE_FALLBACK',
          base_url: baseUrl,
          signature_format: 'COMPLETE_DOCUMENTATION_BASED',
          text_field_fix: 'T_PREFIX_APPLIED_CORRECTLY',
          tp_sl_method: 'MANUAL_SET_REQUIRED_CCXT_FAILED'
        };
      }

      const orderId = data.id;
      console.log('🔥 GATE.IO v4 FIXED PREFIX order with TP/SL created:', orderId);

      // 📱 ОТПРАВЛЯЕМ TELEGRAM УВЕДОМЛЕНИЕ ДЛЯ УСПЕШНОГО ОРДЕРА
      const telegramMessage = `
<b>🚀 ОРДЕР РАЗМЕЩЕН</b>
<b>Биржа:</b> Gate.io (ИСПРАВЛЕННЫЙ t- префикс)
<b>Символ:</b> ${symbol}
...
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
      ...
      return {
        ...
        side: "long",
        status: 'LIVE',
        message: `Боевой ордер GATE.IO v4 с ИСПРАВЛЕННЫМ t- префиксом TP/SL: ${orderId}`,
        quantity: quantity,
        price: currentPrice,
        tp_price: tpPrice,
        sl_price: slPrice,
        ...
        api_url: 'gateio.ws',
        debug_url: fullOrderUrl,
        api_version: 'GATE_V4_FIXED_T_PREFIX_TP_SL_SUCCESS',
        base_url: baseUrl,
        signature_format: 'COMPLETE_DOCUMENTATION_BASED',
        text_field_fix: 'T_PREFIX_APPLIED_CORRECTLY',
        tp_sl_method: 'CCXT_INTEGRATED_TP_SL_WITH_FIXED_PREFIX'
      };
    }
...
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
      ...
      await forceSendTelegramNotification(supabase, user_id, telegramMessage, 'cancel');
...
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
      ...
      await forceSendTelegramNotification(supabase, user_id, telegramMessage, 'close');
