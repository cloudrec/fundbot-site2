...
// Функция правильного округления количества для Bybit
...
async function placeOrderWithTPSL(apiKey: any, settings: any) {
  try {
    ...
    if (apiKey.exchange === 'binance') {
      ...
      // 4. Take Profit ордер - используем LIMIT вместо TAKE_PROFIT
      ...
      console.log('🔥 TP order query (LIMIT):', tpQuery);
    ...
    // ИСПРАВЛЕННЫЙ BYBIT КОД С ПРАВИЛЬНЫМ РАСЧЕТОМ КОЛИЧЕСТВА
    if (apiKey.exchange === 'bybit') {
      ...
      // ИСПРАВЛЕННЫЙ РАСЧЕТ КОЛИЧЕСТВА
