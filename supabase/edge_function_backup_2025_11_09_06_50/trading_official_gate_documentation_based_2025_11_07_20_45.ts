...
// 🆕 ОФИЦИАЛЬНАЯ ФУНКЦИЯ СОЗДАНИЯ ПОДПИСИ ДЛЯ GATE.IO API V4 ПО ДОКУМЕНТАЦИИ
async function createOfficialGateSignature(secret: string, method: string, url: string, queryString: string = '', payloadString: string = ''): Promise<{ signature: string, timestamp: string }> {
  try {
    // Используем текущее время в секундах (как в официальном примере)
    const timestamp = Math.floor(Date.now() / 1000).toString();
    
    console.log('🔥 OFFICIAL GATE.IO v4 signature inputs:');
    ...
    console.log('🔥 OFFICIAL GATE.IO v4 hashed payload:', hashedPayload);
    
    // ОФИЦИАЛЬНЫЙ ФОРМАТ СТРОКИ ПОДПИСИ ПО ДОКУМЕНТАЦИИ:
    // method + '\n' + url + '\n' + query_string + '\n' + hashed_payload + '\n' + timestamp
    const signatureString = `${method}\n${url}\n${queryString}\n${hashedPayload}\n${timestamp}`;
    
    console.log('🔥 OFFICIAL GATE.IO v4 signature string:');
...
async function getBalance(apiKey: any, settings: any) {
  try {
    console.log('🔥 OFFICIAL GATE.IO getBalance started for exchange:', apiKey.exchange);
    ...
    // 🆕 ОФИЦИАЛЬНАЯ GATE.IO API V4 РЕАЛИЗАЦИЯ ПО ДОКУМЕНТАЦИИ
    if (apiKey.exchange === 'gate') {
      ...
      console.log('🔥 OFFICIAL GATE.IO v4: Base URL:', baseUrl);
      console.log('🔥 OFFICIAL GATE.IO v4: Prefix:', prefix);
      console.log('🔥 OFFICIAL GATE.IO v4: URL path:', url);
      console.log('🔥 OFFICIAL GATE.IO v4: Full URL:', baseUrl + prefix + url);
      
      // Создаем подпись по официальному примеру
      const { signature, timestamp } = await createOfficialGateSignature(apiKey.api_secret, 'GET', prefix + url, queryString, payloadString);
      ...
      const result = {
        ...
        base_url: baseUrl,
        signature_format: 'OFFICIAL_DOCUMENTATION_BASED'
      };
    ...
    console.error('🔥 OFFICIAL GATE.IO Balance error:', error);
...
async function placeOrderWithTPSL(apiKey: any, settings: any) {
  try {
    console.log('🔥 OFFICIAL GATE.IO placeOrderWithTPSL started for exchange:', apiKey.exchange);
    ...
    // 🆕 ОФИЦИАЛЬНАЯ GATE.IO API V4 РЕАЛИЗАЦИЯ - ТОЛЬКО ОСНОВНОЙ ОРДЕР
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing OFFICIAL GATE.IO API v4 order (BASIC ORDER ONLY)');
      ...
      console.log('🔥 OFFICIAL GATE.IO v4 ORDER: Base URL:', baseUrl);
      console.log('🔥 OFFICIAL GATE.IO v4 ORDER: Symbol:', symbol);
      
      // Получаем текущую цену
      const priceUrl = `/futures/usdt/tickers?contract=${symbol}`;
      const fullPriceUrl = baseUrl + prefix + priceUrl;
      console.log('🔥 OFFICIAL GATE.IO v4 ORDER: Price URL:', fullPriceUrl);
      
      const priceResponse = await fetch(fullPriceUrl);
      const priceData = await priceResponse.json();
      
      console.log('🔥 OFFICIAL GATE.IO v4 ORDER: Price response status:', priceResponse.status);
      console.log('🔥 OFFICIAL GATE.IO v4 ORDER: Price data:', priceData);
      ...
      console.log('🔥 OFFICIAL GATE.IO v4 order params (BASIC ONLY):', { 
        symbol, 
        quantity, 
        currentPrice
      });
      ...
      console.log('🔥 OFFICIAL GATE.IO v4 BASIC main order data:', orderData);
      
      const orderUrl = '/futures/usdt/orders';
      const payloadString = JSON.stringify(orderData);
      const { signature, timestamp } = await createOfficialGateSignature(apiKey.api_secret, 'POST', prefix + orderUrl, '', payloadString);
      ...
      console.log('🔥 OFFICIAL GATE.IO v4 BASIC main order created:', orderId);

      // Рассчитываем TP/SL для отображения (но не устанавливаем автоматически)
      ...
      return {
        ...
        side: "long",
        status: 'LIVE',
        message: `Боевой ордер OFFICIAL GATE.IO v4 (БАЗОВЫЙ): ${orderId}`,
        quantity: quantity,
        price: currentPrice,
        ...
        api_version: 'OFFICIAL_GATE_V4_BASIC_ORDER_ONLY',
        base_url: baseUrl,
        signature_format: 'OFFICIAL_DOCUMENTATION_BASED',
        text_field_fix: 'APPLIED',
        tp_sl_implementation: 'BASIC_ORDER_ONLY_MANUAL_TP_SL_REQUIRED',
        note: 'TP/SL нужно установить вручную через интерфейс Gate.io'
      };
    }
    ...
    console.error('🔥 OFFICIAL GATE.IO Set TP/SL error:', error);
    throw error;
  }
}

// 🆕 ОФИЦИАЛЬНАЯ РЕАЛИЗАЦИЯ ПОЛУЧЕНИЯ ПОЗИЦИЙ ДЛЯ GATE.IO
async function getPositions(apiKey: any, settings: any) {
  try {
    console.log('🔥 OFFICIAL GATE.IO getPositions started for exchange:', apiKey.exchange);
    
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing OFFICIAL GATE.IO API v4 positions request');
      ...
      console.log('🔥 OFFICIAL GATE.IO v4 positions URL:', baseUrl + prefix + url);
      ...
      const data = await response.json();
      console.log('🔥 OFFICIAL GATE.IO v4 positions response status:', response.status);
      console.log('🔥 OFFICIAL GATE.IO v4 positions response:', data);

      if (response.status !== 200) {
        throw new Error(`OFFICIAL GATE.IO v4 Positions API Error: ${data.message || 'Unknown error'} (Code: ${response.status})`);
      }

      // Фильтруем только открытые позиции
      const openPositions = data.filter((position: any) => parseFloat(position.size) !== 0);
      
      console.log('🔥 OFFICIAL GATE.IO v4 open positions:', openPositions);
    ...
    console.error('🔥 OFFICIAL GATE.IO Positions error:', error);
    throw new Error(`Ошибка получения позиций: ${error.message}`);
  }
}

// 🆕 ОФИЦИАЛЬНАЯ РЕАЛИЗАЦИЯ ОТМЕНЫ ОРДЕРОВ ДЛЯ GATE.IO
async function cancelAllOrders(apiKey: any, settings: any) {
  try {
    console.log('🔥 OFFICIAL GATE.IO cancelAllOrders started for exchange:', apiKey.exchange);
    
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing OFFICIAL GATE.IO API v4 cancel orders request');
      ...
      // ОФИЦИАЛЬНЫЙ ENDPOINT ДЛЯ ОТМЕНЫ ОРДЕРОВ ПО ДОКУМЕНТАЦИИ
      const ordersUrl = `/futures/usdt/orders?contract=${symbol}&status=open`;
      const { signature: ordersSig, timestamp: ordersTs } = await createOfficialGateSignature(apiKey.api_secret, 'DELETE', prefix + ordersUrl, '', '');
      
      console.log('🔥 OFFICIAL GATE.IO v4 cancel orders URL:', baseUrl + prefix + ordersUrl);
      ...
      console.log('🔥 OFFICIAL GATE.IO v4 cancel orders response status:', ordersResponse.status);
      console.log('🔥 OFFICIAL GATE.IO v4 cancel orders response:', ordersData);
      ...
      return {
        ...
        exchange: 'GATE',
        status: 'LIVE',
        message: `Отмена ордеров OFFICIAL GATE.IO v4: ${cancelledOrders} ордеров отменено`,
        api_version: 'OFFICIAL_GATE_V4',
        symbol: symbol,
        endpoint_used: ordersUrl
      };
    }
    ...
    console.error('🔥 OFFICIAL GATE.IO Cancel orders error:', error);
    throw new Error(`Ошибка отмены ордеров: ${error.message}`);
  }
}

// 🆕 ОФИЦИАЛЬНАЯ РЕАЛИЗАЦИЯ ЗАКРЫТИЯ ПОЗИЦИЙ ДЛЯ GATE.IO
async function closeAllPositions(apiKey: any, settings: any) {
  try {
    console.log('🔥 OFFICIAL GATE.IO closeAllPositions started for exchange:', apiKey.exchange);
    
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing OFFICIAL GATE.IO API v4 close positions request');
      ...
      const { signature: posSig, timestamp: posTs } = await createOfficialGateSignature(apiKey.api_secret, 'GET', prefix + positionsUrl, '', '');
      
      console.log('🔥 OFFICIAL GATE.IO v4 get positions URL:', baseUrl + prefix + positionsUrl);
      ...
      console.log('🔥 OFFICIAL GATE.IO v4 positions response status:', positionsResponse.status);
      console.log('🔥 OFFICIAL GATE.IO v4 positions response:', positionsData);

      if (positionsResponse.status !== 200) {
        throw new Error(`OFFICIAL GATE.IO v4 Positions API Error: ${positionsData.message || 'Unknown error'} (Code: ${positionsResponse.status})`);
      }
      ...
      console.log('🔥 OFFICIAL GATE.IO v4 open positions for', symbol, ':', openPositions);
      ...
      // 2. ИСПОЛЬЗУЕМ ОФИЦИАЛЬНЫЙ ENDPOINT ДЛЯ ЗАКРЫТИЯ ПОЗИЦИЙ
      ...
      for (const position of openPositions) {
        try {
          ...
          // ОФИЦИАЛЬНЫЙ СПОСОБ ЗАКРЫТИЯ ПОЗИЦИЙ ЧЕРЕЗ MARKET ORDER С ПРОТИВОПОЛОЖНЫМ РАЗМЕРОМ
          const closeSize = -positionSize; // Противоположный размер для закрытия
          
          console.log('🔥 OFFICIAL GATE.IO v4 closing position:', { 
          ...
          const closeOrderData = {
            ...
            price: "0", // Market order
            tif: "ioc",
            text: `t-${Date.now()}-close-position-official`
          };

          console.log('🔥 OFFICIAL GATE.IO v4 close order data:', closeOrderData);

          const closePayload = JSON.stringify(closeOrderData);
          const { signature: closeSig, timestamp: closeTs } = await createOfficialGateSignature(apiKey.api_secret, 'POST', prefix + '/futures/usdt/orders', '', closePayload);
          ...
          console.log('🔥 OFFICIAL GATE.IO v4 close position response status:', closeResponse.status);
          console.log('🔥 OFFICIAL GATE.IO v4 close position response:', closeData);
          ...
          if (closeResponse.status === 201) {
            ...
            console.log('🔥 OFFICIAL GATE.IO v4 position closed successfully:', closeData.id);
          } else {
            closeResults.push({
              contract: position.contract,
              status: 'ERROR',
            ...
            console.error('🔥 OFFICIAL GATE.IO v4 close position error:', closeData.message);
          }
          ...
          console.error('🔥 OFFICIAL GATE.IO v4 close position error:', closeError);
          closeResults.push({
            contract: position.contract,
            status: 'ERROR',
      ...
      return {
        ...
        exchange: 'GATE',
        status: 'LIVE',
        message: `Закрытие позиций OFFICIAL GATE.IO v4: ${closedPositions}/${openPositions.length} успешно`,
        api_version: 'OFFICIAL_GATE_V4',
        symbol: symbol,
        close_method: 'OFFICIAL_MARKET_ORDER_WITH_OPPOSITE_SIZE'
      };
    }
    ...
    console.error('🔥 OFFICIAL GATE.IO Close positions error:', error);
