...
serve(async (req) => {
  ...
  try {
    ...
    console.log('🟡 BINANCE SIMPLE: Starting action:', action);
    ...
    console.log('🟡 BINANCE SIMPLE: Keys loaded, settings:', !!settings);
    ...
    console.error('❌ BINANCE SIMPLE Error:', error.message);
    return new Response(
...
async function handleGetBalance(apiKeys: any) {
  console.log('🟡 BINANCE SIMPLE: Getting balance');
...
async function handleGetPositions(apiKeys: any) {
  console.log('🟡 BINANCE SIMPLE: Getting positions');
...
// Размещение ордера с TP/SL - ПРОСТАЯ ЛОГИКА
async function handlePlaceOrderWithTPSL(apiKeys: any, settings: any) {
  console.log('🟡 BINANCE SIMPLE: Placing order with TP/SL');
  ...
  try {
    ...
    // 2. Рассчитываем количество (простая логика)
    const orderAmountUSD = settings.order_amount_usd || 10;
    const quantity = (orderAmountUSD / currentPrice).toFixed(3);
    
    console.log('🟡 BINANCE SIMPLE: Order calculation:', {
      symbol,
      currentPrice,
      orderAmountUSD,
      quantity
    ...
    // 4. Размещаем Take Profit ордер (+2%)
    const tpPrice = (currentPrice * 1.02).toFixed(4);
    ...
    // 5. Размещаем Stop Loss ордер (-2%)
    const slPrice = (currentPrice * 0.98).toFixed(4);
...
async function placeOrder(apiKeys: any, orderParams: any) {
  ...
  const data = await response.json();
  console.log('🟡 BINANCE SIMPLE: Order response:', data);
...
async function handleClosePositions(apiKeys: any) {
  console.log('🟡 BINANCE SIMPLE: Closing positions');
...
async function handleCancelOrders(apiKeys: any) {
  console.log('🟡 BINANCE SIMPLE: Canceling orders');
