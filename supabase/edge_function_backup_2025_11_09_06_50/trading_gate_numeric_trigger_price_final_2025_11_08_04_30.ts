...
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
    ...
    // 🔵 ИСПРАВЛЕННЫЙ GATE.IO TP/SL ПОДХОД С ЧИСЛОВЫМИ TRIGGER_PRICE
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing GATE.IO API v4 order with NUMERIC TRIGGER_PRICE APPROACH');
      ...
      console.log('🔥 GATE.IO v4 NUMERIC TRIGGER_PRICE: Base URL:', baseUrl);
      console.log('🔥 GATE.IO v4 NUMERIC TRIGGER_PRICE: Symbol:', symbol);
      
      // Получаем текущую цену
      const priceUrl = `/futures/usdt/tickers?contract=${symbol}`;
      const fullPriceUrl = baseUrl + prefix + priceUrl;
      console.log('🔥 GATE.IO v4 NUMERIC TRIGGER_PRICE: Price URL:', fullPriceUrl);
      
      const priceResponse = await fetch(fullPriceUrl);
      const priceData = await priceResponse.json();
      
      console.log('🔥 GATE.IO v4 NUMERIC TRIGGER_PRICE: Price response status:', priceResponse.status);
      console.log('🔥 GATE.IO v4 NUMERIC TRIGGER_PRICE: Price data:', priceData);
      ...
      console.log('🔥 GATE.IO v4 NUMERIC TRIGGER_PRICE current price:', currentPrice);
      
      // Рассчитываем количество (Gate.io использует контракты)
      const calculatedQuantity = Math.floor(settings.order_amount_usd / currentPrice);
      const quantity = calculatedQuantity;
      
      // Рассчитываем цены TP/SL КАК ЧИСЛА (НЕ СТРОКИ!)
      ...
      // 1. ОСНОВНОЙ MARKET ОРДЕР (БЕЗ TP/SL)
      const orderData = {
        contract: symbol,
        size: quantity,
        price: "0", // Market order
        tif: "ioc",
        text: `t-main-${Date.now().toString().slice(-6)}` // Основной ордер
      };
      
      console.log('🔥 GATE.IO v4 NUMERIC TRIGGER_PRICE main order data:', orderData);
      ...
      console.log('🔥 GATE.IO v4 NUMERIC TRIGGER_PRICE order request URL:', fullOrderUrl);
      ...
      const data = await response.json();
      console.log('🔥 GATE.IO v4 NUMERIC TRIGGER_PRICE order response status:', response.status);
      console.log('🔥 GATE.IO v4 NUMERIC TRIGGER_PRICE order response:', data);

      if (response.status !== 201) {
        throw new Error(`GATE.IO v4 NUMERIC TRIGGER_PRICE API Error: ${data.message || 'Unknown error'} (Code: ${response.status})`);
      }

      const orderId = data.id;
      console.log('🔥 GATE.IO v4 NUMERIC TRIGGER_PRICE main order created:', orderId);

      // Задержка перед установкой триггерных ордеров
      await delay(3000);

      // 2. ИСПРАВЛЕННЫЙ TAKE PROFIT TRIGGER ORDER С ЧИСЛОВЫМИ ЗНАЧЕНИЯМИ
      let tpStatus = 'NOT_SET';
      let slStatus = 'NOT_SET';
      let tpOrderId = null;
      let slOrderId = null;
      ...
      try {
        // ИСПРАВЛЕННАЯ СТРУКТУРА TP TRIGGER ORDER С ЧИСЛОВЫМИ ЗНАЧЕНИЯМИ
        const tpTriggerData = {
          initial: {
            contract: symbol,
            size: -quantity, // Противоположный размер для закрытия
            price: tpPriceString, // Строка для цены ордера
            tif: "gtc",
            text: `t-tp-${Date.now().toString().slice(-6)}`,
            reduce_only: true
          },
          trigger: {
            strategy_type: 0, // По цене
            price_type: 0, // Последняя цена
            rule: 1, // >= (greater than or equal) - числовое значение
            trigger_price: tpPriceNumber, // ✅ ЧИСЛОВОЕ ЗНАЧЕНИЕ вместо строки!
            expiration: 86400 // 24 часа
          }
        };

        console.log('🔥 GATE.IO v4 NUMERIC TP trigger data:', JSON.stringify(tpTriggerData, null, 2));

        const tpPayload = JSON.stringify(tpTriggerData);
        const tpSig = await createCompleteGateSignature(apiKey.api_secret, 'POST', prefix + '/futures/usdt/price_orders', '', tpPayload);
        ...
        console.log('🔥 GATE.IO v4 NUMERIC TP trigger response status:', tpResponse.status);
        console.log('🔥 GATE.IO v4 NUMERIC TP trigger response:', tpResult);

        if (tpResponse.status === 201) {
          tpStatus = 'SUCCESS';
          tpOrderId = tpResult.id;
          console.log('🔥 GATE.IO v4 NUMERIC TP trigger order created:', tpOrderId);
        } else {
          tpStatus = 'ERROR';
          tpError = tpResult.message || 'Unknown TP error';
          console.error('🔥 GATE.IO v4 NUMERIC TP trigger error:', tpResult.message);
        }

        // Задержка между TP и SL
        await delay(2000);

        // 3. ИСПРАВЛЕННЫЙ STOP LOSS TRIGGER ORDER С ЧИСЛОВЫМИ ЗНАЧЕНИЯМИ
        const slTriggerData = {
          initial: {
            contract: symbol,
            size: -quantity, // Противоположный размер для закрытия
            price: slPriceString, // Строка для цены ордера
            tif: "gtc",
            text: `t-sl-${Date.now().toString().slice(-6)}`,
            reduce_only: true
          },
          trigger: {
            strategy_type: 0, // По цене
            price_type: 0, // Последняя цена
            rule: 2, // <= (less than or equal) - числовое значение
            trigger_price: slPriceNumber, // ✅ ЧИСЛОВОЕ ЗНАЧЕНИЕ вместо строки!
            expiration: 86400 // 24 часа
          }
        };

        console.log('🔥 GATE.IO v4 NUMERIC SL trigger data:', JSON.stringify(slTriggerData, null, 2));

        const slPayload = JSON.stringify(slTriggerData);
        const slSig = await createCompleteGateSignature(apiKey.api_secret, 'POST', prefix + '/futures/usdt/price_orders', '', slPayload);
        ...
        console.log('🔥 GATE.IO v4 NUMERIC SL trigger response status:', slResponse.status);
        console.log('🔥 GATE.IO v4 NUMERIC SL trigger response:', slResult);

        if (slResponse.status === 201) {
          slStatus = 'SUCCESS';
          slOrderId = slResult.id;
          console.log('🔥 GATE.IO v4 NUMERIC SL trigger order created:', slOrderId);
        } else {
          slStatus = 'ERROR';
          slError = slResult.message || 'Unknown SL error';
          console.error('🔥 GATE.IO v4 NUMERIC SL trigger error:', slResult.message);
        }

      } catch (triggerError) {
        console.error('🔥 GATE.IO v4 NUMERIC trigger orders error:', triggerError);
        tpStatus = 'ERROR';
        slStatus = 'ERROR';
        tpError = triggerError.message;
        slError = triggerError.message;
      }

      // 📱 ОТПРАВЛЯЕМ В ВАШ TELEGRAM БОТ
      const telegramMessage = `
<b>🚀 ОРДЕР РАЗМЕЩЕН</b>
<b>Биржа:</b> Gate.io (Numeric Trigger Price)
<b>Символ:</b> ${symbol}
<b>Сторона:</b> LONG
<b>Количество:</b> ${quantity}
<b>Цена входа:</b> $${currentPrice.toFixed(4)}
...
<b>ID основного ордера:</b> ${orderId}
<b>ID TP ордера:</b> ${tpOrderId || 'Не установлен'}
<b>ID SL ордера:</b> ${slOrderId || 'Не установлен'}
${tpError ? `<b>TP ошибка:</b> ${tpError}` : ''}
${slError ? `<b>SL ошибка:</b> ${slError}` : ''}
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
      `.trim();
      ...
      return {
        ...
        side: "long",
        status: 'LIVE',
        message: `Боевой ордер GATE.IO v4 с Numeric Trigger Price: ${orderId}`,
        quantity: quantity,
        price: currentPrice,
        ...
        api_version: 'GATE_V4_NUMERIC_TRIGGER_PRICE',
        base_url: baseUrl,
        signature_format: 'COMPLETE_DOCUMENTATION_BASED',
        tp_sl_method: 'NUMERIC_TRIGGER_PRICE_APPROACH',
        trigger_orders_endpoint: '/futures/usdt/price_orders',
        fix_applied: 'NUMERIC_TRIGGER_PRICE_VALUES_INSTEAD_OF_STRINGS',
        tp_price_number: tpPriceNumber,
        sl_price_number: slPriceNumber
      };
    }
