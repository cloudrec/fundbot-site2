...
// 💰 ИСПРАВЛЕННАЯ ФУНКЦИЯ СКАНИРОВАНИЯ ФАНДИНГА
async function scanFunding(supabase: any, user_id: string, settings: any) {
  try {
    console.log('💰 FUNDING SCAN: Starting for user:', user_id);
    console.log('💰 FUNDING SCAN: Settings:', settings);
    
    const symbol = `${settings.base_asset}_${settings.quote_asset}`;
    const exchange = settings.exchange;
    
    console.log('💰 FUNDING SCAN: Symbol:', symbol, 'Exchange:', exchange);
    ...
    try {
      if (exchange === 'binance') {
        ...
        console.log('💰 FUNDING SCAN: Fetching Gate.io funding rate...');
        const response = await fetch(`https://api.gateio.ws/api/v4/futures/usdt/contracts/${symbol}`);
        const data = await response.json();
        console.log('💰 FUNDING SCAN: Gate.io response:', data);
      ...
      console.error('💰 FUNDING SCAN: API Error:', apiError);
      fundingSuccess = false;
    }
    
    console.log('💰 FUNDING SCAN: Success:', fundingSuccess);
    console.log('💰 FUNDING SCAN: Rate:', fundingRate);
    console.log('💰 FUNDING SCAN: Next time:', nextFundingTime);
    console.log('💰 FUNDING SCAN: URL:', exchangeUrl);
    
    if (!fundingSuccess) {
      console.log('💰 FUNDING SCAN: Failed to get funding data, sending error notification');
      
      const errorMessage = `
<b>💰 ОШИБКА ФАНДИНГА</b>
<b>Биржа:</b> ${exchange.toUpperCase()}
<b>Символ:</b> ${symbol}
<b>Статус:</b> Не удалось получить данные о фандинге
<b>Время сканирования:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
      ...
      console.log('💰 FUNDING SCAN: Error notification result:', telegramResult);
      ...
      return {
        ...
        message: 'Ошибка получения данных о фандинге',
...
<b>Время сканирования:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
    ...
    console.log('💰 FUNDING SCAN: Sending Telegram message:', telegramMessage);
    
    const telegramResult = await sendTelegramNotification(supabase, user_id, telegramMessage, 'funding');
    console.log('💰 FUNDING SCAN: Telegram send result:', telegramResult);
...
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
    ...
    // 🔵 GATE.IO IMPLEMENTATION - С АВТОМАТИЧЕСКИМИ TP/SL
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing GATE.IO API v4 order - WITH AUTOMATIC TP/SL');
      ...
      console.log('🔥 GATE.IO v4 order data:', orderData);
      ...
      console.log('🔥 GATE.IO v4: Waiting 3 seconds before setting TP/SL...');
      await delay(3000);

      // 🔥 АВТОМАТИЧЕСКИЕ TP/SL ЧЕРЕЗ PRICE ORDERS API
      let tpStatus = 'NOT_SET';
      let slStatus = 'NOT_SET';
      let tpOrderId = null;
      let slOrderId = null;
      ...
      try {
        // TAKE PROFIT ORDER
        console.log('🔥 GATE.IO v4: Setting Take Profit...');
        ...
        const tpUrl = '/futures/usdt/price_orders';
        const { signature: tpSig, timestamp: tpTs } = await createCompleteGateSignature(apiKey.api_secret, 'POST', prefix + tpUrl, '', tpPayload);

        const tpResponse = await fetch(baseUrl + prefix + tpUrl, {
          method: 'POST',
          headers: {
            'KEY': apiKey.api_key,
            'SIGN': tpSig,
        ...
        // STOP LOSS ORDER
        console.log('🔥 GATE.IO v4: Setting Stop Loss...');
        ...
        const { signature: slSig, timestamp: slTs } = await createCompleteGateSignature(apiKey.api_secret, 'POST', prefix + tpUrl, '', slPayload);

        const slResponse = await fetch(baseUrl + prefix + tpUrl, {
          method: 'POST',
          headers: {
            'KEY': apiKey.api_key,
            'SIGN': slSig,
        ...
        console.error('🔥 GATE.IO v4 TP/SL error:', tpslError);
        tpStatus = 'ERROR';
        slStatus = 'ERROR';
        tpError = tpslError.message;
        slError = tpslError.message;
...
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
      ...
      return {
        ...
        side: "long",
        status: 'LIVE',
        message: `Боевой ордер GATE.IO v4 с автоматическими TP/SL: ${orderId}`,
        quantity: quantity,
        price: currentPrice,
        tp_price: tpPrice,
        sl_price: slPrice,
        ...
        api_version: 'GATE_V4_WITH_AUTOMATIC_TP_SL',
        base_url: baseUrl,
        signature_format: 'COMPLETE_DOCUMENTATION_BASED',
        text_field_fix: 'CORRECT_T_PREFIX_APPLIED',
        tp_sl_method: 'PRICE_ORDERS_API_WITH_TRIGGER_RULES'
      };
    }
...
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
    ...
    if (apiKey.exchange === 'gate') {
      ...
      try {
        ...
        console.log('🔥 GATE.IO v4: Cancelling price orders (TP/SL)...');
        ...
        if (priceResponse.status === 200) {
          const priceCancelled = Array.isArray(priceData) ? priceData.length : 0;
          totalCancelled += priceCancelled;
          cancelResults.push({ type: 'price_orders', cancelled: priceCancelled, status: 'SUCCESS' });
          console.log('🔥 GATE.IO v4: Price orders cancelled:', priceCancelled);
        } else {
          cancelResults.push({ type: 'price_orders', cancelled: 0, status: 'ERROR', error: priceData.message });
        }
...
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
      ...
      return {
        ...
        cancel_types: ['regular_orders', 'price_orders']
      };
    }
