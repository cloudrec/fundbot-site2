...
async function getBalance(apiKey: any, settings: any) {
  try {
    ...
    // 🆕 GATE.IO С ПРАВИЛЬНЫМ АДРЕСОМ GATE.COM
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing Gate.io balance request (GATE.COM)');
      ...
      console.log('🔥 Gate.io balance request URL:', baseUrl + url);
...
async function placeOrderWithTPSL(apiKey: any, settings: any) {
  try {
    ...
    // 🆕 GATE.IO С ПРАВИЛЬНЫМ АДРЕСОМ GATE.COM
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing Gate.io order with TP/SL (GATE.COM)');
      ...
      const priceResponse = await fetch(baseUrl + priceUrl);
      const priceData = await priceResponse.json();
      ...
      console.log('🔥 Gate.io order request URL:', baseUrl + url);
      ...
      return {
        ...
        side: "long",
        status: 'LIVE',
        message: `Боевой ордер Gate.io (gate.com): ${data.id}`,
        quantity: quantity,
        price: currentPrice,
        tp_price: tpPrice,
        sl_price: slPrice,
...
async function getPositions(apiKey: any, settings: any) {
  try {
    ...
    if (apiKey.exchange === 'gate') {
      ...
      console.log('🔥 Gate.io positions request URL:', baseUrl + url);
...
async function cancelAllOrders(apiKey: any, settings: any) {
  try {
    ...
    if (apiKey.exchange === 'gate') {
      ...
      console.log('🔥 Gate.io cancel orders request URL:', `${baseUrl}${url}?${queryString}`);
    ...
    if (apiKey.exchange === 'gate') {
      ...
      for (const position of positions) {
        try {
          ...
          console.log('🔥 Gate.io close position request URL:', baseUrl + url);
