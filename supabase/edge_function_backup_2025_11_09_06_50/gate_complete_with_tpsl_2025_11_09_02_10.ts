...
serve(async (req) => {
  ...
  try {
    ...
    console.log('🎯 GATE.IO COMPLETE: Starting action:', action);
    ...
    console.log('🎯 GATE.IO COMPLETE: Keys loaded, settings:', !!settings);
    ...
    console.error('❌ GATE.IO COMPLETE Error:', error.message);
    return new Response(
...
async function handleGetBalance(apiKeys: any) {
  console.log('🎯 GATE.IO COMPLETE: Getting balance');
...
async function handlePlaceOrderWithTPSL(apiKeys: any, settings: any) {
  console.log('🎯 GATE.IO COMPLETE: Placing order with TP/SL');
  
  if (!settings) {
    console.log('❌ GATE.IO COMPLETE: No settings found');
    return new Response(
      JSON.stringify({
        success: false,
  ...
  console.log('🎯 GATE.IO COMPLETE: Contract:', contract);
  ...
  console.log('🎯 GATE.IO COMPLETE: Order params:', {
    contract,
    size,
    currentPrice,
    orderAmount,
...
async function placeOrder(apiKeys: any, orderData: any) {
  ...
  console.log('🎯 GATE.IO COMPLETE: Placing order:', orderData);
  ...
  const responseData = await response.json();
  console.log('🎯 GATE.IO COMPLETE: Order response:', responseData);
...
async function handleGetPositions(apiKeys: any) {
  console.log('🎯 GATE.IO COMPLETE: Getting positions');
  ...
  const responseData = await response.json();
  console.log('🎯 GATE.IO COMPLETE: Positions response:', responseData);
  ...
  if (response.status === 200) {
    ...
    console.log('🎯 GATE.IO COMPLETE: Open positions found:', openPositions.length);
...
async function handleCancelAllOrders(apiKeys: any) {
  console.log('🎯 GATE.IO COMPLETE: Cancelling all orders');
  ...
  const responseData = await response.json();
  console.log('🎯 GATE.IO COMPLETE: Cancel orders response:', responseData);
...
async function handleCloseAllPositions(apiKeys: any) {
  console.log('🎯 GATE.IO COMPLETE: Closing all positions');
  ...
  for (const position of openPositions) {
    try {
      ...
      const closeResult = await placeOrder(apiKeys, closeOrderData);
