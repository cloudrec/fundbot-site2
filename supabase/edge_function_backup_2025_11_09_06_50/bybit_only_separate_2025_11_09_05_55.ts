...
serve(async (req) => {
  ...
  try {
    ...
    console.log('🔵 BYBIT ONLY FUNCTION: Starting action:', action);
    ...
    console.log('🔵 BYBIT ONLY: Keys loaded, settings:', !!settings);
...
// Получение баланса ТОЛЬКО Bybit
async function handleGetBalance(apiKeys: any) {
  console.log('🔵 BYBIT ONLY: Getting balance');
...
// Получение позиций ТОЛЬКО Bybit
async function handleGetPositions(apiKeys: any) {
  console.log('🔵 BYBIT ONLY: Getting positions');
  ...
  const url = `https://api.bybit.com/v2/private/position/list?${queryString}&sign=${signature}`;
  
  const response = await fetch(url);
  const data = await response.json();

  if (response.ok && data.ret_code === 0) {
    const activePositions = data.result?.filter((pos: any) => parseFloat(pos.size) > 0) || [];
    ...
    return new Response(
      JSON.stringify({
        success: false,
        error: `Bybit positions error: ${data.ret_msg || 'Unknown error'}`
      }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    );
  }
...
// Размещение ордера с TP/SL ТОЛЬКО Bybit
async function handlePlaceOrderWithTPSL(apiKeys: any, settings: any) {
  console.log('🔵 BYBIT ONLY: Placing order with TP/SL');
...
// Закрытие позиций ТОЛЬКО Bybit
async function handleClosePositions(apiKeys: any) {
  console.log('🔵 BYBIT ONLY: Closing positions');
...
// Отмена ордеров ТОЛЬКО Bybit
async function handleCancelOrders(apiKeys: any) {
  console.log('🔵 BYBIT ONLY: Canceling orders');
