...
serve(async (req) => {
  ...
  try {
    console.log('🔥 NEW Edge Function started - GATE.COM FIXED');
...
// 🆕🆕🆕 НОВАЯ ФУНКЦИЯ СОЗДАНИЯ ПОДПИСИ ДЛЯ GATE.COM 🆕🆕🆕
async function createGateSignature(secret: string, method: string, url: string, queryString: string, body: string): Promise<{ signature: string, timestamp: string }> {
  try {
    ...
    console.log('🔥 NEW Gate signature message:', message);
    ...
    console.log('🔥 NEW Gate signature result:', result);
    return { signature: result, timestamp };
  } catch (error) {
    console.error('❌ Error creating NEW Gate signature:', error);
    throw error;
  }
}
...
async function getBalance(apiKey: any, settings: any) {
  try {
    console.log('🔥 NEW getBalance started for exchange:', apiKey.exchange);
    ...
    // 🆕🆕🆕 НОВЫЙ GATE.COM КОД С ПРАВИЛЬНЫМИ АДРЕСАМИ 🆕🆕🆕
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing NEW Gate.com balance request');
      
      // 🆕 НОВЫЙ ПРАВИЛЬНЫЙ АДРЕС GATE.COM
      const GATE_API_BASE = 'https://api.gate.com';
      const GATE_TESTNET_BASE = 'https://fx-api-testnet.gate.com';
      ...
      console.log('🔥 NEW GATE: Base URL:', baseUrl);
      console.log('🔥 NEW GATE: API Path:', url);
      console.log('🔥 NEW GATE: Full URL will be:', baseUrl + url);
      
      const { signature, timestamp } = await createGateSignature(apiKey.api_secret, 'GET', url, queryString, body);
      
      const fullUrl = baseUrl + url;
      console.log('🔥 NEW Gate.com balance request FULL URL:', fullUrl);
      ...
      const data = await response.json();
      console.log('🔥 NEW Gate.com balance response status:', response.status);
      console.log('🔥 NEW Gate.com balance response:', data);

      if (response.status !== 200) {
        throw new Error(`Gate.com API Error: ${data.message || 'Unknown error'} (Code: ${response.status})`);
      }
      ...
      const result = {
        ...
        result: data,
        debug_url: fullUrl,
        api_version: 'NEW_GATE_COM_FIXED'
      };
      
      console.log('🔥 NEW Gate.com balance result:', result);
      return result;
    }
    ...
    console.error('🔥 NEW Balance error:', error);
...
async function placeOrderWithTPSL(apiKey: any, settings: any) {
  try {
    console.log('🔥 NEW placeOrderWithTPSL started for exchange:', apiKey.exchange);
    ...
    // 🆕🆕🆕 НОВЫЙ GATE.COM КОД С ПРАВИЛЬНЫМИ АДРЕСАМИ 🆕🆕🆕
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing NEW Gate.com order with TP/SL');
      ...
      console.log('🔥 NEW GATE ORDER: Base URL:', baseUrl);
      console.log('🔥 NEW GATE ORDER: Symbol:', symbol);
      
      // Получаем текущую цену
      const priceUrl = `/api/v4/futures/usdt/tickers?contract=${symbol}`;
      const fullPriceUrl = baseUrl + priceUrl;
      console.log('🔥 NEW GATE ORDER: Price URL:', fullPriceUrl);
      
      const priceResponse = await fetch(fullPriceUrl);
      const priceData = await priceResponse.json();
      
      console.log('🔥 NEW GATE ORDER: Price response status:', priceResponse.status);
      console.log('🔥 NEW GATE ORDER: Price data:', priceData);
      
      if (!priceData || priceData.length === 0) {
        throw new Error(`Не удалось получить цену для ${symbol} на NEW Gate.com`);
      }
      
      const currentPrice = parseFloat(priceData[0].last);
      console.log('🔥 NEW Gate.com current price:', currentPrice);
      ...
      console.log('🔥 NEW Gate.com order params:', { symbol, quantity, currentPrice, tpPrice, slPrice });
      ...
      const { signature, timestamp } = await createGateSignature(apiKey.api_secret, 'POST', url, '', body);
      
      const fullOrderUrl = baseUrl + url;
      console.log('🔥 NEW Gate.com order request FULL URL:', fullOrderUrl);
      ...
      const data = await response.json();
      console.log('🔥 NEW Gate.com order response status:', response.status);
      console.log('🔥 NEW Gate.com order response:', data);

      if (response.status !== 201) {
        throw new Error(`NEW Gate.com API Error: ${data.message || 'Unknown error'} (Code: ${response.status})`);
      }
      ...
      return {
        ...
        side: "long",
        status: 'LIVE',
        message: `Боевой ордер NEW Gate.com: ${data.id}`,
        quantity: quantity,
        price: currentPrice,
        tp_price: tpPrice,
        sl_price: slPrice,
        ...
        exchange: 'GATE',
        api_url: 'NEW_gate.com',
        debug_url: fullOrderUrl,
        api_version: 'NEW_GATE_COM_FIXED',
        note: 'TP/SL нужно установить вручную через интерфейс NEW Gate.com'
      };
    }
    ...
    console.error('🔥 NEW Set TP/SL error:', error);
    throw error;
  }
}
...
async function getPositions(apiKey: any, settings: any) {
  try {
    console.log('🔥 NEW getPositions started for exchange:', apiKey.exchange);
    ...
    if (apiKey.exchange === 'gate') {
      ...
      console.log('🔥 NEW GATE POSITIONS: Base URL:', baseUrl);
      
      const { signature, timestamp } = await createGateSignature(apiKey.api_secret, 'GET', url, queryString, body);
      
      const fullUrl = baseUrl + url;
      console.log('🔥 NEW Gate.com positions request FULL URL:', fullUrl);
      ...
      const data = await response.json();
      console.log('🔥 NEW Gate.com positions response:', data);
    ...
    console.error('🔥 NEW Positions error:', error);
...
async function cancelAllOrders(apiKey: any, settings: any) {
  try {
    console.log('🔥 NEW cancelAllOrders started for exchange:', apiKey.exchange);
    ...
    if (apiKey.exchange === 'gate') {
      ...
      console.log('🔥 NEW GATE CANCEL: Base URL:', baseUrl);
      
      const { signature, timestamp } = await createGateSignature(apiKey.api_secret, 'DELETE', url, queryString, body);
      
      const fullUrl = `${baseUrl}${url}?${queryString}`;
      console.log('🔥 NEW Gate.com cancel orders request FULL URL:', fullUrl);
      ...
      const data = await response.json();
      console.log('🔥 NEW Gate.com cancel orders response:', data);
      ...
      return {
        ...
        exchange: 'GATE',
        status: 'LIVE',
        message: `Отменено ${Array.isArray(data) ? data.length : 0} ордеров NEW Gate.com`,
        details: data
      };
    }
    ...
    console.error('🔥 NEW Cancel orders error:', error);
...
async function closeAllPositions(apiKey: any, settings: any) {
  try {
    console.log('🔥 NEW closeAllPositions started for exchange:', apiKey.exchange);
    ...
    if (apiKey.exchange === 'binance') {
      ...
      for (const position of positions) {
        try {
          ...
          // Задержка между ордерами (увеличенная для rate limit)
          await delay(3000);
        } catch (positionError) {
    ...
    if (apiKey.exchange === 'gate') {
      ...
      console.log('🔥 NEW GATE CLOSE: Base URL:', baseUrl);
      ...
      for (const position of positions) {
        try {
          ...
          const orderData = {
            ...
            size: -size, // Противоположный размер для закрытия
            price: "0", // Market order
            tif: "ioc",
            text: "NEW_close_position"
          };
          ...
          const fullUrl = baseUrl + url;
          console.log('🔥 NEW Gate.com close position request FULL URL:', fullUrl);
          ...
          const data = await response.json();
          console.log('🔥 NEW Gate.com close position response:', data);

          if (response.status === 201) {
            closedCount++;
            closeResults.push({ symbol: position.contract, status: 'SUCCESS', orderId: data.id });
          } else {
            closeResults.push({ symbol: position.contract, status: 'ERROR', error: data.message });
          }
          
          await delay(1000);
        } catch (positionError) {
          console.error('🔥 Error closing position:', position.contract, positionError);
          closeResults.push({ symbol: position.contract, status: 'ERROR', error: positionError.message });
        }
      }
      ...
      return {
        ...
        exchange: "GATE",
        status: "LIVE",
        message: `Закрыто ${closedCount} из ${positions.length} позиций NEW Gate.com`,
        details: closeResults
      };
    }
    ...
    console.error('🔥 NEW Close positions error:', error);
