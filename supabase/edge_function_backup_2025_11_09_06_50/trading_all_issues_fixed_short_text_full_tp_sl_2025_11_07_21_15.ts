...
serve(async (req) => {
  ...
  try {
    console.log('🔥 FIXED ALL ISSUES Gate.io Function started');
...
async function createFixedGateSignature(secret: string, method: string, url: string, queryString: string = '', payloadString: string = ''): Promise<{ signature: string, timestamp: string }> {
  try {
    // Используем текущее время в секундах (как в официальном примере)
    const timestamp = Math.floor(Date.now() / 1000).toString();
    
    console.log('🔥 FIXED GATE.IO v4 signature inputs:');
    ...
    console.log('🔥 FIXED GATE.IO v4 hashed payload:', hashedPayload);
    
    // ОФИЦИАЛЬНЫЙ ФОРМАТ СТРОКИ ПОДПИСИ ПО ДОКУМЕНТАЦИИ:
    // method + '\n' + url + '\n' + query_string + '\n' + hashed_payload + '\n' + timestamp
    const signatureString = `${method}\n${url}\n${queryString}\n${hashedPayload}\n${timestamp}`;
    
    console.log('🔥 FIXED GATE.IO v4 signature string:');
    ...
    console.log('🔥 FIXED GATE.IO v4 signature result:', result);
    return { signature: result, timestamp };
  } catch (error) {
    console.error('❌ Error creating FIXED GATE.IO v4 signature:', error);
    throw error;
  }
}
...
async function getBalance(apiKey: any, settings: any) {
  try {
    console.log('🔥 FIXED GATE.IO getBalance started for exchange:', apiKey.exchange);
    ...
    // 🆕 ИСПРАВЛЕННАЯ GATE.IO API V4 РЕАЛИЗАЦИЯ
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing FIXED GATE.IO API v4 balance request');
      ...
      console.log('🔥 FIXED GATE.IO v4: Base URL:', baseUrl);
      console.log('🔥 FIXED GATE.IO v4: Prefix:', prefix);
      console.log('🔥 FIXED GATE.IO v4: URL path:', url);
      console.log('🔥 FIXED GATE.IO v4: Full URL:', baseUrl + prefix + url);
      
      // Создаем подпись по официальному примеру
      const { signature, timestamp } = await createFixedGateSignature(apiKey.api_secret, 'GET', prefix + url, queryString, payloadString);
      
      const fullUrl = baseUrl + prefix + url;
      console.log('🔥 FIXED GATE.IO v4 balance request URL:', fullUrl);
      ...
      const data = await response.json();
      console.log('🔥 FIXED GATE.IO v4 balance response status:', response.status);
      console.log('🔥 FIXED GATE.IO v4 balance response:', data);

      if (response.status !== 200) {
        throw new Error(`FIXED GATE.IO v4 API Error: ${data.message || 'Unknown error'} (Code: ${response.status})`);
      }
      ...
      const result = {
        ...
        result: data,
        debug_url: fullUrl,
        api_version: 'FIXED_GATE_V4',
        base_url: baseUrl,
        signature_format: 'FIXED_DOCUMENTATION_BASED'
      };
      
      console.log('🔥 FIXED GATE.IO v4 balance result:', result);
      return result;
    }
    ...
    console.error('🔥 FIXED GATE.IO Balance error:', error);
...
async function placeOrderWithTPSL(apiKey: any, settings: any) {
  try {
    console.log('🔥 FIXED GATE.IO placeOrderWithTPSL started for exchange:', apiKey.exchange);
    ...
    // 🆕 ИСПРАВЛЕННАЯ GATE.IO API V4 РЕАЛИЗАЦИЯ С ПОЛНЫМИ TP/SL
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing FIXED GATE.IO API v4 order WITH FULL TP/SL');
      ...
      console.log('🔥 FIXED GATE.IO v4 ORDER: Base URL:', baseUrl);
      console.log('🔥 FIXED GATE.IO v4 ORDER: Symbol:', symbol);
      
      // Получаем текущую цену
      const priceUrl = `/futures/usdt/tickers?contract=${symbol}`;
      const fullPriceUrl = baseUrl + prefix + priceUrl;
      console.log('🔥 FIXED GATE.IO v4 ORDER: Price URL:', fullPriceUrl);
      
      const priceResponse = await fetch(fullPriceUrl);
      const priceData = await priceResponse.json();
      
      console.log('🔥 FIXED GATE.IO v4 ORDER: Price response status:', priceResponse.status);
      console.log('🔥 FIXED GATE.IO v4 ORDER: Price data:', priceData);
      
      if (!priceData || priceData.length === 0) {
        throw new Error(`Не удалось получить цену для ${symbol} на FIXED GATE.IO v4`);
      }
      
      const currentPrice = parseFloat(priceData[0].last);
      console.log('🔥 FIXED GATE.IO v4 current price:', currentPrice);
      ...
      console.log('🔥 FIXED GATE.IO v4 order params WITH TP/SL:', { 
        symbol, 
        quantity, 
        currentPrice,
      ...
      // 1. ОСНОВНОЙ ОРДЕР С КОРОТКИМ TEXT (ИСПРАВЛЕНО!)
      const orderData = {
        contract: symbol,
        size: quantity,
        price: "0", // Market order
        tif: "ioc",
        text: `t-${Date.now().toString().slice(-8)}` // 🔧 КОРОТКИЙ TEXT (ИСПРАВЛЕНО!)
      };
      
      console.log('🔥 FIXED GATE.IO v4 MAIN order data (SHORT TEXT):', orderData);
      
      const orderUrl = '/futures/usdt/orders';
      const payloadString = JSON.stringify(orderData);
      const { signature, timestamp } = await createFixedGateSignature(apiKey.api_secret, 'POST', prefix + orderUrl, '', payloadString);
      
      const fullOrderUrl = baseUrl + prefix + orderUrl;
      console.log('🔥 FIXED GATE.IO v4 order request URL:', fullOrderUrl);
      ...
      const data = await response.json();
      console.log('🔥 FIXED GATE.IO v4 order response status:', response.status);
      console.log('🔥 FIXED GATE.IO v4 order response:', data);
      ...
      console.log('🔥 FIXED GATE.IO v4 MAIN order created:', orderId);

      // Задержка перед установкой TP/SL
      console.log('🔥 Waiting 3 seconds before setting TP/SL...');
      await delay(3000);

      // 2. ПОЛНАЯ РЕАЛИЗАЦИЯ TP/SL ЧЕРЕЗ PRICE ORDERS API
      let tpStatus = 'NOT_SET';
      let slStatus = 'NOT_SET';
      let tpOrderId = null;
      let slOrderId = null;
      ...
      try {
        // TAKE PROFIT ORDER через Price Orders API
        const tpOrderData = {
          initial: {
            contract: symbol,
          ...
          trigger: {
            ...
            price: tpPriceNumber.toString(), // 🔧 ИСПРАВЛЕНО: СТРОКА, НЕ ЧИСЛО
            rule: 1 // 1 = >=
          }
        };

        console.log('🔥 FIXED GATE.IO v4 TP order data:', tpOrderData);

        const tpPayload = JSON.stringify(tpOrderData);
        const { signature: tpSig, timestamp: tpTs } = await createFixedGateSignature(apiKey.api_secret, 'POST', prefix + '/futures/usdt/price_orders', '', tpPayload);
        ...
        const tpData = await tpResponse.json();
        console.log('🔥 FIXED GATE.IO v4 TP response status:', tpResponse.status);
        console.log('🔥 FIXED GATE.IO v4 TP response:', tpData);

        if (tpResponse.status === 201) {
          tpStatus = 'SUCCESS';
          tpOrderId = tpData.id;
          console.log('🔥 FIXED GATE.IO v4 TP order created:', tpOrderId);
        } else {
          tpStatus = 'ERROR';
          tpError = tpData.message || 'Unknown TP error';
          console.error('🔥 FIXED GATE.IO v4 TP error:', tpError);
        }

        // Задержка между TP и SL
        await delay(2000);

        // STOP LOSS ORDER через Price Orders API
        const slOrderData = {
          initial: {
            contract: symbol,
            size: -quantity, // Противоположное направление для закрытия
          ...
          trigger: {
            strategy_type: 0, // 0 = by_price
            price_type: 0, // 0 = latest_price
            price: slPriceNumber.toString(), // 🔧 ИСПРАВЛЕНО: СТРОКА, НЕ ЧИСЛО
            rule: 2 // 2 = <=
          }
        };

        console.log('🔥 FIXED GATE.IO v4 SL order data:', slOrderData);

        const slPayload = JSON.stringify(slOrderData);
        const { signature: slSig, timestamp: slTs } = await createFixedGateSignature(apiKey.api_secret, 'POST', prefix + '/futures/usdt/price_orders', '', slPayload);
        ...
        const slData = await slResponse.json();
        console.log('🔥 FIXED GATE.IO v4 SL response status:', slResponse.status);
        console.log('🔥 FIXED GATE.IO v4 SL response:', slData);

        if (slResponse.status === 201) {
          slStatus = 'SUCCESS';
          slOrderId = slData.id;
          console.log('🔥 FIXED GATE.IO v4 SL order created:', slOrderId);
        } else {
          slStatus = 'ERROR';
          slError = slData.message || 'Unknown SL error';
          console.error('🔥 FIXED GATE.IO v4 SL error:', slError);
        }

      } catch (tpslError) {
        console.error('🔥 FIXED GATE.IO v4 TP/SL error:', tpslError);
        tpStatus = 'ERROR';
        slStatus = 'ERROR';
        tpError = tpslError.message;
        slError = tpslError.message;
      ...
      return {
        ...
        side: "long",
        status: 'LIVE',
        message: `Боевой ордер FIXED GATE.IO v4 с полными TP/SL: ${orderId}`,
        quantity: quantity,
        price: currentPrice,
        tp_price: tpPriceNumber.toFixed(4),
        sl_price: slPriceNumber.toFixed(4),
        tp_status: tpStatus,
        sl_status: slStatus,
        tp_error: tpError,
        sl_error: slError,
        exchange: 'GATE',
        api_url: 'FIXED_gateio.ws',
        debug_url: fullOrderUrl,
        api_version: 'FIXED_GATE_V4_WITH_FULL_TP_SL',
        base_url: baseUrl,
        signature_format: 'FIXED_DOCUMENTATION_BASED',
        text_field_fix: 'SHORT_TEXT_APPLIED',
        tp_sl_implementation: 'FULL_PRICE_ORDERS_API_WITH_NUMERIC_FIXES',
        tp_api_method: 'PRICE_ORDERS_API_TRIGGER_RULE_1_GTE',
        sl_api_method: 'PRICE_ORDERS_API_TRIGGER_RULE_2_LTE'
      };
    }
    ...
    console.error('🔥 FIXED GATE.IO Set TP/SL error:', error);
    throw error;
  }
}

// 🆕 ИСПРАВЛЕННАЯ РЕАЛИЗАЦИЯ ПОЛУЧЕНИЯ ПОЗИЦИЙ ДЛЯ GATE.IO
async function getPositions(apiKey: any, settings: any) {
  try {
    console.log('🔥 FIXED GATE.IO getPositions started for exchange:', apiKey.exchange);
    
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing FIXED GATE.IO API v4 positions request');
      ...
      console.log('🔥 FIXED GATE.IO v4 positions URL:', baseUrl + prefix + url);
      ...
      const data = await response.json();
      console.log('🔥 FIXED GATE.IO v4 positions response status:', response.status);
      console.log('🔥 FIXED GATE.IO v4 positions response:', data);

      if (response.status !== 200) {
        throw new Error(`FIXED GATE.IO v4 Positions API Error: ${data.message || 'Unknown error'} (Code: ${response.status})`);
      }

      // Фильтруем только открытые позиции
      const openPositions = data.filter((position: any) => parseFloat(position.size) !== 0);
      
      console.log('🔥 FIXED GATE.IO v4 open positions:', openPositions);
    ...
    console.error('🔥 FIXED GATE.IO Positions error:', error);
    throw new Error(`Ошибка получения позиций: ${error.message}`);
  }
}

// 🆕 ИСПРАВЛЕННАЯ РЕАЛИЗАЦИЯ ОТМЕНЫ ОРДЕРОВ ДЛЯ GATE.IO
async function cancelAllOrders(apiKey: any, settings: any) {
  try {
    console.log('🔥 FIXED GATE.IO cancelAllOrders started for exchange:', apiKey.exchange);
    
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing FIXED GATE.IO API v4 cancel orders request');
      ...
      try {
        ...
        const { signature: ordersSig, timestamp: ordersTs } = await createFixedGateSignature(apiKey.api_secret, 'DELETE', prefix + ordersUrl, '', '');
        
        console.log('🔥 FIXED GATE.IO v4 cancel regular orders URL:', baseUrl + prefix + ordersUrl);
        ...
        console.log('🔥 FIXED GATE.IO v4 cancel regular orders response status:', ordersResponse.status);
        console.log('🔥 FIXED GATE.IO v4 cancel regular orders response:', ordersData);
        ...
        console.error('🔥 FIXED GATE.IO v4 regular orders cancel error:', regularError);
        cancelResults.push({
          type: 'REGULAR_ORDERS',
          cancelled: 0,
          status: 'ERROR',
      ...
      try {
        ...
        const { signature: priceSig, timestamp: priceTs } = await createFixedGateSignature(apiKey.api_secret, 'DELETE', prefix + priceOrdersUrl, '', '');
        
        console.log('🔥 FIXED GATE.IO v4 cancel price orders URL:', baseUrl + prefix + priceOrdersUrl);
        ...
        const priceData = await priceResponse.json();
        console.log('🔥 FIXED GATE.IO v4 cancel price orders response status:', priceResponse.status);
        console.log('🔥 FIXED GATE.IO v4 cancel price orders response:', priceData);
        ...
        console.error('🔥 FIXED GATE.IO v4 price orders cancel error:', priceError);
        cancelResults.push({
          type: 'PRICE_ORDERS_TP_SL',
          cancelled: 0,
          status: 'ERROR',
      ...
      return {
        ...
        exchange: 'GATE',
        status: 'LIVE',
        message: `Отмена ордеров FIXED GATE.IO v4: ${totalCancelled} ордеров отменено`,
        api_version: 'FIXED_GATE_V4',
        symbol: symbol,
    ...
    console.error('🔥 FIXED GATE.IO Cancel orders error:', error);
...
async function closeAllPositions(apiKey: any, settings: any) {
  try {
    console.log('🔥 FIXED GATE.IO closeAllPositions started for exchange:', apiKey.exchange);
    
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing FIXED GATE.IO API v4 close positions request');
      ...
      const { signature: posSig, timestamp: posTs } = await createFixedGateSignature(apiKey.api_secret, 'GET', prefix + positionsUrl, '', '');
      
      console.log('🔥 FIXED GATE.IO v4 get positions URL:', baseUrl + prefix + positionsUrl);
      ...
      console.log('🔥 FIXED GATE.IO v4 positions response status:', positionsResponse.status);
      console.log('🔥 FIXED GATE.IO v4 positions response:', positionsData);

      if (positionsResponse.status !== 200) {
        throw new Error(`FIXED GATE.IO v4 Positions API Error: ${positionsData.message || 'Unknown error'} (Code: ${positionsResponse.status})`);
      }
      ...
      console.log('🔥 FIXED GATE.IO v4 open positions for', symbol, ':', openPositions);
      ...
      // 2. ИСПРАВЛЕННОЕ ЗАКРЫТИЕ ПОЗИЦИЙ С КОРОТКИМ TEXT
      ...
      for (const position of openPositions) {
        try {
          ...
          // ИСПРАВЛЕННЫЙ СПОСОБ ЗАКРЫТИЯ ПОЗИЦИЙ ЧЕРЕЗ MARKET ORDER С ПРОТИВОПОЛОЖНЫМ РАЗМЕРОМ
          const closeSize = -positionSize; // Противоположный размер для закрытия
          
          console.log('🔥 FIXED GATE.IO v4 closing position:', { 
          ...
          const closeOrderData = {
            ...
            price: "0", // Market order
            tif: "ioc",
            text: `c-${Date.now().toString().slice(-8)}` // 🔧 КОРОТКИЙ TEXT ДЛЯ ЗАКРЫТИЯ (ИСПРАВЛЕНО!)
          };

          console.log('🔥 FIXED GATE.IO v4 close order data (SHORT TEXT):', closeOrderData);

          const closePayload = JSON.stringify(closeOrderData);
          const { signature: closeSig, timestamp: closeTs } = await createFixedGateSignature(apiKey.api_secret, 'POST', prefix + '/futures/usdt/orders', '', closePayload);
          ...
          console.log('🔥 FIXED GATE.IO v4 close position response status:', closeResponse.status);
          console.log('🔥 FIXED GATE.IO v4 close position response:', closeData);
          ...
          if (closeResponse.status === 201) {
            ...
            console.log('🔥 FIXED GATE.IO v4 position closed successfully:', closeData.id);
          } else {
            closeResults.push({
              contract: position.contract,
              status: 'ERROR',
            ...
            console.error('🔥 FIXED GATE.IO v4 close position error:', closeData.message);
          }
          ...
          console.error('🔥 FIXED GATE.IO v4 close position error:', closeError);
          closeResults.push({
            contract: position.contract,
            status: 'ERROR',
      ...
      return {
        ...
        exchange: 'GATE',
        status: 'LIVE',
        message: `Закрытие позиций FIXED GATE.IO v4: ${closedPositions}/${openPositions.length} успешно`,
        api_version: 'FIXED_GATE_V4',
        symbol: symbol,
        close_method: 'FIXED_MARKET_ORDER_WITH_OPPOSITE_SIZE_SHORT_TEXT',
        text_field_fix: 'SHORT_TEXT_FOR_CLOSE_ORDERS'
      };
    }
    ...
    console.error('🔥 FIXED GATE.IO Close positions error:', error);
