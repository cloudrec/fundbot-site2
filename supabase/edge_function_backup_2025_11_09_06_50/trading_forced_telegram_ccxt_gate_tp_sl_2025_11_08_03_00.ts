...
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
    ...
    // 🔵 GATE.IO IMPLEMENTATION - CCXT-ПОДОБНЫЙ ПОДХОД ДЛЯ TP/SL
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing GATE.IO API v4 order with CCXT-LIKE TP/SL APPROACH');
      ...
      console.log('🔥 GATE.IO v4 CCXT ORDER: Base URL:', baseUrl);
      console.log('🔥 GATE.IO v4 CCXT ORDER: Symbol:', symbol);
      
      // Получаем текущую цену
      const priceUrl = `/futures/usdt/tickers?contract=${symbol}`;
      const fullPriceUrl = baseUrl + prefix + priceUrl;
      console.log('🔥 GATE.IO v4 CCXT ORDER: Price URL:', fullPriceUrl);
      
      const priceResponse = await fetch(fullPriceUrl);
      const priceData = await priceResponse.json();
      
      console.log('🔥 GATE.IO v4 CCXT ORDER: Price response status:', priceResponse.status);
      console.log('🔥 GATE.IO v4 CCXT ORDER: Price data:', priceData);
      ...
      console.log('🔥 GATE.IO v4 CCXT current price:', currentPrice);
      ...
      console.log('🔥 GATE.IO v4 CCXT order params:', { 
        symbol, 
        quantity, 
        currentPrice,
        tpPrice,
      ...
      // 1. ОСНОВНОЙ MARKET ОРДЕР С CCXT-ПОДОБНЫМИ ПАРАМЕТРАМИ TP/SL
      const orderData = {
        contract: symbol,
        size: quantity,
        price: "0", // Market order
        tif: "ioc",
        text: `t-ccxt-${Date.now().toString().slice(-6)}`, // Gate.io требует префикс t-
      ...
      console.log('🔥 GATE.IO v4 CCXT main order data:', orderData);
      ...
      console.log('🔥 GATE.IO v4 CCXT order request URL:', fullOrderUrl);
      ...
      const data = await response.json();
      console.log('🔥 GATE.IO v4 CCXT order response status:', response.status);
      console.log('🔥 GATE.IO v4 CCXT order response:', data);

      if (response.status !== 201) {
        // Если основной подход не работает, пробуем простой ордер без TP/SL
        console.log('🔥 GATE.IO v4 CCXT: Main approach failed, trying simple order...');
        ...
        const simpleOrderData = {
          ...
          price: "0", // Market order
          tif: "ioc",
          text: `t-simple-${Date.now().toString().slice(-6)}` // Gate.io требует префикс t-
        };
        ...
        console.log('🔥 GATE.IO v4 CCXT simple order response status:', simpleResponse.status);
        console.log('🔥 GATE.IO v4 CCXT simple order response:', simpleData);

        if (simpleResponse.status !== 201) {
          throw new Error(`GATE.IO v4 CCXT API Error: ${simpleData.message || data.message || 'Unknown error'} (Code: ${simpleResponse.status})`);
        }

        const orderId = simpleData.id;
        console.log('🔥 GATE.IO v4 CCXT simple order created:', orderId);

        // 📱 ОТПРАВЛЯЕМ TELEGRAM УВЕДОМЛЕНИЕ ДЛЯ ПРОСТОГО ОРДЕРА
        const telegramMessage = `
<b>🚀 ОРДЕР РАЗМЕЩЕН</b>
<b>Биржа:</b> Gate.io (CCXT-подход)
<b>Символ:</b> ${symbol}
<b>Сторона:</b> LONG
<b>Количество:</b> ${quantity}
<b>Цена входа:</b> $${currentPrice.toFixed(4)}
...
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
        ...
        return {
          ...
          side: "long",
          status: 'LIVE',
          message: `Боевой ордер GATE.IO v4 (CCXT-подход, простой): ${orderId}`,
          quantity: quantity,
          price: currentPrice,
          tp_price: tpPrice,
          sl_price: slPrice,
          ...
          api_version: 'GATE_V4_CCXT_SIMPLE_FALLBACK',
          base_url: baseUrl,
          signature_format: 'COMPLETE_DOCUMENTATION_BASED',
          text_field_fix: 'CCXT_PREFIX_APPLIED',
          tp_sl_method: 'MANUAL_SET_REQUIRED_CCXT_FAILED'
        };
      }

      const orderId = data.id;
      console.log('🔥 GATE.IO v4 CCXT order with TP/SL created:', orderId);

      // 📱 ОТПРАВЛЯЕМ TELEGRAM УВЕДОМЛЕНИЕ ДЛЯ УСПЕШНОГО CCXT ОРДЕРА
      const telegramMessage = `
<b>🚀 ОРДЕР РАЗМЕЩЕН</b>
<b>Биржа:</b> Gate.io (CCXT-подход)
<b>Символ:</b> ${symbol}
...
<b>Время:</b> ${new Date().toLocaleString('ru-RU', { timeZone: 'UTC' })} UTC
      ...
      return {
        ...
        side: "long",
        status: 'LIVE',
        message: `Боевой ордер GATE.IO v4 с CCXT TP/SL: ${orderId}`,
        quantity: quantity,
        price: currentPrice,
        tp_price: tpPrice,
        sl_price: slPrice,
        ...
        api_url: 'gateio.ws',
        debug_url: fullOrderUrl,
        api_version: 'GATE_V4_CCXT_TP_SL_SUCCESS',
        base_url: baseUrl,
        signature_format: 'COMPLETE_DOCUMENTATION_BASED',
        text_field_fix: 'CCXT_PREFIX_APPLIED',
        tp_sl_method: 'CCXT_INTEGRATED_TP_SL'
      };
    }
      ...
      for (const position of openPositions) {
        try {
          ...
          const closeOrderData = {
            ...
            price: "0", // Market order
            tif: "ioc",
            text: `t-close-${Date.now().toString().slice(-6)}`, // Gate.io требует префикс t-
