...
serve(async (req) => {
  ...
  try {
    console.log('🔧 FIXED BYBIT SIGNATURE Function started');
    ...
    console.error('❌ FIXED BYBIT SIGNATURE Error:', error.message);
    return new Response(
...
// Bybit баланс с исправленной подписью
async function getBybitBalance(apiKeys: any) {
  console.log('🔧 Getting Bybit balance with FIXED signature');
  ...
  // Правильная подпись для Bybit согласно документации
  console.log('🔧 Creating FIXED Bybit signature...');
  const { signature } = await createFixedBybitSignature(apiKeys.api_key, apiKeys.api_secret, timestamp, recvWindow);
...
async function getGateBalance(apiKeys: any) {
  console.log('🔧 Gate.io balance - basic version');
...
// ИСПРАВЛЕННАЯ Bybit подпись согласно официальной документации
async function createFixedBybitSignature(apiKey: string, secret: string, timestamp: string, recvWindow: string) {
  console.log('🔧 Creating FIXED Bybit signature with correct format');
  
  // Правильный формат для Bybit V5 API согласно документации:
  // timestamp + apiKey + recvWindow + queryString (для GET запросов без body)
  const queryString = 'category=unified';
  const message = timestamp + apiKey + recvWindow + queryString;
  
  console.log('🔧 Bybit signature components:', {
    timestamp,
  ...
  console.log('🔧 FIXED Bybit signature created:', result.substring(0, 20) + '...');
