...
serve(async (req) => {
  ...
  try {
    console.log('🔥 WORKING ORDERS Function started');
    ...
    console.error('❌ WORKING ORDERS Error:', error.message);
    return new Response(
...
async function scanFunding() {
  ...
  try {
    ...
    return {
      message: 'WORKING: Фандинг сканирование выполнено',
...
async function placeTestOrder(apiKeys: any, settings: any) {
  return {
    message: `WORKING: Тестовый ордер на ${settings.exchange.toUpperCase()}`,
    exchange: settings.exchange.toUpperCase(),
    symbol: `${settings.base_asset}${settings.quote_asset}`,
    amount: settings.order_amount_usd,
    leverage: settings.leverage,
...
// РАБОЧЕЕ размещение ордера с TP/SL
async function placeOrderWithTPSL(apiKeys: any, settings: any) {
  console.log('🔥 WORKING placeOrderWithTPSL started for exchange:', apiKeys.exchange);
  ...
  if (apiKeys.exchange === 'bybit') {
    ...
    return {
      ...
      side: "Buy",
      status: 'LIVE',
      message: `РАБОЧИЙ ордер Bybit с TP/SL: ${data.result.orderId}`,
      quantity: quantity,
      price: currentPrice,
      tp_price: tpPrice,
      sl_price: slPrice,
...
async function cancelAllOrders(apiKeys: any, settings: any) {
  return {
    message: `WORKING: Отмена ордеров на ${settings.exchange.toUpperCase()}`,
    cancelled_orders: 0,
    exchange: settings.exchange.toUpperCase()
  };
}
...
async function closeAllPositions(apiKeys: any, settings: any) {
  return {
    message: `WORKING: Закрытие позиций на ${settings.exchange.toUpperCase()}`,
    closed_positions: 0,
    exchange: settings.exchange.toUpperCase()
  };
}
...
// Bybit подпись для POST запросов
async function createBybitSignatureV2(secret: string, timestamp: string, apiKey: string, body: string) {
  ...
  console.log('🔥 Bybit POST signature message preview:', message.substring(0, 100) + '...');
