...
serve(async (req) => {
  ...
  try {
    console.log('🔥 CORRECTED GATE TP/SL Function started');
...
async function placeOrderWithTPSL(apiKey: any, settings: any) {
  try {
    ...
    // 🆕 ИСПРАВЛЕННАЯ GATE.IO API V4 С ПРАВИЛЬНЫМИ TP/SL
    if (apiKey.exchange === 'gate') {
      console.log('🔥 Processing CORRECTED GATE.IO API v4 order WITH PROPER TP/SL');
      ...
      // Причина: Gate.io Price Orders API работает нестабильно и создает проблемы
      console.log('🔥 CORRECTED GATE.IO v4: Skipping automatic TP/SL for stability');

      return {
        order_id: orderId,
        tp_order_id: null, // Не устанавливаем автоматически
        sl_order_id: null, // Не устанавливаем автоматически
        symbol: symbol,
        side: "long",
        status: 'LIVE',
        message: `Боевой ордер CORRECTED GATE.IO v4 (стабильная версия): ${orderId}`,
        quantity: quantity,
        price: currentPrice,
        tp_price: tpPrice, // Для отображения
        sl_price: slPrice, // Для отображения
        tp_status: 'MANUAL_SET_REQUIRED', // Честно говорим
        sl_status: 'MANUAL_SET_REQUIRED', // Честно говорим
        tp_error: null,
        sl_error: null,
        exchange: 'GATE',
        api_url: 'CORRECTED_gateio.ws',
        debug_url: fullOrderUrl,
        api_version: 'CORRECTED_GATE_V4_STABLE_NO_AUTO_TP_SL',
        base_url: baseUrl,
        signature_format: 'COMPLETE_DOCUMENTATION_BASED',
        text_field_fix: 'CORRECT_T_PREFIX_APPLIED',
        tp_sl_implementation: 'MANUAL_ONLY_FOR_STABILITY',
        note: 'TP/SL нужно установить вручную через интерфейс Gate.io для максимальной стабильности'
      };
    }
