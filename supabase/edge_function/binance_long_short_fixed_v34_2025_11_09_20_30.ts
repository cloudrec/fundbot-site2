...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`🟨 BINANCE V60 RESTORED: ${action} - ${order_type} direction`);
    ...
    console.error('❌ BINANCE V60 RESTORED Error:', error.message);
    return new Response(
...
async function handleBinanceBalance(apiKeys: any) {
  console.log('🟨 BINANCE V60 RESTORED: Getting balance');
  ...
  try {
    ...
    console.log('🟨 BINANCE V60 RESTORED: Balance extracted:', balance);
    ...
    console.error('❌ BINANCE V60 RESTORED balance error:', error.message);
    return new Response(
...
// Размещение ордера с TP/SL (версия v60)
async function handleBinanceOrderWithTPSL(apiKeys: any, settings: any, orderType: string = 'LONG') {
  console.log(`🟨 BINANCE V60 RESTORED: Placing ${orderType} order with TP/SL`);
  ...
  try {
    ...
    if (positionSize < 10) {
      positionSize = 10;
      console.log('🟨 BINANCE V60 RESTORED: Position size increased to minimum 10 USDT');
    }
    
    console.log(`🟨 BINANCE V60 RESTORED: Balance: ${availableBalance}, Leverage: ${leverage}x, Risk: ${riskPercent * 100}%, Position Size: ${positionSize} USDT`);
    ...
    // Рассчитываем quantity (простой подход v60)
    const rawQuantity = positionSize / currentPrice;
    const finalQuantity = parseFloat(rawQuantity.toFixed(6)); // Простое округление до 6 знаков
    
    console.log(`🟨 BINANCE V60 RESTORED: Price: ${currentPrice}, Raw Quantity: ${rawQuantity}, Final Quantity: ${finalQuantity}`);
    ...
    console.log(`🟨 BINANCE V60 RESTORED: TP: ${tpPrice}, SL: ${slPrice}, Quantity: ${finalQuantity}`);
    ...
    console.log(`🟨 BINANCE V60 RESTORED: Order params: ${orderParams}`);
    ...
    console.log('🟨 BINANCE V60 RESTORED: Order response:', JSON.stringify(orderData, null, 2));
    
    if (orderData.code) {
      throw new Error(`Binance order error: ${orderData.msg}`);
    }

    console.log('🟨 BINANCE V60 RESTORED: Main order placed successfully');
    ...
    if (tpData.code) {
      console.log('⚠️ BINANCE V60 RESTORED: TP order failed:', tpData.msg);
    } else {
      console.log('🟨 BINANCE V60 RESTORED: TP order placed:', tpData.orderId);
    }
    ...
    if (slData.code) {
      console.log('⚠️ BINANCE V60 RESTORED: SL order failed:', slData.msg);
    } else {
      console.log('🟨 BINANCE V60 RESTORED: SL order placed:', slData.orderId);
    }
    
    console.log('🟨 BINANCE V60 RESTORED: All orders completed');
    ...
    console.error('❌ BINANCE V60 RESTORED Order error:', error.message);
    return new Response(
...
async function handleBinancePositions(apiKeys: any) {
  console.log('🟨 BINANCE V60 RESTORED: Getting positions');
  ...
  try {
    ...
    console.log(`🟨 BINANCE V60 RESTORED: Found ${openPositions.length} open positions`);
    ...
    console.error('❌ BINANCE V60 RESTORED Positions error:', error.message);
    return new Response(
...
async function handleBinanceClosePositions(apiKeys: any) {
  console.log('🟨 BINANCE V60 RESTORED: Closing all positions');
  ...
  try {
    ...
    console.log(`🟨 BINANCE V60 RESTORED: Closed ${successCount} of ${closeResults.length} positions`);
    ...
    console.error('❌ BINANCE V60 RESTORED Close positions error:', error.message);
    return new Response(
...
async function handleBinanceCancelOrders(apiKeys: any) {
  console.log('🟨 BINANCE V60 RESTORED: Canceling orders');
  ...
  try {
    ...
    console.error('❌ BINANCE V60 RESTORED Cancel orders error:', error.message);
    return new Response(
