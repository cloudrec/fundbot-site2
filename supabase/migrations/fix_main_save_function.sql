-- Исправляем основную функцию по образцу рабочей тестовой
...
BEGIN
    ...
    -- Удаляем старые настройки пользователя (как в рабочей тестовой функции)
    DELETE FROM public.trading_settings_dev WHERE user_id = p_user_id;
    
    -- Вставляем новые настройки (точно так же как в тестовой функции)
    ...
    -- Возвращаем простой текст как в тестовой функции
