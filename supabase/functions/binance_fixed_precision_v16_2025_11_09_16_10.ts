...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`🟢 BINANCE v16: ${action} - ${order_type} direction`);
    ...
    console.error('❌ BINANCE v16 Error:', error.message);
    return new Response(
...
async function handleBinanceBalance(apiKeys: any) {
  console.log('🟢 BINANCE v16: Getting balance');
  ...
  try {
    ...
    console.log('🟢 BINANCE v16: API Response:', JSON.stringify(data, null, 2));
    ...
    console.log('🟢 BINANCE v16: Balance extracted:', balance);
    ...
    console.error('❌ BINANCE v16 balance error:', error.message);
    return new Response(
...
// ИСПРАВЛЕННАЯ функция размещения ордера с TP/SL
async function handleBinanceOrderWithTPSL(apiKeys: any, settings: any, orderType: string = 'LONG') {
  console.log(`🟢 BINANCE v16: Placing ${orderType} order with TP/SL`);
  ...
  try {
    ...
    if (positionSize < 10) {
      positionSize = 10;
      console.log('🟢 BINANCE v16: Position size increased to minimum 10 USDT');
    }
    
    positionSize = parseFloat(positionSize.toFixed(2));
    
    console.log(`🟢 BINANCE v16: Balance: ${availableBalance}, Leverage: ${leverage}x, Risk: ${riskPercent * 100}%, Size: ${positionSize}`);
    ...
    console.log(`🟢 BINANCE v16: Symbol: ${symbol}, Price: ${currentPrice}, TP: ${tpPrice}, SL: ${slPrice}`);
    
    // Получаем информацию о символе для правильного количества
    const exchangeInfoResponse = await fetch(`https://fapi.binance.com/fapi/v1/exchangeInfo?symbol=${symbol}`);
    const exchangeInfo = await exchangeInfoResponse.json();
    
    if (exchangeInfo.code) {
      throw new Error(`Не удалось получить информацию о символе: ${exchangeInfo.msg}`);
    }
    ...
    // ИСПРАВЛЕННЫЙ расчет количества с учетом минимального количества
    const rawQuantity = positionSize / currentPrice;
    
    // Убеждаемся что количество больше минимального
    const adjustedQuantity = Math.max(rawQuantity, minQty);
    
    // Округляем до правильного stepSize
    const quantity = Math.floor(adjustedQuantity / stepSize) * stepSize;
    
    // Определяем количество знаков после запятой
    const stepSizeDecimals = stepSize.toString().split('.')[1]?.length || 0;
    const finalQuantity = parseFloat(quantity.toFixed(stepSizeDecimals));
    
    console.log(`🟢 BINANCE v16: Raw: ${rawQuantity}, Min: ${minQty}, Step: ${stepSize}, Decimals: ${stepSizeDecimals}, Final: ${finalQuantity}`);
    
    if (finalQuantity < minQty) {
      throw new Error(`Количество ${finalQuantity} меньше минимального ${minQty}`);
    }
    ...
    console.log('🟢 BINANCE v16: Order response:', JSON.stringify(orderData, null, 2));
    
    if (orderData.code) {
      throw new Error(`Binance order error: ${orderData.msg}`);
    }

    console.log('🟢 BINANCE v16: Main order placed successfully');
    ...
    if (tpData.code) {
      console.log('⚠️ BINANCE v16: TP order failed:', tpData.msg);
    } else {
      console.log('🟢 BINANCE v16: TP order placed:', tpData.orderId);
    }
    ...
    if (slData.code) {
      console.log('⚠️ BINANCE v16: SL order failed:', slData.msg);
    } else {
      console.log('🟢 BINANCE v16: SL order placed:', slData.orderId);
    }
    
    console.log('🟢 BINANCE v16: All orders completed');
    ...
    console.error('❌ BINANCE v16 Order error:', error.message);
    return new Response(
...
async function handleBinancePositions(apiKeys: any) {
  console.log('🟢 BINANCE v16: Getting positions');
  ...
  try {
    ...
    console.log(`🟢 BINANCE v16: Found ${openPositions.length} open positions`);
    ...
    console.error('❌ BINANCE v16 Positions error:', error.message);
    return new Response(
...
async function handleBinanceClosePositions(apiKeys: any) {
  console.log('🟢 BINANCE v16: Closing all positions');
  ...
  try {
    ...
    console.log(`🟢 BINANCE v16: Closed ${successCount} of ${closeResults.length} positions`);
    ...
    console.error('❌ BINANCE v16 Close positions error:', error.message);
    return new Response(
...
async function handleBinanceCancelOrders(apiKeys: any) {
  console.log('🟢 BINANCE v16: Canceling orders');
  ...
  try {
    ...
    console.error('❌ BINANCE v16 Cancel orders error:', error.message);
    return new Response(
