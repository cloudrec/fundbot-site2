...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`🟨 BINANCE NOTIONAL FORCED: ${action} - ${order_type} direction`);
    ...
    console.error('❌ BINANCE NOTIONAL FORCED Error:', error.message);
    return new Response(
...
async function handleBinanceBalance(apiKeys: any) {
  console.log('🟨 BINANCE NOTIONAL FORCED: Getting balance');
  ...
  try {
    ...
    console.log('🟨 BINANCE NOTIONAL FORCED: Balance extracted:', balance);
    ...
    console.error('❌ BINANCE NOTIONAL FORCED balance error:', error.message);
    return new Response(
...
// НОВАЯ функция размещения ордера с NOTIONAL (размер в USDT)
async function handleBinanceOrderWithTPSL(apiKeys: any, settings: any, orderType: string = 'LONG') {
  console.log(`🟨 BINANCE NOTIONAL FORCED: Placing ${orderType} order with TP/SL using NOTIONAL`);
  ...
  try {
    ...
    if (positionSize < 10) {
      positionSize = 10;
      console.log('🟨 BINANCE NOTIONAL FORCED: Position size increased to minimum 10 USDT');
    }
    
    // Округляем до 2 знаков после запятой для USDT
    const notionalSize = parseFloat(positionSize.toFixed(2));
    
    console.log(`🟨 BINANCE NOTIONAL FORCED: Balance: ${availableBalance}, Leverage: ${leverage}x, Risk: ${riskPercent * 100}%, Notional Size: ${notionalSize} USDT`);
    ...
    console.log(`🟨 BINANCE NOTIONAL FORCED: Symbol: ${symbol}, Price: ${currentPrice}, TP: ${tpPrice}, SL: ${slPrice}, Notional: ${notionalSize} USDT`);
    
    // 1. Размещаем основной ордер с NOTIONAL (размер в USDT)
    const timestamp1 = Date.now();
    const orderParams = `symbol=${symbol}&side=${orderType === 'LONG' ? 'BUY' : 'SELL'}&type=MARKET&quoteOrderQty=${notionalSize}&timestamp=${timestamp1}`;
    const signature1 = await createBinanceSignature(orderParams, apiKeys.api_secret);
    
    console.log(`🟨 BINANCE NOTIONAL FORCED: Order params: ${orderParams}`);
    ...
    console.log('🟨 BINANCE NOTIONAL FORCED: Order response:', JSON.stringify(orderData, null, 2));
    
    if (orderData.code) {
      throw new Error(`Binance order error: ${orderData.msg}`);
    }

    console.log('🟨 BINANCE NOTIONAL FORCED: Main order placed successfully');
    ...
    if (tpData.code) {
      console.log('⚠️ BINANCE NOTIONAL FORCED: TP order failed:', tpData.msg);
    } else {
      console.log('🟨 BINANCE NOTIONAL FORCED: TP order placed:', tpData.orderId);
    }
    ...
    if (slData.code) {
      console.log('⚠️ BINANCE NOTIONAL FORCED: SL order failed:', slData.msg);
    } else {
      console.log('🟨 BINANCE NOTIONAL FORCED: SL order placed:', slData.orderId);
    }
    
    console.log('🟨 BINANCE NOTIONAL FORCED: All orders completed');
    ...
    console.error('❌ BINANCE NOTIONAL FORCED Order error:', error.message);
    return new Response(
...
async function handleBinancePositions(apiKeys: any) {
  console.log('🟨 BINANCE NOTIONAL FORCED: Getting positions');
  ...
  try {
    ...
    console.log(`🟨 BINANCE NOTIONAL FORCED: Found ${openPositions.length} open positions`);
    ...
    console.error('❌ BINANCE NOTIONAL FORCED Positions error:', error.message);
    return new Response(
...
async function handleBinanceClosePositions(apiKeys: any) {
  console.log('🟨 BINANCE NOTIONAL FORCED: Closing all positions');
  ...
  try {
    ...
    console.log(`🟨 BINANCE NOTIONAL FORCED: Closed ${successCount} of ${closeResults.length} positions`);
    ...
    console.error('❌ BINANCE NOTIONAL FORCED Close positions error:', error.message);
    return new Response(
...
async function handleBinanceCancelOrders(apiKeys: any) {
  console.log('🟨 BINANCE NOTIONAL FORCED: Canceling orders');
  ...
  try {
    ...
    console.error('❌ BINANCE NOTIONAL FORCED Cancel orders error:', error.message);
    return new Response(
